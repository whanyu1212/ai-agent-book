...
[^ch1-agent-interface]: John L. Hennessy and David A. Patterson, *Computer Architecture: A Quantitative Approach*, 6th ed., Morgan Kaufmann, 2019, Chapter 1, “What Is Computer Architecture?”. Kitap komut kümesi mimarisi, bilgisayar organizasyonu ve donanım uygulamasını birbirinden ayırır; yazılım ile donanım arasındaki arayüz özellikle komut kümesi mimarisidir. Bkz. https://shop.elsevier.com/books/computer-architecture/hennessy/978-0-12-811905-1
...
>
> Bu deneyin özü olan model yönlendirmeli çok turlu arama, sunucu tarafında kapalı döngü kod yürütme ve yürütmeden önce niyeti netleştirme, tek bir sağlayıcıya bağlı değildir. OpenAI kredisi olmayan okuyucular, eşdeğer yönetilen araçlar sunan bir sağlayıcıyla deneyi yeniden üretebilir. Örneğin Alibaba Cloud Bailian'ın qwen3.7-plus Responses API'si de yerleşik `web_search` ve `code_interpreter` sunar; bu bölümün eşlik eden kodu (`chapter1/search-codegen`), yukarıdaki ASEAN başkentleri arasındaki mesafe hesabını ve Bitcoin teknik analizini bu API ile gerçekten çalıştırmıştır. Kimi K3'ün Formula tarafından yönetilen araması ve `code_runner` aracı da aynı sınıfta yetenekler sağlar.
>
...
LoopX bu fikrin somut bir framework'üdür: hedefleri, kapıları, yapılacak işleri, kanıtları, kotayı, devirleri, doğrulamayı ve durma koşullarını model ile sohbetin dışındaki kalıcı bir kontrol düzlemine taşır; böylece farklı runtime'lardaki Agent'lar aynı kurtarılabilir ve incelenebilir döngü üzerinde çalışabilir. Bölüm 10, Loop Engineering'in gerçek bir sisteme nasıl dönüştüğünü LoopX üzerinden gösterir.

Temmuz 2026'da sektör, daha üst düzey bir orkestrasyon perspektifi için **Graph Engineering** terimini kullanmaya başladı: Agent döngülerini, deterministik programları ve insan onaylarını açık bir execution graph içinde düzenlemek; burada node'lar yetenekleri sağlar, edge'ler yönlendirme ile bağımlılıkları tanımlar ve yapılandırılmış state bu edge'ler boyunca taşınıp önemli sınırlarda kalıcılaştırılır.[^ch1-graph-engineering] Graph Engineering, Loop Engineering'in yerini almaz ve yukarıdaki ilerleyişin basitçe “altıncı katmanı” olarak görülmemelidir. Bir loop zaten geri dönen bir edge'e sahip graph'tır ve graph içindeki bir node, kendi içinde ReAct veya başka bir Agent döngüsü çalıştırabilir. Ad henüz yerleşmediği için bu kitap onu mevcut orkestrasyon ve Harness pratiklerine ait yeni bir terminoloji olarak ele alır; Bölüm 10 bunun multi-agent bölümünü geliştirir. Buradaki “graph”, GraphRAG'ın kullandığı bilgi grafı değil, kontrol akışı veya yürütme grafıdır.
...
Guardrail'lerin başka bir hata biçimi daha vardır: **yanlış ret**. Tehlikeli istekleri geçirme olasılığını azaltmaya çalışırken model, yetkili güvenlik testleri, model değerlendirmesi veya damıtma araştırması gibi meşru fakat hassas görünen işleri de reddedebilir. Bu nedenle guardrail değerlendirmesi yalnızca yasak isteklerin engellenip engellenmediğini değil, açıkça izin verilen isteklerin hâlâ tamamlanıp tamamlanamadığını da sınamalıdır. Meşru fakat hassas görevlerde ideal sistem yalnızca genel bir ret vermek yerine açıklama, insana eskalasyon veya yetkilendirilmiş bir yürütme yolu sunmalıdır.
