...
Daha kritik olarak, Agent dosya yazabildiği için kendi harici artefaktlarını değiştirecek teknik araca sahiptir. Bir Agent bir görevi ilk kez yerine getirirken daha önce bilmediği kilit bir bilgiyi keşfettiğinde—örneğin belirli bir bankayı ararken bankanın kimlik doğrulaması için şube adresini istediğini öğrendiğinde—bu keşfi önce bir kayda yazabilir. Böyle bir kaydın ne zaman güvenilir bilgiye, talimata veya programa dönüşmek için yeterli olduğuna karar vermek ise ek trajectory'ler ve sonuç doğrulaması gerektirir. Bu, Bölüm 8'de ele alınan sürekli evrim problemidir.
...
Farklı modeller bu iş akışını farklı biçimlerde kısaltır. Bazı Coding modelleri ilk düzenlemeden önce depo yapısını, uygulamayı, çağrı noktalarını ve testleri genişçe okur. Bazıları ise yalnızca ilgili olma olasılığı en yüksek birkaç dosyayı inceler, erken bir yama yapar ve derleyici ile test geri bildirimini araştırmanın bir parçası olarak kullanır. Bilgi toplamayı ne zaman bırakıp harekete geçileceğini belirleyen bu eşik, Harness değişse bile modeli izlemeye devam edebilir ve aynı Harness içinde yalnızca model değiştirildiğinde farklılaşabilir. Bu nedenle bu, yalnızca bir Coding ürününün arayüz tarzı değil, öncelikle **modelin öğrendiği bir davranıştır**. Harness içindeki prompt'lar, araçlar ve bütçeler bu eğilimi güçlendirebilir ya da bastırabilir, ancak kaynağı olmak zorunda değildir. Deney 6-7 bu farkı sabit bir Harness içinde ölçer; Bölüm 7 ise post-training'in böyle bir politikayı parametrelere nasıl yazabileceğini açıklar.
...
Bu ipliği izlemek—içeriden dışarıya, nihayetinde Agent'ın kendisine geri dönerek—kodun meta-yetenek olarak birleşik değerini görmeyi kolaylaştırır. Bölüm 8 bu temel üzerine kurularak hangi operasyonel kanıtın öz-değişikliği tetiklemesi gerektiğini ve aday değişikliklerin test, yayınlama ve geri alma yoluyla yeni bir sürüme nasıl girdiğini inceler.
...
Kitabın “Agent İnşa Etme” kısmını artık tamamladık; kod üretimi de bu kısmın en genel meta-yeteneğidir. Ancak kilit bir soru hâlâ yanıtsızdır: bu tasarım kararlarının etkisini bilimsel olarak nasıl ölçebiliriz? Bir sonraki bölümle birlikte kitap “Değerlendirme ve Evrim” kısmına geçer. Bölüm 6; değerlendirme ortamlarını, veri kümelerini, otomatik yargıyı ve model seçimini kapsayan bir metodoloji geliştirir. Bölüm 7 ve 8 ise sırasıyla parametre düzeyinde ve Agent sisteminin tamamında sürekli iyileştirmeyi ele alır.
