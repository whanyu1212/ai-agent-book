...
Такой подход решает проблему устранения неоднозначности, характерную для традиционных систем. В реальных сценариях у пользователя может быть несколько врачей (для себя, для родителей, для детей), и простое хранение «ключ-значение» не может их точно различить. Advanced JSON Cards через backstory предоставляет контекст получения информации («почему» эта информация сохранена), а через person и relationship выстраивает чёткую модель сущностей («для кого» сохранена). Когда пользователь говорит «организуй ежегодный осмотр для моей семьи», система может через relationship определить всех членов семьи, а через backstory узнать историю здоровья. Плата за это — более высокая стоимость генерации и поддержки.
...
Начнём с конкретного примера, чтобы выработать интуицию. Предположим, в базе знаний есть 100 технических статей, и пользователь ищет "дистилляция модели". Слово "модель" встречается в 60 статьях (слишком распространено, низкая различительная способность), а "дистилляция" — только в 3 статьях (очень редкое слово, высокая различительная способность). Хороший алгоритм поиска должен придавать слову "дистилляция" больший вес — статьи, содержащие "дистилляция", с большей вероятностью и есть то, что действительно ищет пользователь. Это и есть суть TF-IDF и BM25.

TF-IDF основан на простой интуиции: чем выше частота, с которой слово встречается в документе (TF, частота термина, Term Frequency), и чем ниже частота его встречаемости во всей коллекции документов (IDF, обратная частота документа, Inverse Document Frequency), тем важнее это слово. В примере выше "модель" встречается в 60% документов — значение IDF низкое; "дистилляция" встречается только в 3% документов — значение IDF высокое, поэтому вклад "дистилляции" в ранжирование намного больше, чем у "модели". Однако TF-IDF не учитывает длину документа (длинные документы естественным образом имеют более высокую частоту слов), а рост частоты термина считается линейным (действительно ли слово, встретившееся 10 раз, вдвое важнее слова, встретившегося 5 раз?). BM25 вводит два ключевых параметра для исправления этих проблем. `k1` управляет "насыщением" частоты термина: интуитивно понятно, что если статья упоминает "дистилляцию" 20 раз против 10 раз, её релевантность запросу о "дистилляции" на самом деле не отличается вдвое. `k1` заставляет вклад частоты термина постепенно выходить на плато по мере её роста, не позволяя длинным документам несправедливо выигрывать за счёт накопления повторов слов; `b` же управляет нормализацией по длине документа, позволяя алгоритму более справедливо обрабатывать документы разной длины. Это делает BM25 более устойчивой и эффективной функцией ранжирования, которая до сих пор остаётся незаменимым ключевым компонентом в крупнейших поисковых системах.
