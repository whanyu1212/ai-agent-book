...
- **Рубеж защиты устойчивой памяти между сессиями** — это расширение, которое данная глава особо подчёркивает сверх «смертельного трио»: содержимое, записываемое в долговременную память, должно проходить ту же проверку доверия, что и внешний контент, чтобы избежать ситуации, когда вредоносная инструкция затаивается в `MEMORY.md` и действует долгое время.
...
Разные модели сокращают этот рабочий процесс по-разному. Одни Coding-модели до первой правки широко изучают структуру репозитория, реализацию, места вызова и тесты. Другие просматривают лишь несколько наиболее вероятно связанных файлов, рано вносят патч и считают обратную связь компилятора и тестов частью исследования. Порог, определяющий, когда прекратить сбор информации и начать действовать, может сохраняться за моделью при смене Harness и меняться при замене одной только модели внутри того же Harness. Поэтому это прежде всего **усвоенное моделью поведение**, а не просто стиль интерфейса Coding-продукта. Промпты, инструменты и бюджет Harness могут усиливать или подавлять эту склонность, но не обязаны быть её источником. Эксперимент 6-7 измеряет различие в фиксированном Harness, а глава 7 объясняет, как постобучение может записать такую политику в параметры.
...
8. ★★ Паттерн Artifact позволяет сгенерированному агентом SQL или фронтенд-коду выполняться непосредственно в браузере пользователя или базе данных. Но сгенерированный SQL может выполнить разрушительную операцию, а сгенерированный HTML — содержать уязвимости. Как обеспечить безопасность системы?
