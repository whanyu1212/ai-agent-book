...
В классическом учебнике Хеннесси и Паттерсона *Computer Architecture: A Quantitative Approach* первая глава ставит вопрос «What Is Computer Architecture?» и определяет **архитектуру набора команд** (Instruction Set Architecture, ISA) как интерфейс между программным и аппаратным обеспечением[^ch1-agent-interface]. Этот взгляд полезен и для понимания агентов: **пространства наблюдений и действий вместе образуют интерфейс между LLM и внешней средой**. Пространство наблюдений преобразует информацию из среды в контекст, который модель способна обработать, а пространство действий превращает решения модели в операции над внешним миром. Информация, не попавшая в пространство наблюдений, для модели словно не существует. Если же операция отсутствует в пространстве действий, модель может лишь описать её словами, даже когда точно знает, что нужно сделать.
...
>
> Важно отметить, что суть этого эксперимента—управляемый моделью многоэтапный поиск, серверное исполнение кода в замкнутом цикле и уточнение намерения до начала работы—не привязана к одному поставщику. Читатели без кредитов OpenAI могут воспроизвести его у поставщика с равноценными управляемыми инструментами. Например, Responses API модели qwen3.7-plus от Alibaba Cloud Bailian также включает встроенные `web_search` и `code_interpreter`; сопроводительный код этой главы (`chapter1/search-codegen`) действительно выполнил с их помощью обе описанные выше задачи: расчёт расстояний между столицами АСЕАН и технический анализ биткоина. Управляемый поиск Formula и `code_runner` в Kimi K3 предоставляют возможности того же класса.
>
...
У ограждений есть и другой режим отказа — **ложный отказ**. Стремясь снизить вероятность пропуска опасного запроса, модель может одновременно отклонять законную, но внешне чувствительную работу, например санкционированное тестирование безопасности, оценку моделей или исследование дистилляции. Поэтому при оценке ограждений нужно проверять не только блокировку запрещённых запросов, но и возможность нормально выполнить явно разрешённые. Для законных, но чувствительных задач идеальная система должна предлагать объяснение, передачу человеку или авторизованный путь выполнения, а не ограничиваться общим отказом.
