...
>
> Fontos megjegyezni, hogy a kísérlet lényege—a modell által irányított többkörös keresés, a szerveroldali zárt hurkú kódfuttatás és a szándék végrehajtás előtti tisztázása—nem kötődik egyetlen szolgáltatóhoz sem. Az OpenAI-kredittel nem rendelkező olvasók egyenértékű felügyelt eszközöket kínáló szolgáltatóval is reprodukálhatják. Az Alibaba Cloud Bailian qwen3.7-plus Responses API-ja például szintén beépített `web_search` és `code_interpreter` eszközt kínál; a fejezet kísérőkódja (`chapter1/search-codegen`) ezzel ténylegesen lefuttatta a fenti két feladatot, az ASEAN-fővárosok távolságának kiszámítását és a Bitcoin technikai elemzését. A Kimi K3 Formula által felügyelt keresése és `code_runner` eszköze ugyanehhez a képességosztályhoz tartozik.
>
...
A LoopX ennek az elképzelésnek egy konkrét keretrendszere: a célokat, kapukat, teendőket, bizonyítékokat, kvótát, átadásokat, ellenőrzést és leállási feltételeket a modellen és a csevegésen kívüli tartós vezérlési síkra helyezi, így a különböző futtatókörnyezetek Ügynökei ugyanazon a helyreállítható és felülvizsgálható hurkon dolgozhatnak. A 10. fejezet ezen keresztül mutatja be, hogyan válik a Loop Engineering valós rendszerré.
...
A védőkorlátoknak van egy másik hibamódjuk is: a **téves elutasítás**. A veszélyes kérések átengedési esélyének csökkentése közben a modell jogszerű, de érzékenynek látszó munkákat is elutasíthat, például engedélyezett biztonsági tesztelést, modellértékelést vagy desztillációs kutatást. A védőkorlátok értékelésének ezért nemcsak azt kell vizsgálnia, hogy a tiltott kéréseket blokkolják-e, hanem azt is, hogy az egyértelműen engedélyezett kérések továbbra is teljesíthetők-e. Jogszerű, de érzékeny feladatoknál az ideális rendszer általános elutasítás helyett magyarázatot, emberhez történő eszkalációt vagy engedélyezett végrehajtási útvonalat kínál.
