...
**後訓練也會塑造模型何時行動。** 以 Coding 模型為例，GPT 系列與 Claude 系列經常呈現不同的預設行動閾值：前者可能先讀更多儲存庫資訊再修改，後者可能用較少檔案完成定位、先實作再藉測試回饋修正。這不是把模型擬人化成「謹慎」或「有直覺」，而是參數中的策略在估計：多讀一個檔案的預期價值，是否仍高於提交目前修補並驗證的預期價值。若 SFT 示範反覆包含廣泛調查後才編輯的軌跡，模型就會模仿較高的行動閾值；若 RL 的過程或結果獎勵持續認可快速定位、儘早進入可驗證迴圈，機率質量就會移向較早行動的軌跡。第六章實驗 6-7 在完全相同的中性 Coding Harness 中換模，確實測到這種差異隨模型變化，說明 Harness 無須強制流程，模型自身也會攜帶穩定的工具使用策略。Harness 可以調節它，但行為的主要來源可以位於後訓練後的模型參數中。由於廠商不公開完整資料與獎勵配方，這個實驗能證明的是模型端的行為差異，不能據此斷言某一種具體的私有演算法造成了它。
...
>
> 需要注意的是，這個現象不能簡單解釋為「Claude 不提供思維鏈」，也不能據此斷言「Kimi 的安全護欄更弱」。Claude API 是否返回 summarized thinking、Coding Agent 是否願意實現蒸餾程式，以及服務條款是否允許將模型輸出用於訓練，是三個不同的問題。本實驗沒有嘗試繞過任何模型的隱藏推理或安全機制，只使用產品公開提供的能力完成經授權的研究流程。
>
