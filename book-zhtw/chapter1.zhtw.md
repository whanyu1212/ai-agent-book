...
>
> 需要說明的是，這個實驗的本質——模型自主決定的多輪搜尋、服務端閉環的程式碼執行、執行前先澄清意圖——並不綁定某一家廠商。沒有 OpenAI 額度的讀者完全可以用具備等價託管工具的供應商重現：例如阿里雲百鍊 qwen3.7-plus 的 Responses API 同樣內建 `web_search` 與 `code_interpreter`，本章配套程式碼（`chapter1/search-codegen`）已用其實際跑通上述東協首都距離計算與比特幣技術分析兩個任務；Kimi K3 的 Formula 託管搜尋與 `code_runner` 也屬於同類能力。
>
...
LoopX 是這一理念的一個具體框架：它把目標、閘門、待辦、證據、配額、移交、驗證與終止條件放進模型和聊天之外的持久控制面，讓不同執行環境中的 Agent 都能在同一條可恢復、可審查的迴圈上工作。第十章將以它說明 Loop 工程如何落到真實系統中。
...
護欄也存在另一類失敗：**誤拒絕**。為了降低危險請求被放行的機率，模型可能同時拒絕一部分合法但形式敏感的任務，例如經過授權的安全測試、模型評估或蒸餾研究。因此，護欄評估不能只測試「應當拒絕的請求是否被攔截」，還要測試「明確允許的請求是否能夠正常完成」。對於合法但敏感的任務，理想的系統應提供解釋、人工升級或經過授權的執行路徑，而不是只返回無法繼續的籠統拒絕。
