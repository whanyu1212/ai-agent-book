...
圖中的「第 1 次」和「第 2 次呼叫」均指**呼叫模型 API**，而不是先後呼叫兩個工具。在這個例子中，`get_current_time` 的時區參數和 `get_weather` 的城市、單位參數都可以直接確定；天氣服務會自行傳回該城市的最新天氣，不依賴時間工具的輸出，因此 Agent 框架可以並行執行它們。如果後一個工具的參數必須來自前一個工具的結果，模型就需要在後續一輪中再發起工具呼叫，兩個工具只能依序執行。
