...
不同模型裁剪這套流程的方式並不相同。有些 Coding 模型會在第一次修改前廣泛閱讀目錄、實作、呼叫端與測試；另一些則只讀少數最可能相關的檔案，立即提交一個修補，再把編譯與測試回饋當作調查的一部分。這種「何時停止收集資訊、開始行動」的閾值，可以在更換 Harness 後繼續跟隨模型，也可以在同一 Harness 內隨模型切換而改變，因此首先是**模型學到的行為策略**，而不只是 Coding 產品的介面風格。Harness 的提示詞、工具與預算仍能放大或抑制它，但不必是它的來源。第六章實驗 6-7 將在固定 Harness 中測量這個差異，第七章再從後訓練角度解釋它可能如何寫入參數。
