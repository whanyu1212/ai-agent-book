...
更關鍵的是，Agent 擁有寫檔案的能力，這意味著它具備了修改自身外部製品的技術條件。當 Agent 首次執行某個任務並發現了之前不知道的關鍵資訊（例如給某銀行打電話時，發現對方要求提供開戶行地址才能驗證身份），它可以先把發現寫入記錄。記錄何時足以成為可靠知識、指令或程式，仍需結合更多軌跡與結果驗證；這是第八章將討論的持續進化問題。
