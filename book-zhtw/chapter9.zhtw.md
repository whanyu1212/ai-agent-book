...
>
> **實驗 9-2 ★：使用 WebRTC 建構「呼叫使用者」的語音 Agent**
>
> 「電話 Agent」不一定要接入 PSTN 或要求讀者準備一個真實號碼。許多提醒、資訊收集、確認和後續追蹤任務的被叫方就是使用者本人；此時瀏覽器 WebRTC 更容易重現：使用者開啟本機頁面並明確授權麥克風，Agent 與瀏覽器建立即時媒體工作階段，直接「打給」使用者。瀏覽器把麥克風 RTP 傳送給本機 peer，並接收 Agent 的下行語音 RTP；data channel 只承載音訊提交控制和無障礙字幕鏡像，不承載 canonical 使用者語意。整個過程不需要 E.164 號碼或電話供應商帳戶。PSTN/IVR 仍適合必須聯絡外部機構的生產任務，但不是理解「語音通話作為 Agent 工具」的必要前提。
>
> **實驗目標**：建構一個能主動發起瀏覽器語音工作階段、向使用者收集缺失資訊、複述確認並返回結構化結果的 Agent；同時保留「直接呼叫」和「ReAct 規劃」兩組對照。
>
> **技術方案**：本機 FastAPI 服務接收瀏覽器 SDP offer，由 `aiortc` 返回 answer、終止媒體並把實際收到的麥克風 RTP 解碼為 PCM；本機 Whisper 對這份 PCM 做 ASR，只有 ASR transcript 會進入對話模型。Agent 的澄清與最終確認都由真實 TTS 合成為 PCM，再排入伺服器端 WebRTC 下行音軌，而不是用連線提示音、瀏覽器 `speechSynthesis` 或 data-channel 文字冒充語音。直接組要求呼叫者預先填寫姓名、目標、上下文和指令。ReAct 組只接收自然語言任務，由真實外部 LLM 留下去識別化 raw request/response、response ID、精確模型、usage、finish status、latency 及 hash，並透過可稽核的 observation / reason / action 摘要決定：(1) 通話目標；(2) 已知上下文；(3) 缺少哪些資訊；(4) 通話時要詢問和確認什麼。兩組都使用同一 `complete_task` 工具儲存使用者明確確認的欄位；模型、ASR 或 TTS 失敗時驗收直接失敗，不走本機 planner/parser fallback。
>
> Agent 的工作流程：任務為「打給我，確認明天的牙科檢查」 → ReAct 規劃發現缺少具體時間和確認號 → 使用者在瀏覽器接聽 → Agent 語音詢問 → 使用者回答 → Agent 複述並請求最終確認 → 呼叫 `complete_task` → 介面儲存 transcript、關鍵欄位和傳輸統計 → Agent 向使用者讀回結果。這個本機實驗只記錄使用者確認的內容，不會虛構診所已經完成了真實預約。
>
> **驗收標準**：兩組各建立一通真實 WebRTC 工作階段，並同時留下 SDP offer/answer、ICE 已連線、data channel 已開啟、瀏覽器麥克風軌和伺服器端下行音軌存在、雙向 audio RTP packet/byte 大於零的證據；伺服器還必須儲存並 hash 由麥克風 RTP 得到的 ASR 輸入、Whisper checkpoint 溯源、真實模型 receipt，以及至少兩個已完整傳送的 TTS asset。canonical 使用者 transcript source 只能是 ASR，Agent transcript source 只能是 TTS；data channel 只能作控制/字幕。通話結束後應證明缺失欄位澄清、明確確認與 `complete_task` 結構欄位。不能用 mock、fallback、連線提示音、文字輸入或只跑預檢替代。對照還必須說明：直接組需要四個完整參數，而 ReAct 組只輸入一段資訊不完整的任務，就能由真實 LLM 識別缺失欄位並在通話中補齊。
>
> 這個實驗展示了語音 Agent 的一個重要應用方向：**Agent 不只是等待使用者開啟聊天框，也能主動建立一個有明確開始、確認和結束狀態的即時工作階段**。瀏覽器 WebRTC 覆蓋了最容易重現的「呼叫使用者」路徑；需要代替使用者聯絡外部世界、等待人工客服或導航 IVR 時，再把同一個通話工具契約換成合規的 PSTN/SIP 供應商即可。
...
**MiniCPM-o 4.5** 則把這條路線壓縮到可在單張消費級或工作站 GPU 上本機執行的規模。它以 SigLip2、Whisper-medium、CosyVoice2 和 Qwen3-8B 為基礎，總計約 9B 參數，原生接收文字、影像、影片與音訊，也能直接生成文字和語音。這裡值得實驗的不是再複製一張排行榜，而是前文「端到端 vs. 自級聯」的判斷：同一模型面對同一段聲音，直接從音訊隱空間作答，與先壓成純文字再作答，失敗方式是否真的會隨任務性質改變？

> **實驗 9-4 ★★：本機執行 MiniCPM-o 4.5，比較端到端與自級聯**
>
> 本實驗固定開放權重 `openbmb/MiniCPM-o-4_5` 的 revision `1f761131…`，在單張 96GB RTX PRO 6000 Blackwell 上以 BF16 本機執行；實測峰值配置顯存 20.27GiB，模型載入 6.15 秒，全程沒有外部 API。為避免混淆兩個問題，實驗明確關閉 thinking mode：它測量 Omni 模型是否保留音訊資訊，**不是**後文的「邊想邊說」。
>
> 四條小型合成 WAV 分成兩類：兩道答案只取決於文字內容的口述算術題，以及文字完全相同但一快一慢的兩條語速題。**端到端組**把 WAV 直接交給 MiniCPM-o 作答；**自級聯組**先讓同一個 MiniCPM-o 只轉錄文字、明確丟棄語氣和語速，再讓它只依據轉錄作答。兩組均關閉取樣。
>
> 表9-1 MiniCPM-o 4.5 本機端到端與自級聯結果（4 條機制檢查，不是 benchmark）
>
> | 任務類型 | 端到端 | 自級聯 | 觀察 |
> | --- | ---: | ---: | --- |
> | 語義算術（2 條） | 1/2 | 2/2 | 端到端把 “twelve boxes” 聽成 8；明確轉錄保留了正確的 12 |
> | 副語言語速（2 條） | 2/2 | 1/2 | 兩條轉錄都變成同一句話；自級聯因此把 fast 樣本也猜成 slow |
> | 合計 | 3/4 | 3/4 | 總分相同，但失敗位置相反 |
>
> 這次小實驗重現了前文的質性預測：文字足以承載資訊時，中間轉錄可以修正感知錯誤；答案依賴語速時，純文字瓶頸會不可逆地抹掉證據。它也提醒我們不要把「端到端」直接等同於「更準確」——兩組總分都是 75%。載入完成後的平均整次呼叫為端到端 0.69 秒、自級聯 0.55 秒，但執行順序固定、輸出長度不同、樣本只有四條，不能把這組數字解讀成嚴謹的延遲排名。
>
> 最後另跑一條原生 audio-to-audio 路徑，保存了 11.56 秒、24kHz 單聲道 WAV，證明本機路徑確實完成「聽→答→說」；不過答案也沿用了把 12 聽成 8 的錯誤。完整原始回覆、模型轉錄、逐階段延遲、輸入/輸出 hash 與驗收檔案見 [`chapter9/end-to-end-speech`](../chapter9/end-to-end-speech/)。
...
> **實驗 9-6 ★：執行 Computer Use（Anthropic 參考路徑或開放模型路徑）**
>
