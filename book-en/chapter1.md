...
>
> It is important to note that the essence of this experiment—model-directed multi-round search, server-side closed-loop code execution, and intent clarification before execution—is not tied to any one vendor. Readers without OpenAI credits can reproduce it with providers that offer equivalent managed tools. For example, Alibaba Cloud Bailian's qwen3.7-plus Responses API also includes built-in `web_search` and `code_interpreter`; this chapter's companion code (`chapter1/search-codegen`) has run both of the tasks above—the ASEAN-capital distance calculation and Bitcoin technical analysis—with that API. Kimi K3's Formula-managed search and `code_runner` provide the same class of capability.
>
...
LoopX is a concrete framework for this idea: it moves objectives, gates, todos, evidence, quota, handoffs, verification, and stop conditions into a durable control plane outside the model and chat, so Agents in different runtimes can work on the same recoverable, reviewable loop. Chapter 10 uses it to show how Loop Engineering becomes a real system.
...
Guardrails also have another failure mode: **false refusal**. To reduce the chance of allowing dangerous requests, a model may also reject legitimate but sensitive-looking work, such as authorized security testing, model evaluation, or distillation research. Guardrail evaluation should therefore test not only whether prohibited requests are blocked, but also whether clearly permitted requests can still be completed. For legitimate but sensitive tasks, an ideal system should offer an explanation, human escalation, or an authorized execution path instead of only a generic refusal.
