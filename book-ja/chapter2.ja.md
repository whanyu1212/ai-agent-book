...
図中の「1 回目」と「2 回目」は、どちらも**モデル API の呼び出し**を指しており、2 つのツールを順番に呼び出すという意味ではありません。この例では、`get_current_time` のタイムゾーンパラメータと `get_weather` の都市および単位のパラメータをあらかじめ決定できます。天気サービス自体がその都市の最新の天気を返し、時刻ツールの出力には依存しないため、Agent フレームワークは両者を並列に実行できます。後続ツールのパラメータを先行ツールの結果から得る必要がある場合、モデルは次のラウンドでそのツール呼び出しを要求する必要があり、2 つのツールは直列に実行することになります。
...
- **メタデータのリスト**——インストール済みのすべての Skill の `name` + `description`（通常はわずか数百 token）——をあらかじめモデルに提供し、現在のタスクにどの Skill が関係するかを判断できるようにします。注意すべきなのは、**このメタデータをどのメッセージロールでコンテキストに注入するかは Claude Code Agent Harness の実装詳細であり、Agent Skills の仕組み自体における固定要件ではない**という点です。Claude Code の一部の過去バージョンでは、この種の動的コンテキストは `<system-reminder>` で包まれた user-role のコンテンツブロックとして現れていました。mid-conversation system message をサポートする新しい実装経路では、追加された system-role の context block を使用することもできます。どの表現を採用しても、安定したコンテキストのプレフィックスを繰り返し書き換えることなく、現在利用可能な Skills をモデルに認識させるという目的は共通しています。

- **完全な内容**——モデルがメタデータに基づいてある Skill が現在のタスクに適していると判断すると、Skill ツールを通じて対応する `SKILL.md` をオンデマンドで読み込み、その内容を現在の実行コンテキストに取り込みます。これにより、会話の開始時にすべての Skill の完全な説明を一括して読み込まずに済み、無関係なコンテキストの占有を減らせます。
...
注意すべきは、**会話の途中で動的にシステムコンテキストをモデルへ補足する仕組みは Skills に固有のものではない**ということです。利用可能な Skill のメタデータに加えて、Agent は現在のタスク状態、実行環境、その他の動的な情報を継続的にモデルへ提供する必要がある場合があります。次節の **Agent ステータスバー（Agent Status Bar）** ではこの仕組みをさらに展開し、Skill のメタデータのリストはその具体例の一つと見なせます。
