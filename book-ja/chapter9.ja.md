...
>
> **実験 9-2 ★：WebRTC で「ユーザーに電話する」音声 Agent を構築する**
>
> 「電話 Agent」は必ずしも PSTN に接続したり、読者に実在の電話番号を用意してもらったりする必要はありません。リマインダー、情報収集、確認、フォローアップといったタスクでは、着信側がユーザー本人であることも少なくありません。その場合はブラウザ WebRTC のほうが再現しやすくなります。ユーザーがローカルページを開いてマイクの利用を明示的に許可すると、Agent はブラウザとのリアルタイムメディアセッションを確立し、ユーザーへ直接「電話」します。ブラウザはマイクの RTP をローカル peer に送り、Agent からの下り音声 RTP を受信します。data channel が運ぶのは音声送信の制御とアクセシビリティ用字幕のミラーだけであり、canonical なユーザーの意味内容ではありません。この一連の処理に E.164 番号や電話事業者のアカウントは不要です。PSTN/IVR は外部組織への連絡が必須となる本番タスクには引き続き適していますが、「Agent のツールとしての音声通話」を理解するための前提条件ではありません。
>
> **実験の目標**：ブラウザ音声セッションを能動的に開始し、ユーザーから不足情報を収集して復唱確認し、構造化された結果を返せる Agent を構築します。比較のため、「直接呼び出し」グループと「ReAct プランニング」グループの両方を用意します。
>
> **技術方針**：ローカルの FastAPI サービスがブラウザから SDP offer を受け取り、`aiortc` が answer を返してメディアを終端し、実際に受信したマイク RTP を PCM にデコードします。ローカルの Whisper がこの PCM に対して ASR を行い、対話モデルに入るのは ASR transcript だけです。Agent の確認質問と最終確認はいずれも実際の TTS で PCM に合成し、サーバー側 WebRTC の下り音声トラックにキューイングします。接続音、ブラウザの `speechSynthesis`、data-channel のテキストで音声を装ってはいけません。直接グループでは、呼び出し側が名前、目標、コンテキスト、指示を事前にすべて指定します。ReAct グループが受け取るのは自然言語のタスクだけです。実在する外部 LLM が秘匿化済み raw request/response、response ID、正確なモデル、usage、finish status、latency、hash を残し、監査可能な observation / reason / action の要約を通じて、(1) 通話の目的、(2) 既知のコンテキスト、(3) 不足している情報、(4) 通話中に質問・確認すべき内容を決定します。両グループとも同じ `complete_task` ツールを使い、ユーザーが明示的に確認したフィールドを保存します。モデル、ASR、TTS のいずれかが失敗した場合はそのまま不合格とし、ローカルの planner/parser fallback は使いません。
>
> Agent のワークフロー：タスクは「電話して、明日の歯科検診を確認して」→ ReAct のプランニングが正確な時刻と確認番号の不足を検出する → ユーザーがブラウザで応答する → Agent が音声で質問する → ユーザーが答える → Agent が内容を復唱して最終確認を求める → `complete_task` を呼び出す → インターフェースが transcript、主要フィールド、転送統計を保存する → Agent が結果をユーザーに読み上げる。このローカル実験が記録するのはユーザーが確認した内容だけであり、クリニックで実際の予約が完了したかのように装うことはありません。
>
> **合格基準**：両グループがそれぞれ実際の WebRTC セッションを 1 回確立し、SDP offer/answer、ICE の接続完了、開いた data channel、ブラウザのマイクトラックとサーバーの下りトラックの存在、双方向 audio RTP の packet/byte がゼロより大きいことを示す証拠をすべて残すこと。さらにサーバーは、マイク RTP から得た ASR 入力とその hash、Whisper checkpoint の来歴、実在モデルの receipt、完全に送信された TTS asset を 2 つ以上保存しなければなりません。canonical なユーザー transcript source は ASR、Agent の transcript source は TTS のみに限定し、data channel は制御と字幕にしか使えません。通話後の証拠では、不足フィールドの確認、明示的な最終確認、`complete_task` の構造化フィールドを示す必要があります。mock、fallback、接続音、テキスト入力、preflight だけの実行で代用することはできません。また比較では、直接グループには完全な 4 パラメータが必要である一方、ReAct グループは不完全なタスク記述 1 件だけから開始し、実在する LLM が不足フィールドを特定して通話中に補完できることも示します。
>
> この実験は、音声 Agent の重要な応用方向の一つを示しています。**Agent はユーザーがチャット画面を開くのをただ待つだけでなく、開始・確認・完了の状態が明確なリアルタイムセッションを能動的に確立できます**。ブラウザ WebRTC は、最も再現しやすい「ユーザーに電話する」経路をカバーします。ユーザーに代わって外部世界と連絡する、人間の担当者を待つ、IVR を操作するといったタスクでは、同じ通話ツールの契約を準拠した PSTN/SIP プロバイダーに接続すればよいのです。
...
通常のストリーミング化でも、**VAD の無音待機とターン判断そのもの**は解消できません。この待機中にもストリーミング ASR はすでに動いており、ターン判断が止めているのは、最終書き起こしの確定と返答の再生です。先行生成はその計算の一部を隠せますが、いつ安全に話し始められるかは依然として判断しなければなりません。この遅延をさらに短縮するには、最前端の知覚と終端点判断そのものを改善する必要があります。
...
**MiniCPM-o 4.5** は、この路線を民生用またはワークステーション用 GPU 1 枚でローカル実行できる規模に圧縮したモデルである。SigLip2、Whisper-medium、CosyVoice2、Qwen3-8B を基盤とする約 9B パラメータのモデルで、テキスト・画像・動画・音声をネイティブに受け取り、テキストと音声を直接生成する。ここで有用なのは別のランキングを転載することではなく、前述のエンドツーエンド対セルフカスケードという主張を試すことだ。同じモデルでも、音声の潜在表現から直接答える場合と、まず音声をプレーンテキストへ平坦化する場合とで、失敗の仕方は変わるだろうか。

> **実験 9-4 ★★：MiniCPM-o 4.5 をローカル実行する — エンドツーエンド対セルフカスケード**
>
> オープンな `openbmb/MiniCPM-o-4_5` checkpoint を revision `1f761131…` に固定し、96GB RTX PRO 6000 Blackwell 1 枚で BF16 によりローカル実行した。ピーク割当 VRAM は 20.27GiB、モデル読み込みは 6.15 秒で、外部 API は呼び出していない。thinking mode は意図的に無効化した。この実験が測るのは Omni モデルの情報保持であり、後節の「話しながら考える」能力ではない。
>
> 4 本の小さな合成 WAV は、言葉だけで答えが決まる音声算術問題 2 問と、文言は同一だが発話速度が速い／遅い 2 本からなる。**エンドツーエンド側**は WAV を MiniCPM-o に直接渡す。**セルフカスケード側**は、同じモデルに口調と速度を意図的に除いた単語のみの文字起こしを作らせ、その文字だけから回答させる。どちらも sampling は無効である。
>
> 表 9-1 MiniCPM-o 4.5 のローカル結果（4 件の機構確認であり benchmark ではない）
>
> | タスク種別 | エンドツーエンド | セルフカスケード | 観察 |
> | --- | ---: | ---: | --- |
> | 意味的な算術（2） | 1/2 | 2/2 | 直接側は “twelve boxes” を 8 と聞き取ったが、明示的な文字起こしは正しい 12 を保持した |
> | 副言語的な速度（2） | 2/2 | 1/2 | 2 本の文字起こしは同じ文になり、セルフカスケードは速い音声にも “slow” と答えた |
> | 合計 | 3/4 | 3/4 | 合計は同じだが、失敗箇所は逆 |
>
> この小規模実行は質的な予測を再現した。テキストが必要情報をすべて運べる場合、明示的な文字起こしは知覚誤りを直せる。一方、答えが発話速度に依存する場合、プレーンテキストのボトルネックは証拠を不可逆に消す。両者とも 75% であり、エンドツーエンドが自動的に高精度という意味ではない。読み込み後の平均呼び出し時間は 0.69 秒対 0.55 秒だったが、固定された実行順、異なる出力長、4 件だけという条件から、厳密なレイテンシ順位にはできない。
>
> ネイティブな audio-to-audio 呼び出しでは、11.56 秒、24kHz、mono の実 WAV も保存したが、12→8 の知覚誤りを引き継いだ。生の応答、文字起こし、段階別時間、入出力 hash、受入検査は [`chapter9/end-to-end-speech`](../chapter9/end-to-end-speech/) にある。
...
> **実験 9-6 ★：Computer Use を実行する（Anthropic 参照経路またはオープンモデル経路）**
>
