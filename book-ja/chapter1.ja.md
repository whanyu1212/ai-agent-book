...
Hennessy と Patterson の古典的教科書『Computer Architecture: A Quantitative Approach』は、第 1 章で「What Is Computer Architecture?」と問いかけ、**命令セットアーキテクチャ**（Instruction Set Architecture, ISA）をソフトウェアとハードウェアの間のインターフェースとして捉えています[^ch1-agent-interface]。この視点は Agent を理解するうえでも示唆的です。**観測空間と行動空間は、LLM と外部環境の間のインターフェースを共同で形づくります**。観測空間は環境内の情報をモデルが処理できるコンテキストへ変換し、行動空間はモデルの意思決定を外部世界への操作へ変換します。観測空間に入らない情報は、モデルにとって存在しないも同然です。行動空間にない操作は、モデルが何をすべきか正確に理解していても、言葉で提案することしかできません。
...
空間の拡張は、あらゆる情報とツールを一度にモデルへ詰め込むことではありません。無関係なコンテキストはノイズを生み、ツールが多すぎると選択コストとセキュリティリスクが増します。有効な拡張は、**オンデマンドで、関連性が高く、制御可能**でなければなりません。検索によって適切な情報だけをコンテキストへ入れ、ツール発見によって現在必要な行動だけを公開し、権限制御と結果検証でそれらの行動を制約します。以降の章で、こうした工学的手法をそれぞれ詳しく説明します。

[^ch1-agent-interface]: John L. Hennessy and David A. Patterson, *Computer Architecture: A Quantitative Approach*, 6th ed., Morgan Kaufmann, 2019, Chapter 1, “What Is Computer Architecture?”。同書は命令セットアーキテクチャ、コンピュータ構成、ハードウェア実装を区別し、そのうち命令セットアーキテクチャをソフトウェアとハードウェアのインターフェースと位置づけています。https://shop.elsevier.com/books/computer-architecture/hennessy/978-0-12-811905-1 を参照。
...
>
> なお、この実験の本質である、モデルが自律的に判断する複数ラウンドの検索、サーバー側の閉ループでのコード実行、実行前の意図の明確化は、特定のベンダーに依存しません。OpenAI のクレジットがない読者も、同等のマネージドツールを提供する事業者で再現できます。たとえば Alibaba Cloud Bailian の qwen3.7-plus Responses API にも `web_search` と `code_interpreter` が組み込まれており、本章の付属コード（`chapter1/search-codegen`）では、前述の ASEAN 首都間距離の計算とビットコインのテクニカル分析という 2 つのタスクを実際に実行済みです。Kimi K3 の Formula によるマネージド検索と `code_runner` も同種の能力を提供します。
>
...
LoopX は、この考え方を具体化したフレームワークです。目標、ゲート、Todo、証拠、クォータ、引き継ぎ、検証、終了条件をモデルやチャットの外にある永続的なコントロールプレーンへ移し、異なるランタイムの Agent が同じ回復可能でレビュー可能なループ上で働けるようにします。第 10 章では、Loop Engineering が実際のシステムになる過程を LoopX で示します。

2026 年 7 月、業界では **Graph エンジニアリング**（Graph Engineering）という言葉が、より上位のオーケストレーション視点を表すものとして使われ始めました。Agent のループ、決定論的なプログラム、人間による承認を明示的な実行グラフとして構成し、ノードが個別の能力を担い、エッジがルーティングと依存関係を規定し、構造化された状態がエッジに沿って受け渡され、重要な境界で永続化されます[^ch1-graph-engineering-ja]。ただし、Graph エンジニアリングは Loop エンジニアリングを置き換えるものではなく、上述の進化の「第 6 層」と単純にみなすべきでもありません。ループ自体が戻りエッジを持つグラフであり、グラフ内の各ノードも内部で ReAct や別の Agent ループを実行できます。この名称はまだ定着していないため、本書では既存のオーケストレーションと Harness の実践に対する新しい呼び名として扱い、マルチ Agent に関わる部分は第 10 章で展開します。ここでいう「グラフ」は制御フローまたは実行のグラフであり、GraphRAG が利用する知識グラフではありません。
...
ガードレールには、もう一つの失敗モードである**誤拒否**もあります。危険なリクエストを許可する確率を下げようとする結果、認可されたセキュリティテスト、モデル評価、蒸留研究など、正当でありながら外見上は機微に見える作業までモデルが拒否することがあります。したがって、ガードレールの評価では「拒否すべきリクエストを遮断できるか」だけでなく、「明確に許可されたリクエストを正常に完了できるか」も検証しなければなりません。正当だが機微なタスクに対して、理想的なシステムは一般的な拒否だけを返すのではなく、理由の説明、人間へのエスカレーション、または認可済みの実行経路を提供すべきです。
