...
付属実験では、同じ 60 問を 3 つのメモリ構成に解かせ、実 API の実行軌跡を 180 本保存しました。表 6-4 では、割合だけで標本数が見えなくならないよう、全体成功率に正解数も併記しています。

表 6-4 3 つのユーザーメモリ構成の難易度別成功率
...
手書きの Rubric は、このような診断軸を素早く作るのに向いています。規模が大きくなれば、専用の **生成式報酬モデル** で評価を自動化する方法もあります。訓練方法は第 7 章で扱います。
...
> **実験 6-7 ★★：固定 Coding Harness でモデルの行動閾値を測定する**
>
...
実際の費用の内訳を見るため、付属実験では 8 ターンの返金業務を固定しました。注文、配送、返金規約、ナレッジベースを確認し、リスク判定、返金、通知、クローズまでを実行します。gpt-4o-mini の実 API 呼び出しで「安定したプレフィックス」と「履歴圧縮」を個別にオン・オフし、同じ業務を 4 構成で比較しました。表 6-5 の金額は、各実行に保存された token 使用量と当時の価格から計算しています。

表 6-5 8 ターンの Agent タスクにおける実コスト
...
> **実験 6-8 ★：Agent タスクのエンドツーエンドコスト分析**
>
...
>
> **実験 6-10 ★★：ユーザーメモリシステムのエンドツーエンド選定評価**
>
...
3 ラウンドの実測値を表 6-6 にまとめます。各群 4 タスクしかないため、ここで判断できるのは試験を拡大する価値があるかどうかまでです。AndroidWorld 全体の成功率は推定できません。

表 6-6 AndroidWorld の Wi-Fi サブセットに対する 3 ラウンド
...
> **実験 6-12 ★★：OpenVLA と RoboTwin2 の身体化知能環境を構成する**
>
