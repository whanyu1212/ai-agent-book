...
**TTS** 把回复文字转成语音，短句合成通常需要 200-500ms。图9-2用一个简短、不开启 reasoning 的回复说明串行延迟如何累积：VAD（500-800ms）+ ASR（50-200ms）+ LLM TTFT（100-500ms）+ LLM 生成短句（100-300ms）+ TTS（200-500ms），合计约 0.95-2.3 秒。这只是统一口径下的示意区间；真实数值还取决于输入与回复长度、模型、硬件、网络和负载。
...
图9-2算的是“每一环都跑完再交棒”的**完全串行**情形。生产系统通常保留 VAD-ASR-LLM-TTS 的模块化分工，但让各阶段尽早产出增量结果，以缩短用户听到第一个音节的时间：
...
- **TTS 增量合成音频**：TTS 收到第一段文本便开始合成，并在整段波形尚未生成完时持续返回音频块。此后 LLM 生成剩余文本、TTS 合成后续语音和客户端播放已有音频可以重叠进行。

但“每一级都流式”不等于三个模型会针对同一轮对话同时开工。在不做推测的标准级联中，依赖关系仍然存在：用户说话时 ASR 可以持续工作；轮次结束、转录稳定后，LLM 才生成最终回复；LLM 产出第一段可播报文本后，TTS 才能开始。真正发生重叠的是“用户说话与 ASR 转录”，以及“LLM 生成剩余回复与 TTS 合成/播放”，而不是 ASR、LLM、TTS 三段计算从头到尾全部并行。

更激进的系统会做**抢跑生成（preemptive/speculative generation）**：根据较稳定的部分转录提前启动 LLM；如果后续转录改变，就取消、重启或修正这次生成。还可以提前合成语音，但通常要等轮次确认后才播放，以免 Agent 在用户尚未说完时抢话。LiveKit Agents、Pipecat 这类框架可以承载普通流式级联与这类抢跑策略；是否让 ASR 与 LLM 真正重叠，取决于系统是否显式实现了部分转录提交、失效与回滚机制，而不是只要打开 `stream` 开关就会自动获得。
...
**MiniCPM-o 4.5** 则把这条路线压缩到可在单张消费级或工作站 GPU 上本地运行的规模。它以 SigLip2、Whisper-medium、CosyVoice2 和 Qwen3-8B 为基础，总计约 9B 参数，原生接收文本、图像、视频与音频，也能直接生成文本和语音；除轮次式语音对话外，还提供全双工流式模式。这里最值得实验的不是再复制一张模型榜单，而是前文“端到端 vs. 自级联”的判断：同一个模型面对同一段声音，直接从音频隐空间作答，与先压成纯文字再作答，失败方式是否真的随任务类型而改变？

> **实验 9-4 ★★：本地运行 MiniCPM-o 4.5，对比端到端与自级联**
>
> 本实验固定开放权重 `openbmb/MiniCPM-o-4_5` 的 revision `1f761131…`，在单张 96GB RTX PRO 6000 Blackwell 上以 BF16 本地运行；实测峰值显存 20.27GiB，模型加载 6.15 秒，全程没有外部 API。为避免把两个问题混在一起，实验明确关闭 thinking mode：它测的是 Omni 模型是否保留音频信息，**不是**后文的“边想边说”。
>
> 四条小型合成 WAV 分成两类：两道答案只取决于文字内容的口述算术题，以及文字完全相同但一快一慢的两条语速题。**端到端臂**把 WAV 直接交给 MiniCPM-o 作答；**自级联臂**先让同一个 MiniCPM-o 只转录文字、明确丢弃语气和语速，再让它只依据转录作答。两臂均关闭采样。
>
> 表9-1 MiniCPM-o 4.5 本地端到端与自级联结果（4 条机制检查，不是 benchmark）
>
> | 任务类型 | 端到端 | 自级联 | 观察 |
> | --- | ---: | ---: | --- |
> | 语义算术（2 条） | 1/2 | 2/2 | 端到端把 “twelve boxes” 听成 8；自级联的显式转录保留了正确的 12 |
> | 副语言语速（2 条） | 2/2 | 1/2 | 两条转录都变成同一句话；自级联因此把 fast 样本也猜成 slow |
> | 合计 | 3/4 | 3/4 | 总分相同，但失败位置相反 |
>
> 这次小实验恰好复现了前文的预测：文字足够承载信息时，中间转录可以纠正感知错误；答案依赖语速时，纯文字瓶颈会不可逆地抹掉证据。它同时提醒我们不要把“端到端”直接等同于“更准确”——两臂总分都是 75%。加载完成后的平均整次调用为端到端 0.69 秒、自级联 0.55 秒，但运行顺序固定、输出长度不同、样本只有四条，不能把这组数字解释成严谨的延迟排名。
>
> 最后另跑一条原生 audio-to-audio 路径，保存了 11.56 秒、24kHz 单声道 WAV，证明本地路径确实完成“听→答→说”；不过其答案沿用了端到端臂把 12 听成 8 的错误。完整原始回复、模型自产转录、逐阶段延迟、输入/输出 hash 与验收文件见 [`chapter9/end-to-end-speech`](../chapter9/end-to-end-speech/)。
...
> **实验 9-6 ★：运行 Computer Use（Anthropic 参考路径或开放模型路径）**
>
