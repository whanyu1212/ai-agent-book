...
更关键的是，Agent 拥有写文件的能力，这意味着它具备了修改自身外部制品的技术条件。当 Agent 首次执行某个任务并发现了之前不知道的关键信息（例如给某银行打电话时，发现对方要求提供开户行地址才能验证身份），它可以先把发现写入记录。记录何时足以成为可靠知识、指令或程序，仍需结合更多轨迹与结果验证；这是第八章将讨论的持续进化问题。
...
这里要先和第八章划清分工。本节讨论 Coding Agent 如何用代码**修复和创建与自己同类的 Agent**——自我修复、自我复制、按需生成新 Agent，重点是代码生成与系统构造能力，因此称为**自举**（bootstrapping）。第八章的重点不是再次讲怎样写出这些代码，而是怎样由已评价的生产经验触发自我修改：选择知识、指令、程序或参数作为更新对象，从稳定版本生成候选版本，并通过回归、灰度发布和回滚控制风险。两章在“修改代码”处相交，但回答的问题不同。
