...
配套实验用同一组 60 个问题测试了三套记忆方案，共保留 180 条真实 API 运行轨迹。结果见表6-4；总体成功率后面同时列出成功题数，以免百分比掩盖样本量。
...
另一个容易被忽视的数字是：180 次评判中，幻觉否决触发了 28 次。可见否决项并不是写在 Rubric 里“以防万一”的装饰，它确实会改变最终结果。工程上不应先假定“结构化 + RAG”必有协同，而应先看不同难度下各自怎么失败，再决定哪些问题常驻结构化记忆，哪些问题触发检索。这次实验使用的是一组合成用例，也只跑了一套模型和评判配置；它说明的是这几种方案为何会成、为何会败，并不能据此排出一张放之四海而皆准的记忆系统榜单。

当然，以上结论成立还有一个前提：评判模型本身足够可靠。如果 Agent 和评判者来自同一个模型家族，两者可能恰好共享同一种偏好和盲点。下面就来讨论这个问题。
...
人工编写 Rubric 适合快速建立这样的诊断维度。评估规模再扩大时，还可以训练专门的**生成式奖励模型**来自动打分——相关训练方法将在第七章讨论。
...
为了看清这些费用究竟从哪里来，配套实验选了一个固定的八轮客服退款任务：查询订单、物流、退款政策和知识库，随后完成风控、退款、通知与关单。实验用真实的 gpt-4o-mini 调用，分别打开或关闭“稳定前缀”和“压缩历史”两个开关，得到四组对照。四组完成的业务完全相同，表6-5中的费用根据当次运行保存的 token 用量和当时价格计算。

表6-5 八轮 Agent 任务的真实成本对照
...
不过，“仅稳定前缀”已经节省 28.3%，“仅压缩历史”又节省 17.5%，两者一起却只节省 30%，并不是二者相加。原因是压缩历史的同时，也缩短了可以命中缓存的前缀。因此，**几项上下文优化同时使用时，必须在完整任务中一起实测，不能把各自的节省比例直接相加。**换一个模型、价格或任务长度，30% 这个数字都可能变化；真正值得复用的是这种四组对照的测法。
...
>
> **验收标准**：生成成本拆解报告，识别主要成本驱动因素。四种开关组合都要运行，既比较每项优化单独带来的变化，也检查两项优化合用后的结果。更换模型后必须重新实测，不能沿用配套轨迹中的节省比例。
>
>
...
Agent 评估还有一层额外的非确定性：同一模型、同一数据集，两次运行的结果也会漂移——温度采样、工具返回的波动、环境时序都会引入随机性。因此单次运行的数字不应作为部署依据，而应**多次运行取均值**（如每个配置跑 3-5 次），同时报告均值与波动范围。后文 AndroidWorld 的小实验每项任务只做了一次配对运行，所以它只能用来筛选值得扩大验证的方向；真正决定是否部署，还得按计划在完整任务集上跑多个随机种子。
...
下面来看配套仓库里一次真实的 AndroidWorld 调优过程。实验只跑了 API 35 模拟器上的 4 个 Wi-Fi 设置任务，每项任务做一次配对对照；它不是完整的 116 项基准测试，也不能代替 API 33 标准环境下的复测。这个案例的价值不在于证明系统整体提高了多少，而在于展示如何根据一轮结果，决定下一轮只改什么。
...
三轮实测结果汇总在表6-6。每组都只有四项任务，所以这里的数字只能用来决定下一步是否值得扩大测试，不能用来推断整个 AndroidWorld 的成功率。

表6-6 AndroidWorld Wi-Fi 子集的三轮实验
...
本章围绕一个核心问题：怎么判断 Agent 真的变好了？从搭建可复现的测试环境，到设计经得起泄漏考验的数据集，到让 LLM 担任评判者，最后到用评估结果驱动模型选型和迭代——这条链路的每个环节都会影响结论的可信度。本章的几组实测也给出了更具体的提醒：把结构化记忆和 RAG 拼在一起，不一定就有协同；缓存和压缩各自省下的费用，不能直接相加；参考音频选得不同，多模态评分的含义也会改变；而 Agent 能不能看懂界面、要为此花多少 token，往往取决于 Harness 怎样整理输入。模型选型还应比较不同资源预算下的能力增长曲线，而不能只看单点成绩。生产级 Agent 的评估不是偶尔做一次的考试，而是嵌入到每个产品决策中的持续验证。
...
从第一章引入的 Harness 工程视角看，本章的评估方法论是 Harness 中“验证”功能的系统化实现，而“从 Benchmark 报告到系统改进”的闭环则是 Harness 迭代优化的核心机制。本章回答“如何可靠地测量”；第八章将在此基础上回答“如何把多维轨迹评价转化为可执行、可回滚的系统更新”。
