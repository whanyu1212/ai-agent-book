...
1장에서는 컨텍스트를 에이전트가 의사결정을 내리는 순간 활용할 수 있는 정보의 작업 집합으로 정의했습니다. 이 컨텍스트를 설계하고 관리하는 **컨텍스트 엔지니어링(Context Engineering)**은 효과적인 에이전트를 구축하는 핵심입니다. 실무에서 컨텍스트는 특정 상호작용을 위해 모델이 받는 모든 것을 포함합니다. 대화 기록, 시스템 지시, 도구 정의, 검색한 문서, 런타임 상태, 그 밖의 작업별 정보가 모두 해당합니다. 1장에서 소개한 하네스의 관점에서 보면 컨텍스트 엔지니어링은 하네스의 “컨텍스트와 도구” 계층 대부분을 구현합니다. 각 의사결정 시점에 에이전트가 어떤 정보를 보고, 그 정보를 어떻게 구성할지 결정하기 때문입니다. 컨텍스트를 잘 설계하면 모델에 알맞은 배경, 제약, 행동 인터페이스를 제공하여 모델의 범용 사고 능력을 특정 작업에 효과적으로 적용할 수 있습니다.

![그림 2-1 컨텍스트 창의 구성 개요](images/fig2-1.svg)

## 컨텍스트: 에이전트 능력의 상한을 결정하는 핵심

대규모 언어 모델은 표준 벤치마크에서 뛰어난 성과를 내지만 실제 비즈니스 환경에서는 기대에 못 미치는 경우가 많습니다. 이유는 간단합니다. 모델의 능력은 범용적이지만 구체적인 작업은 제품 아키텍처, 비즈니스 규칙, 운영 제약, 내부 관례 같은 현지 지식에 의존합니다. 이런 정보는 대개 모델 파라미터에 들어 있지 않습니다.
...
- **환경 설정**: 개발 환경 구성, 테스트 데이터베이스 연결 문자열, 스테이징 배포 절차, API 키 관리 방식입니다. 이 정보가 없으면 로컬에서 작동한 수정이 테스트 환경에서 곧바로 실패할 수 있습니다.

코드, 프로세스, 환경이라는 세 범주는 에이전트가 효과적으로 일하는 데 필요한 최소 컨텍스트를 이룹니다. 모델에 내재된 능력은 토대일 뿐이며, 컨텍스트가 에이전트 능력의 상한을 결정합니다. 잘 정리된 컨텍스트를 갖춘 보통 수준의 모델이 부족한 컨텍스트로 작동하는 더 강한 모델보다 나은 성과를 내는 경우도 많습니다.
...
컨텍스트 엔지니어링은 기술 문제이지만 더 근본적으로는 조직의 문제입니다. 많은 팀에서 중요한 지식은 암묵적인 상태로 남아 있습니다. 아키텍처 결정은 선임 엔지니어의 기억 속에 있고, 비즈니스 규칙은 비공식적으로 전해지며, 중요한 맥락은 비공개 채팅 기록에 묻혀 있습니다. 팀 자체가 열악한 정보 환경이라면 아무리 강한 AI 에이전트라도 제약을 받을 수밖에 없습니다.
...
OpenAI 연구원 Jiayi Weng은 이 점을 명확하게 표현했습니다. **“사람과 모델 모두에게 가장 중요한 것은 컨텍스트입니다.”** 그는 자신의 일을 돌아보며 “OpenAI에서 하는 제 일은 그리 어렵지 않습니다. 다른 누군가에게 제 컨텍스트가 전부 있다면 그 사람도 할 수 있습니다”라고 말했습니다. 같은 원리가 에이전트에도 적용됩니다. 에이전트 능력의 상한은 모델 크기만으로 정해지지 않으며, 각 의사결정 시점에 제공하는 컨텍스트의 완전성과 정확성에 달려 있습니다. Weng은 팀워크의 핵심 문제가 컨텍스트의 불일치이고, AI와 사람이 같은 환경을 공유하지 않는다는 점이 AI가 단기간에 사람을 대체할 수 없는 이유 중 하나라고도 지적했습니다. 컨텍스트 엔지니어링은 바로 이 문제, 즉 에이전트에 필요한 구조화된 배경 정보를 모델에 체계적으로 전달하는 방법을 다룹니다.
...
도구 호출이 없는 단일 요청이라는 가장 단순한 사례부터 시작하겠습니다. 사용자가 “안녕하세요. 당신은 누구인가요?”라고 묻습니다. 이 예시는 뒤의 로컬 LLM 배포 실험과 연결하기 위해 로컬에 배포한 Qwen3-0.6B 모델을 사용합니다. 예시에 표시된 시간은 설명용일 뿐 이 책의 시간적 배경과는 무관합니다.
...
실제 에이전트의 작업 흐름은 한 번의 질의응답보다 복잡한 경우가 많습니다. 사용자가 “밴쿠버의 현재 시간과 날씨를 알려 줘”라고 물으면 모델은 현재 시간과 최신 날씨라는 동적인 외부 정보에 접근해야 합니다. 다음 예시에서는 에이전트 프레임워크와 모델 사이의 상호작용을 단계별로 살펴봅니다.

![그림 2-3 두 번의 모델 API 호출에 대한 전체 상호작용 과정](images/fig2-3.svg)

그림의 “첫 번째”와 “두 번째”는 모두 두 도구를 순서대로 호출한다는 뜻이 아니라 **모델 API 호출**을 가리킵니다. 이 예에서는 `get_current_time`의 시간대 인수와 `get_weather`의 도시 및 단위 인수를 모두 미리 정할 수 있습니다. 날씨 서비스가 해당 도시의 최신 날씨를 직접 반환하고 시간 도구의 출력에 의존하지 않으므로 에이전트 프레임워크는 두 도구를 병렬로 실행할 수 있습니다. 뒤의 도구 인수를 앞선 도구의 결과에서 얻어야 한다면 모델은 다음 라운드에서 그 도구 호출을 요청해야 하며, 두 도구는 순차적으로 실행해야 합니다.
...
이번에는 모델이 `tool_calls`를 반환하지 않고 텍스트로 응답합니다. 도구 결과만으로 사용자의 질문에 답하기에 충분하기 때문입니다. 더 많은 정보가 필요하면, 예를 들어 사용자가 “도쿄는 어때?”라고 물으면 모델은 다시 `tool_calls`를 반환할 수 있습니다. 그러면 에이전트 프레임워크는 도구 실행, 결과 반환, 모델 재호출이라는 같은 순환을 반복합니다. **이 “요청 → 도구 호출 → 실행 → 결과 반환 → 다음 요청” 순환이 1장에서 소개한 ReAct 루프를 API 수준에서 구현한 것입니다.**
...
>
>
> 이 실험의 목표는 두 가지입니다. 첫째, 소형 모델의 도구 호출 능력을 관찰합니다. 둘째, API 수준에서는 숨겨지는 원시 토큰 스트림, 즉 사고 사슬, 특수 토큰, 도구 호출 형식을 살펴봅니다. 이 과정에서 KV Cache가 첫 토큰 생성 시간(Time to First Token, TTFT)에 미치는 영향도 확인하여 다음 절을 위한 직관을 쌓을 수 있습니다.
>
...
> 4. **스트리밍 응답의 장점**: 스트리밍 출력으로 도구 호출 결정과 결과 처리 과정을 포함한 모델의 사고 과정을 사용자가 실시간으로 볼 수 있습니다.
> 5. **KV Cache의 영향(부수적 관찰)**: 시스템 프롬프트를 바꾸지 않은 채 연속해서 두 대화를 시작하고 두 번째 대화의 TTFT를 기록합니다. 그런 다음 시스템 프롬프트 앞부분의 몇 글자를 바꾸고 새 대화를 시작하여 TTFT를 비교합니다. 접두부를 바꾸지 않은 경우에는 접두부 캐시를 사용할 수 있어 훨씬 빠르지만, 접두부를 수정한 경우에는 전체 접두부를 다시 계산해야 합니다. 이 현상은 다음 절의 주제입니다.
>
> **실전에서의 ReAct 루프.**
>
...
구체적인 사례를 보기 전에 **KV Cache**의 직관부터 이해해 보겠습니다. 모델은 토큰을 하나 생성할 때마다 앞선 토큰의 중간 계산 결과를 다시 참조해야 합니다. 매 라운드마다 이 결과를 처음부터 계산하면 컨텍스트가 길어질수록 비용이 계속 커집니다. KV Cache는 중간 Key-Value 상태를 저장하여 이후 계산에서 재사용하게 합니다. **전제는 접두부가 완전히 같아야 한다는 것입니다.** 글자 하나만 달라져도 해당 지점 이후의 접두부 캐시를 재사용할 수 없어 다시 계산해야 합니다. 여기서 용어를 구분할 필요가 있습니다. 이 절에서 요청 간 “캐시 적중”을 설명할 때 API 공급자가 보통 사용하는 용어는 Prompt Cache입니다. 추론 엔진의 KV Cache 위에 구축한 요청 간 캐시입니다. 이 두 계층의 차이는 이 절 마지막에서 설명합니다.
...
타임스탬프 한 줄이 요청할 때마다 KV Cache를 무효화했습니다. 시스템 프롬프트가 매번 달라지면서 모델이 접두부의 Key-Value 쌍을 처음부터 다시 계산해야 했습니다. 여기서 Key와 Value는 어텐션 메커니즘에 쓰이는 두 종류의 벡터이며, 아래 실험 2-2에서 그 역할을 시각적으로 보여 줍니다. 이처럼 눈에 보이지 않는 비용은 에이전트 시스템에서 되풀이됩니다. 무해해 보이는 코드 한 줄이 전체 추론 파이프라인을 10배 가까이 느리게 만들 수 있습니다. 이 절에서는 이러한 함정을 피하는 방법을 설명합니다.
...
>
> 1. **시스템 프롬프트와 도구 정의를 확정한 뒤에는 바꾸지 마세요.** 공백 하나를 추가하는 사소한 수정도 전체 캐시를 무효화하여 지연 시간과 비용을 몇 배로 늘릴 수 있습니다. 정확한 정도는 모델과 설정에 따라 달라집니다.
...
>
> 세 결론의 직관은 간단합니다. LLM은 컨텍스트를 처리하면서 이미 계산한 접두부의 결과를 캐시하여 다음 요청에서 재사용합니다. **접두부가 바이트 단위로 같으면 계산 결과를 재사용할 수 있고, 달라지면 그 지점 이후의 계산을 다시 해야 합니다.** 시스템 프롬프트와 도구 정의는 보통 접두부의 가장 앞쪽에 놓이고 계산 비용도 가장 큰 부분입니다. 이들이 바뀌면 그 뒤의 캐시된 중간 결과가 모두 무효화됩니다.
>
...
>
> 이 실험은 **긴 사고 사슬 생성과 도구 호출이 모두 컨텍스트 내 학습에 크게 의존한다**는 사실을 보여 줍니다. 컨텍스트 내 학습은 모델을 다시 학습하지 않고 입력에 담긴 지시와 예시에 따라 작업에 적응하는 능력입니다. 컨텍스트 내 학습의 내부 메커니즘과 에이전트 아키텍처 설계에 주는 시사점은 이 장의 컨텍스트 압축 절을 참조하세요.
...
**첫째, 표준 API 형식을 사용해야 하는 이유를 알 수 있습니다.** 개발자가 API를 우회해 메시지를 직접 이어 붙이면, 예를 들어 도구 결과를 도구 메시지가 아니라 일반 사용자 메시지로 전달하면 Chat Template이 대화를 잘못 표현할 수 있습니다. Qwen3의 Chat Template은 다중 라운드 도구 호출에서 이전의 내부 사고 내용을 `<think>` 태그 안에 유지하여 도구 호출 사이의 연속성을 보존할 수 있습니다. 템플릿이 새 사용자 라운드를 감지하면 이 사고 컨텍스트를 지우고 새로 시작합니다. 도구 결과를 사용자 메시지로 잘못 표시하면 엉뚱한 시점에 초기화가 일어나 다단계 사고의 일관성이 약해질 수 있습니다. 모델 계열마다 과거 사고 사슬을 다루는 방법이 크게 다르고 전략도 빠르게 바뀐다는 점에 주의해야 합니다. DeepSeek R1 시대의 공식 지침은 **과거 사고를 모두 제거하는 것**이었습니다. 다중 라운드 대화에서 `reasoning_content`를 제외하고 `content`만 다시 전달했습니다. 과거 CoT가 R1의 학습 입력에 전혀 등장하지 않아 되돌려 보내면 분포 밖 입력이 되어 출력을 방해할 수 있고, 토큰도 상당히 절약할 수 있었기 때문입니다. 하지만 이 전략은 에이전트 시나리오에 문제가 있습니다. 중간 사고에는 “왜 이 도구를 호출했는가, 어떤 가설을 배제했는가” 같은 중요한 상태가 담깁니다. 이를 제거하면 모델이 라운드마다 처음부터 다시 사고하여 같은 실수를 반복하고 장기 계획을 잃기 쉽습니다. 이에 DeepSeek는 V4에서 정책을 **완전히 뒤집어**, `tool_calls`가 있는 메시지를 포함한 모든 어시스턴트 메시지의 `reasoning_content`를 그대로 되돌려 보내도록 의무화했습니다. 이를 지키지 않으면 API가 오류를 반환합니다. Kimi K2, GLM-5 등도 같은 프로토콜을 채택했습니다. 한편 Claude는 도구 호출 루프 안에서 클라이언트가 사고 블록을 서명 검증 정보와 함께 수정 없이 API에 돌려보내도록 요구하며, 새 사용자 라운드가 시작되면 서버가 이전 사고를 무시합니다. “제거”에서 “반드시 되돌려 보내기”로 바뀐 업계 전반의 흐름은 강력한 증거입니다. **에이전트 시나리오에서 사고는 낭비가 아니라 상태입니다.** 사용하기 전에 해당 모델의 최신 템플릿 문서를 확인하세요.

**둘째, KV Cache가 접두부에 매우 민감한 이유를 알 수 있습니다.** Chat Template은 시스템 메시지와 도구 정의를 입력 앞쪽의 고정된 토큰 시퀀스로 변환합니다. 이 토큰의 Key-Value 상태는 캐시하여 요청 간에 재사용할 수 있습니다. 시스템 프롬프트에 공백 하나를 추가하는 정도라도 접두부의 토큰 하나가 바뀌면 그 지점 이후의 캐시를 재사용할 수 없습니다.
...
**접두부를 수정하면 캐시가 무효화되는 이유는 무엇일까요?** 대규모 언어 모델은 여러 Transformer 계층을 쌓아 구성합니다. 현대 LLM은 보통 수십에서 수백 개의 계층을 가지며, 각 계층은 자체 K와 V 캐시를 만듭니다. 이 계층들은 차례로 연결됩니다. 1계층의 출력이 2계층의 입력이 되고, 2계층의 출력이 3계층의 입력이 되는 식입니다. 각 단어를 처리할 때 1계층은 그 단어와 앞선 모든 단어를 고려해 중간 표현을 출력하고, 2계층은 그 표현을 받아 더 처리합니다. 시스템 프롬프트의 글자 하나처럼 앞쪽 토큰이 바뀌면 1계층의 출력과 2계층의 입력이 달라지고, 이 차이가 이후 계층으로 전파됩니다. 변경 지점 이후의 캐시 상태를 다시 계산해야 합니다. 비용은 상당합니다. 이미 처리한 토큰을 다시 계산하고 다시 과금할 수 있으며, 지연 시간도 크게 늘어납니다. 이 장의 실험에서는 몇 배의 증가를 측정했습니다. 그래서 이 책은 시스템 프롬프트를 확정한 뒤에는 바꾸지 말라고 반복해서 강조합니다.
...
>
> **동적 시스템 프롬프트**는 가장 흔한 실수 중 하나입니다. 일부 개발자는 에이전트에 현재 시간을 “알려 주려고” 시스템 프롬프트에 “Current time: 2025-09-14 10:30:45.123456” 같은 타임스탬프를 넣습니다. 유용한 컨텍스트를 제공하는 것처럼 보이지만 타임스탬프가 요청할 때마다 바뀌므로 전체 시스템 프롬프트가 달라지고 KV Cache를 완전히 무효화합니다. 올바른 방법은 시간 정보를 대화 끝의 사용자 메시지 일부로 추가하거나, 정말 필요할 때만 도구 호출로 가져오는 것입니다.
>
> **동적 사용자 설정**은 남은 API 호출 횟수나 계정 잔액 같은 사용자 상태를 요청마다 갱신하려는 방식입니다. 이 정보를 컨텍스트에 삽입하면 캐시가 깨집니다. 필요할 때 전용 상태 관리 메커니즘으로 처리하는 것이 좋습니다.
>
> **도구 정의의 동적 정렬**도 미묘한 함정입니다. 일부 시스템은 사용 빈도에 따라 도구 순서를 동적으로 바꿉니다. 하지만 도구 정의는 컨텍스트에서 큰 비중을 차지하는 경우가 많습니다. 도구 하나의 설명과 파라미터 명세만 수백 토큰이 될 수 있습니다. 순서를 바꾸면 전체 캐시가 무효화됩니다. 실험 결과, 순서를 고정해도 도구 선택 정확도에는 거의 영향이 없었지만 성능은 크게 향상됐습니다.
>
...
>
> **요약**: 이러한 해로운 패턴의 해결책은 모두 이 절 첫머리에서 제시한 세 원칙으로 돌아갑니다. 한 가지를 덧붙이면, 모델 공급자는 표준 인터페이스에 많은 최적화를 적용해 왔으므로 표준 형식에서 벗어나면 문제가 생길 가능성이 큽니다. 앞서 설명했듯 이는 주로 캐시 문제가 아니라 모델 능력의 문제입니다.

### KV Cache와 Prompt Cache: 두 계층의 캐시

계속하기 전에 혼동하기 쉬운 두 개념을 구분하겠습니다. **KV Cache**는 모델 추론 내부의 최적화입니다. 한 번의 추론 과정에서 이미 처리한 토큰의 Key-Value 상태를 캐시하여 중복 계산을 피합니다. **Prompt Cache**는 API 서비스 계층의 최적화입니다. 여러 API 요청에서 동일한 접두부의 계산 결과를 재사용합니다. 둘 다 접두부의 안정성에 의존하지만 작동하는 계층이 다릅니다. KV Cache는 요청 안에서 토큰 생성을 가속하고, Prompt Cache는 요청 사이의 중복 접두부 계산을 줄입니다. 실제로 API 공급자는 요청의 접두부를 대조합니다. 여러 요청이 같은 접두부를 공유하면, 예를 들어 시스템 프롬프트와 도구 정의가 바뀌지 않으면 공급자는 해당 토큰을 다시 계산하지 않고 캐시된 접두부 계산을 재사용할 수 있습니다. 캐시에서 읽는 비용은 새로 계산하는 비용보다 훨씬 낮습니다. Anthropic과 DeepSeek에서는 약 10분의 1, OpenAI의 GPT-5 계열도 마찬가지로 약 10분의 1입니다. 이전 GPT-4o 세대는 절반 가격이었고, GPT-5.6부터는 캐시 쓰기에 1.25배의 추가 비용이 부과됩니다. 캐시 활성화와 과금 방식은 공급자마다 다릅니다. Anthropic은 명시적인 `cache_control` 경계 지점을 요구하고 캐시 쓰기에 할증을 부과하며, 캐시 가능한 최소 길이(예: 1,024토큰)와 TTL 제한(기본 약 5분)을 둡니다. OpenAI는 별도 선언 없이 자동 접두부 캐시를 사용합니다.

컨텍스트를 설계할 때 두 캐시 계층 모두 안정적인 접두부를 요구하지만, API 과금에 직접 영향을 미치는 Prompt Cache의 경제적 효과가 더 큽니다.

### 아키텍처 제약으로서의 캐시

다음 절은 프로덕션급 에이전트의 아키텍처 세부 사항을 다룹니다. 처음 읽는 독자는 건너뛰었다가 에이전트를 구축할 때 다시 읽어도 됩니다.
...
**프롬프트 구조는 캐시 경계에 맞춰집니다.** 시스템 프롬프트는 캐시 경계 표시로 나뉩니다. 표시 앞의 콘텐츠는 사용자와 세션을 넘어 전역으로 캐시할 수 있고, 뒤의 콘텐츠에는 사용자 및 세션별 정보가 들어갑니다. 따라서 프롬프트의 순서는 의미 논리보다 캐시 경제성에 더 크게 좌우됩니다. 캐시 경계 앞에 놓는 런타임 조건, 예를 들어 OS 종류, 현재 모드, 사용자 선호가 하나씩 늘어날 때마다 캐시 키 변형의 수도 늘어납니다. 각 조건이 이진이면 조건 N개에서 2^N개의 조합이 생깁니다. 예를 들어 이진 조건 3개(macOS/Linux, 일반/디버그 모드, 중국어/영어)는 2×2×2 = 8개의 캐시 키를 만듭니다. 따라서 프롬프트 조각은 “캐시 가능” 또는 “캐시 파괴” 유형으로 나누고, 후자에는 명시적인 경고 표시를 붙입니다.

**하위 에이전트는 상위 에이전트와 바이트 단위로 일치해야 합니다.** 주 에이전트가 하위 에이전트를 생성하거나 우회 질의를 수행할 때 하위 에이전트의 프롬프트, 도구 정의, 모델 설정, 메시지 접두부, 사고 설정은 상위 에이전트의 캐시 키와 바이트 단위로 일치해야 합니다. 하위 에이전트가 시작한 API 요청의 접두부가 상위 에이전트의 요청과 같으면 API 공급자의 Prompt Cache를 사용할 수 있어 비용과 지연을 줄일 수 있기 때문입니다. 이 제약은 캐시 계층에서 위로 전파되어 에이전트를 생성하고 파라미터를 전달하는 방식에 영향을 줍니다.
...
핵심 통찰은 **캐시의 경제성이 사후 최적화가 아니라 선행 아키텍처 제약**이라는 점입니다. 에이전트 시스템에서 Prompt Cache를 사용하면 캐시 키 일관성 요구가 프롬프트 설계, 멀티 에이전트 조율, 세션 복구 등 여러 계층으로 퍼집니다. 이 제약을 아키텍처에 일찍 반영할수록 이후의 엔지니어링 비용이 줄어듭니다.
...
- **에이전트 상태 표시줄**: 작업 진행률, 환경 상태, 도구 호출 횟수 같은 동적 메타 정보를 컨텍스트 끝에 주입하는 독립적인 메커니즘입니다. 암묵적인 상태를 모델이 능동적으로 요약하지 못하는 한계를 보완합니다. 스마트폰 화면 상단에 시간, 배터리, 네트워크 신호가 표시되듯 에이전트 상태 표시줄은 모델이 현재 런타임 상태에 언제든 접근하게 합니다.
...
발견한 스키마는 한 번만 추가된다는 점을 오해하기 쉽습니다. 이후에도 궤적의 원래 위치에 남고 다음 메시지는 그 **뒤에** 추가됩니다. 라운드마다 다시 끝으로 옮기지 않습니다. 매번 다시 주입하면 프리필을 반복해 캐시의 목적을 잃습니다. 두 API 모두 이후 요청에서 스키마의 원래 위치를 유지합니다. OpenAI는 이후 요청에도 `tool_search_output` 항목의 위치를 보존하도록 요구하며, 같은 도구를 다음 라운드에서 다시 불러올 필요가 없습니다. Anthropic은 `tool_reference` 블록을 대화 기록의 원래 위치에서 인라인으로 확장합니다. 문서의 표현대로 “모든 라운드에서 같은 캐시 적중을 유지”합니다. Prompt Cache의 TTL이 만료되어 전체 접두부를 다시 계산하거나, 불러온 도구 집합을 수정·제거·재정렬해 해당 지점부터 캐시가 무효화될 때만 재계산합니다.
...
>
> 프롬프트 엔지니어링의 각 요소가 기여하는 정도를 측정하기 위해 `prompt-engineering` 프로젝트는 Tau-Bench 프레임워크를 바탕으로 체계적인 제거 실험을 설계했습니다. Tau-Bench는 항공사 고객 서비스와 소매 고객 지원이라는 두 가지 현실 시나리오를 시뮬레이션합니다. 에이전트는 항공편 변경, 환불 처리, 재고 조회 같은 복잡한 다단계 작업을 처리해야 합니다.
>
...
>
> 제거 실험의 결론은 놀랍지 않습니다. 혼란스러운 정보 구성은 성공률을 30% 넘게 떨어뜨렸습니다. 더 가치 있는 것은 방법론 자체입니다. 에이전트 성능이 나쁠 때 프롬프트 전체를 다시 쓰는 대신 먼저 제거 실험을 하는 편이 좋습니다. 구성 요소를 하나씩 끄고 어느 요소가 가장 큰 영향을 주는지 관찰하세요. 직감으로 추측하는 것보다 훨씬 신뢰할 수 있습니다.
>
...
이 장에서 소개한 컨텍스트 메커니즘 자체가 새로운 주입 표면을 만든다는 점에도 주의하세요. 다음에 다룰 에이전트 스킬이 대표적인 예입니다. 스킬은 외부 콘텐츠를 지시로 불러오는 관행을 공식화합니다. 서드파티 스킬은 권한이 높은 지시 콘텐츠로 컨텍스트에 들어가므로 악성 지시가 웹페이지의 숨은 텍스트보다 더 직접적으로 영향을 미칠 수 있습니다. 알 수 없는 출처의 스킬은 실행할 코드처럼 설치 전에 내용을 검토해야 합니다. 에이전트 상태 표시줄도 마찬가지입니다. 모델이 상태 정보를 상당히 신뢰하기 때문에 이 메커니즘이 효과적입니다. 신뢰할 수 없는 웹페이지 조각처럼 공격자가 조작할 수 있는 출처에서 상태 정보를 가져오면 공격자가 그 신뢰를 악용할 수 있습니다.
...
**1계층(메타데이터)**: 각 스킬에는 반드시 YAML frontmatter로 시작하는 `SKILL.md` 파일이 있어야 합니다. YAML frontmatter는 파일 맨 위에서 `---`로 구분한 메타데이터 블록으로 책의 판권 페이지와 비슷하며, `name`과 `description` 필드를 담습니다. 에이전트 프레임워크는 시작할 때 설치된 모든 스킬을 검색하고 그 `name`과 `description`을 대화 컨텍스트에 주입합니다. 보통 몇백 토큰만 들며, 주입 위치의 상충 관계는 다음 절에서 다룹니다. 모든 스킬 내용을 컨텍스트에 넣지 않고도 어떤 전문 능력을 사용할 수 있는지 에이전트가 발견하게 하는 것이 목적입니다.

라우팅은 메타데이터의 `description` 필드에 크게 의존합니다. 상시 불러오는 토큰 수를 줄일 만큼 간결하면서도 기능 요약이 아니라 라우팅 규칙처럼 작성해야 합니다. 가장 명확한 패턴은 “Use when / Do not use when”이며, 스킬을 실행해서는 안 되는 상황을 알려 주는 **부정 예시**를 함께 제공합니다. 부정 예시는 선택 사항이 아니라 정확한 스킬 라우팅의 필수 요소입니다. “help with backend”처럼 범위가 넓은 설명은 무관한 작업에서도 활성화되지만, 명시적인 제외 조건은 라우팅을 훨씬 정확하게 합니다. 라우팅에서는 “무엇을 할 수 있는가”보다 “언제 사용해야 하는가”가 훨씬 중요합니다.

**2계층(핵심 작업 흐름)**: 특정 작업에 스킬이 필요하다고 판단하면 에이전트는 전용 스킬 도구로 `SKILL.md` 전체를 불러옵니다. 내용은 도구 결과로 대화 기록에 나타납니다. PPTX 스킬[^ch2-4]을 예로 들면 PowerPoint 파일 처리의 핵심 작업 흐름을 담습니다. Microsoft의 오픈 소스 문서-Markdown 변환 도구인 markitdown으로 텍스트를 추출하는 방법, PPTX 파일의 압축을 풀어 원시 XML 구조에 접근하는 방법, 핵심 파일의 경로 규칙 등이 포함됩니다.
...
스킬은 지시 문서뿐 아니라 실행 가능한 코드 도구와 템플릿 파일도 함께 포함할 수 있습니다. 이에 따라 단순한 지식 전달을 넘어 실제 작업 능력으로 확장됩니다.
...
이는 에이전트 개발자에게 중요한 원칙을 보여 줍니다. **에이전트의 상호작용 방식을 고를 때 모델과 API가 지원하도록 설계된 상호작용 패턴에 맞추세요.** Claude로 에이전트를 구축한다면 스킬과 구조화된 시스템 프롬프트를 충분히 활용하고, 다른 모델을 사용한다면 해당 모델 공급자가 최적화한 관례를 따르세요. 기반 모델 회사가 권장하는 에이전트 사용 패턴에는 보통 그 모델이 학습하고 평가받은 방식이 반영되어 있습니다.

### 스킬 구현 방식과 상충 관계

스킬을 정의했으므로 다음 질문은 구체적인 엔지니어링 문제입니다. 스킬 내용을 컨텍스트의 어디에 둘까요? 이 설계 결정은 KV Cache 효율과 모델의 스킬 지시 수행 능력에 직접 영향을 줍니다. 원칙적으로 두 가지 간단한 방법이 있지만 모두 비용이 큽니다. Claude Code 같은 프로덕션 시스템은 양쪽의 주된 단점을 피하는 세 번째 방식을 사용합니다.

**방식 1: 시스템 프롬프트(system 메시지)에 주입합니다.** 스킬 내용을 시스템 프롬프트에 직접 추가합니다. 학습 과정에서 이 위치의 지시가 많이 사용되므로 시스템 위치의 콘텐츠를 따르는 능력이 가장 강하고 스킬 실행 효과도 좋습니다. 문제는 새 스킬을 불러올 때마다 시스템 메시지의 내용이 달라져 KV Cache 접두부가 무효화된다는 점입니다. 검색 스킬 다음에 문서 스킬을 사용해야 하는 작업처럼 에이전트가 스킬을 자주 바꾸면 캐시가 반복해서 무효화되어 지연과 비용이 크게 늘어납니다.

**방식 2: 일반 파일처럼 읽어 컨텍스트 중간에 내용을 둡니다.** 에이전트가 범용 파일 읽기 도구로 스킬 파일을 읽으면 그 내용은 대화 기록의 도구 결과, 즉 컨텍스트 중간에 나타납니다. 시스템 프롬프트가 바뀌지 않으므로 KV Cache에는 전혀 영향을 주지 않습니다. 하지만 모델의 **지시 수행 능력**에 더 높은 요구를 둡니다. 모델은 긴 컨텍스트 중간에 있는 스킬 지시를 일반적인 참고용 도구 출력으로 취급하지 않고 정확히 알아보고 따라야 합니다. 실제로 모델마다 이 방식의 지원 수준이 크게 다릅니다. Claude는 학습에서 중간 위치의 지시 수행 데이터를 많이 사용했으므로 가장 안정적으로 작동하지만, 다른 모델은 컨텍스트 중간에 주입한 지시를 따를 때 성능이 떨어지는 경우가 많습니다.

**방식 3(프로덕션 구현): 메타데이터는 동적 컨텍스트로 제공하고 전체 내용은 전용 도구로 필요할 때 불러옵니다.** Claude Code의 핵심 방식은 스킬의 “라우팅”과 “실행”을 분리합니다. 모델은 먼저 사용할 수 있는 스킬의 메타데이터를 받아 현재 작업에 특정 스킬이 필요한지 판단합니다. 스킬을 선택한 뒤에만 `SKILL.md` 전체를 불러옵니다. 이 설계는 컨텍스트 오버헤드, Prompt Cache 재사용, 지시 수행 능력 사이의 균형을 잡습니다.

- **메타데이터 목록**—설치된 모든 스킬의 `name` + `description`으로, 보통 몇백 토큰에 불과합니다. 모델이 현재 작업과 관련 있는 스킬을 판단할 수 있도록 미리 제공합니다. 중요한 점은 **이 메타데이터를 컨텍스트에 주입할 때 사용하는 메시지 역할이 에이전트 스킬 메커니즘의 고정 요구 사항이 아니라 Claude Code 에이전트 하네스의 구현 세부 사항**이라는 점입니다. 과거 일부 Claude Code 버전에서는 이런 동적 컨텍스트가 `<system-reminder>`로 감싼 user 역할 콘텐츠로 나타났습니다. 대화 중간의 system 메시지를 지원하는 최신 구현 경로에서는 이를 끝에 추가한 system 역할의 컨텍스트 블록으로 대신 표현할 수 있습니다. 표현 방식이 무엇이든 안정적인 컨텍스트 접두부를 반복해서 다시 쓰지 않으면서 현재 사용할 수 있는 스킬을 모델에 알리는 것이 공통 목표입니다.
...
**대화 도중 시스템 컨텍스트를 동적으로 추가하는 방식은 스킬만의 것이 아닙니다.** 사용할 수 있는 스킬의 메타데이터 외에도 현재 작업 상태, 런타임 환경 등 동적 정보를 모델에 계속 알려야 할 수 있습니다. 다음 **에이전트 상태 표시줄** 절에서 이 메커니즘을 더 살펴봅니다. 스킬 메타데이터 목록은 그 한 가지 구체적인 사례로 볼 수 있습니다.

다음 두 그림은 이 설계의 효과를 스킬이 궤적에서 놓이는 위치와 KV Cache의 변화라는 두 관점에서 보여 줍니다.

![그림 2-12 스킬을 활성화한 에이전트 궤적의 전체 구조](images/fig2-12.svg){height=55%}

![그림 2-13 에이전트 궤적이 늘어날 때 KV Cache의 변화](images/fig2-13.svg)

흔한 오해를 하나 바로잡아야 합니다. “KV Cache 친화적”이라는 말은 “비용이 전혀 없다”는 뜻이 아닙니다. 몇백에서 몇천 토큰을 처음 삽입할 때에는 여전히 쓰기 비용이 듭니다. 앞에서 설명했듯 Prompt Cache 쓰기는 할증 요금이 붙을 수도 있습니다. 정확한 의미는 **한 번 쓰고 반복해서 이점을 얻는다**는 것입니다. 모델이 어떤 스킬의 존재나 문서 내용을 알게 하려면 그 정보가 적어도 한 번은 캐시에 들어가야 합니다. Claude Code는 이 비용을 한 번만 지불하고 세션의 나머지 기간에는 반복하지 않습니다. 같은 정보를 시스템 프롬프트에 넣는 방식과 비교해 보세요. 업데이트할 때마다 하류 궤적을 무효화하고, 수만 또는 수십만 토큰의 캐시를 다시 만들어야 할 수 있습니다. 이것이 진정으로 캐시 친화적이지 않은 경우입니다.

### 스킬과 도구의 관계

컨텍스트 관리의 관점에서 스킬 메커니즘은 KV Cache에 매우 친화적입니다. 전문 코드 도구의 정의를 모두 시스템 프롬프트에 넣으면 도구가 늘어날수록 많은 토큰을 차지하고, 변경할 때마다 캐시된 접두부를 무효화합니다. 반면 스킬 + 범용 실행기 모델에서는 도구 집합을 작게 유지할 수 있습니다. 5장에서 보여 주듯 핵심 도구 7개만으로 충분합니다. 스킬 내용은 앞서 설명한 점진적 공개 메커니즘으로 필요할 때 불러오므로 캐시된 접두부에 영향을 주지 않습니다. 4장에서는 두 형태를 비교하고 선택하는 틀을 자세히 제시하며, 8장에서는 지속적으로 진화하는 에이전트가 경험을 지식, 지시, 프로그램, 모델 파라미터 중 무엇으로 인코딩할지 결정하는 방법을 살펴봅니다.
...
스킬 절에서는 “컨텍스트 끝의 user 역할 메타 메시지”를 메타 정보 주입의 범용 통로로 소개했습니다. 스킬 메타데이터 목록이 그 한 가지 용도입니다. 이 절에서는 메커니즘을 더 체계적으로 발전시킵니다. 에이전트 프레임워크는 이 통로로 동적인 런타임 상태를 모델과 동기화할 수 있습니다. 이를 **에이전트 상태 표시줄(Agent Status Bar)**이라고 합니다.
...
시스템 프롬프트와의 차이는 명확합니다. 시스템 프롬프트는 고정된 운영 설명서이고, 에이전트 상태 표시줄은 작업 진행에 따라 계속 갱신되는 실시간 대시보드입니다.
...
실험 2-7은 작은 정성적 사례입니다. 저자와 공동 연구자들은 이 “미리 계산하여 직접 접근”하는 방식의 가치와 한계를 정량화하기 위해 전용 벤치마크로 평가했습니다[^ch2-7]. 이 접근의 일반적인 이름은 **컨텍스트 증류(Context Distillation)**이며, 에이전트 상태 표시줄은 가장 흔한 형태입니다. 벤치마크는 세 종류의 작업(계수, 규칙 귀납, 상태 추적), 고급 API부터 노트북에서 실행할 수 있는 2B 모델까지 11개 모델, 약 24,000회의 평가를 포함했습니다. 결과는 명확합니다.

- **약한 모델에서는 미리 계산한 상태 표시줄이 정확도를 회복합니다.** 가장 약한 모델들의 정확도는 40~54%포인트 높아졌고, 이 작업들에서는 로컬 2B 모델이 상태 표시줄 없는 프런티어 모델과 맞먹기도 했습니다.
- **이미 정답을 내는 강한 모델에서는 효율이 향상됩니다.** 같은 상태 표시줄이 질의당 사고량, 지연 시간, 비용을 대략 10분의 1로 줄였습니다. 사고 토큰은 80~90% 이상 감소했습니다.
- 가장 근본적인 변화는 상태 표시줄이 없을 때에는 컨텍스트가 길어질수록 질의당 사고량이 **계속 증가**하지만, 상태 표시줄이 있으면 **사실상 일정하게 유지**된다는 점입니다. 컨텍스트가 아무리 길어져도 모델은 소수의 상태 항목을 직접 읽습니다. 실험 2-7의 히트맵을 정량화한 결과입니다. 원래는 N이 커질수록 어텐션이 더 얇게 퍼졌지만, 상태 표시줄을 추가하면 고정된 항목에 단단히 집중합니다.

(상태 표시줄은 설명형 문단이 아니라 `Clothes: 9 items (Pass 7, Defect 2)`처럼 빠르게 찾을 수 있는 Key-Value 쌍으로 작성해야 합니다. 논문에서는 같은 상태 정보를 산문으로 작성했을 때 결과가 훨씬 나빴습니다. 모델이 여전히 산문을 읽고 해석해야 하므로 사실상 훑어보기 문제로 되돌아가기 때문입니다.)

그러나 **사전 계산을 어떻게 수행하는지가 매우 중요합니다.** 이 연구에서 실무에 바로 적용할 수 있는 가장 중요한 교훈은 세 가지입니다.

**1. LLM이 아니라 코드로 상태 표시줄을 유지하세요.** 다른 LLM에 기록을 읽고 상태 표시줄을 요약하게 하는 방식이 자연스러워 보일 수 있지만 실험 결과는 나빴습니다. 정규식 함수 20줄은 정답 수준의 정확도를 달성했지만, 전체 기록을 한 번에 처리한 프런티어 모델은 잘못된 항목을 많이 만들었고 하류 정확도를 상태 표시줄이 없을 때보다 더 낮췄습니다. LLM에 긴 기록을 한 번에 요약하게 하면 원래의 컨텍스트 훑어보기 문제를 다른 곳으로 옮길 뿐입니다. 실행 가능한 대안은 **가능하면 코드를 사용**하는 것입니다. LLM이 꼭 필요하다면 **전체 기록을 한 번에 요약하지 말고 항목을 하나씩 추출한 뒤 코드로 집계**하세요.

**2. 원래 컨텍스트를 지우기 전에 상태 표시줄이 물을 수 있는 모든 질문을 다루는지 확인하세요.** 상태 표시줄은 원래 컨텍스트의 **손실 투영**입니다. 관련이 있을 것으로 *예상한* 차원만 미리 계산합니다. 계수와 상태 추적처럼 상태 표시줄만으로 충분한 작업에서는 원래 기록을 삭제하고 상태 표시줄만 남겨 많은 토큰을 절약할 수 있습니다. 하지만 상태 표시줄이 담도록 설계되지 않은 정보를 질문하면 성능이 급격히 나빠질 수 있습니다. 논문의 극단적인 시험에서 상태 표시줄은 “두 항목 조합”의 개수만 저장했지만 질문은 “세 항목의 교집합”을 물었습니다. 상태 표시줄만 남기자 Claude의 정확도는 100%에서 7.6%로 무너졌습니다. 그럴듯하지만 불완전한 상태 표시줄은 모델을 확신에 차서 잘못된 방향으로 이끄는 “거짓 권위”가 될 수 있습니다. 실무에서는 새로운 질문 유형을 **데이터베이스 테이블 스키마 변경**처럼 다루세요. 먼저 상태 표시줄에 해당 필드를 추가하거나 상태 표시줄과 원래 컨텍스트를 모두 유지해야 합니다. 긴 산문을 가로지르는 다단계 사고처럼 깔끔한 구조화 요약으로 표현할 수 없는 작업도 있습니다. 이런 작업에서 상태 표시줄은 토큰을 줄일 수 있지만 정확도 향상까지 기대해서는 안 됩니다.

**3. 상태 표시줄의 정확도를 프로덕션의 최우선 지표로 모니터링하세요.** 실험에서는 놀라운 사실이 드러났습니다. **모델은 상태 표시줄을 거의 무조건 신뢰합니다.** “세 번 전화함”이라고 쓰여 있으면 확인하거나 다시 세지 않고 받아들입니다. 이 신뢰 덕분에 상태 표시줄이 효과적이지만 오류도 최종 답변에 **직접** 흘러듭니다. 시스템은 작은 부정확성을 견딜 수 있습니다. 값의 오차가 약 10%보다 작을 때에는 이점이 대부분 유지됩니다. 하지만 오류가 커지면 잘못된 상태 표시줄이 아예 없는 것보다 나쁠 수 있습니다. 앞에서 설명한 **상태 표시줄 오염** 위험과도 연결됩니다. 상태 정보는 현실 세계의 신뢰할 수 있는 관측에서 가져와야 하며 외부에서 오염될 수 있는 데이터 소스에서는 절대 가져오지 않아야 합니다. 그렇지 않으면 계측기가 잘못된 상태를 보고하여 모델을 오도합니다.
...
(다음은 현재 연구의 선택형 심화 자료입니다. 처음 읽을 때 건너뛰어도 상태 표시줄 사용법을 이해하는 데 지장이 없습니다. 앞의 메커니즘, 근거, 세 가지 교훈만으로도 실무를 안내하기에 충분합니다.)

암묵적 상태의 증류와 어텐션 유도라는 앞의 두 원칙은 상태 표시줄이 효과적인 이유를 설명합니다. 더 깊이 들어가면 상태 표시줄은 **모델이 스스로 추론할 수 없었던 정보를 공급할 수 있습니다**[^ch2-5].

테스트 시점에 모델을 더 강하게 만드는 방법으로 흔히 두 가지를 말합니다. **더 오래 사고하기**, 즉 더 긴 사고 사슬을 생성하는 방법과 **더 많이 샘플링하기**, 즉 여러 답변을 샘플링한 뒤 최선을 고르는 방법입니다. 두 경로에는 같은 한계가 있습니다. 고정된 가중치와 고정된 컨텍스트를 사용해 모델 내부 계산 안에서만 작동합니다. **컨텍스트에 이미 존재하지 않는 정보를 만들 수 없고**, 기존 정보를 재배열할 뿐입니다. 상호작용은 세 번째 경로를 제공합니다. 모델이 출력을 만들고, 외부 계측기가 현실에서의 영향을 관찰한 뒤, 그 관측을 컨텍스트에 다시 기록합니다. 이 관측에는 모델이 **사고만으로 추론할 수 없는 정보**가 들어갈 수 있습니다. 코드가 테스트를 통과했는지, 렌더링한 버튼이 페이지 밖으로 넘쳤는지, 작업 결과 시스템이 어떤 상태가 됐는지 등입니다. 이러한 사실은 가중치나 기존 컨텍스트가 아니라 실행과 측정에서 나옵니다. 이 연구는 개선을 측정하는 기준 자체도 실제 관측에 기반해야 한다는 점을 발견했습니다. 스크린샷만 보는 시각 모델로 점수를 매기면 방금 고친 결함을 발견하지 못하여 루프가 실제로 진전되지 않을 수 있습니다.

에이전트 상태 표시줄은 이 원칙의 가장 흔한 적용 사례입니다. 하네스가 계측기 역할을 합니다. 통화 횟수, 현재 시각, 작업 진행률, 도구가 오류를 보고했는지 같은 런타임 상태를 관측하고, 짧은 구간으로 압축한 뒤 컨텍스트에 다시 씁니다. 상태 표시줄에서 가장 가치 있는 부분은 모델이 대화 기록을 훑어 세어 낼 수 있는 정보보다 **스스로 추론할 수 없는 외부 사실**인 경우가 많습니다. 상태 표시줄은 고립된 사고 작업을 현실 세계의 관측에 기반한 작업으로 바꿉니다. 여기서 설계 원칙 하나를 얻을 수 있습니다. 상태 표시줄이 실제 관측에서 더 많은 정보를 가져올수록 가치가 커집니다. 반대로 상태 요약을 지어내거나 오염될 수 있는 데이터 소스에서 가져오면 계측기가 잘못된 상태를 보고하여 모델을 오도합니다. 앞서 설명한 상태 표시줄 오염 위험에 해당합니다.

[^ch2-5]: Li, Bojie and Noah Shi. *Interaction Scaling: Grounding the Third Axis of Test-Time Compute.* arXiv:2607.11598, 2026.

이 관점에서 보면 1장 발전 흐름의 끝에서 소개하고 10장에서 멀티 에이전트 협업 시스템과 함께 발전시킬 루프 엔지니어링은 상호작용이라는 세 번째 축을 엔지니어링 실천으로 바꿉니다. 검증 과정이 외부 세계의 관측을 컨텍스트에 다시 기록할 때에만 각 반복이 실제로 진전됩니다. 이 단계가 없으면 모델은 기존 정보를 재배열할 뿐입니다. 따라서 “병목은 모델이 아니라 검증기다”라는 주장과 측정 계측기가 실제 관측에 기반해야 한다는 발견은 같은 원칙을 표현합니다.

### 에이전트 상태 표시줄의 구성

앞의 이론적 토대를 바탕으로 에이전트 상태 표시줄에는 다음 종류의 정보를 넣습니다.
...
**현재 환경 상태**: 시스템 시각, 작업 디렉터리 같은 동적 환경 정보, “이 도구를 N번 반복 호출했습니다” 같은 비정상 작업 경고, 암묵적 상태를 명시적 상태로 바꾸는 내용을 포함합니다. 이 설계 원칙은 사람의 인터페이스에도 적용됩니다. 명령줄 인터페이스(CLI)와 그래픽 사용자 인터페이스(GUI)는 모두 사용자가 시스템의 현재 상태를 명확히 인식하게 하는 것을 목표로 합니다.
...
부수 정보와 사용 가능한 능력 목록은 추가된 뒤 보통 바뀌지 않으므로 캐시된 접두부를 무효화하지 않아 캐시 친화적입니다. 작업 계획과 환경 상태는 동적이므로 특수 사용자 메시지로 컨텍스트 끝에 추가한 뒤 작업 진행에 맞춰 갱신해야 합니다. 갱신 방식은 아래에서 설명하듯 KV Cache 비용에 직접 영향을 줍니다.
...
**구현 1: 라운드마다 교체합니다.** API를 호출하기 전에 이전 라운드의 상태 메시지를 메시지 목록에서 제거하고 최신 상태를 끝에 추가합니다. 컨텍스트에는 현재 상태 하나만 남습니다. 비용은 이전 상태를 제거하면 그 위치 이후의 캐시된 콘텐츠가 모두 무효화된다는 것입니다. 이 장의 “동적 타임스탬프” 절에서 설명한 것과 같은 무효화 메커니즘입니다. 다만 상태 메시지는 컨텍스트 끝에 가까우므로 무효화되는 범위가 전체 접두부가 아니라 최근 몇 라운드의 메시지로 제한됩니다.
...
경험칙은 다음과 같습니다. **상태를 자주 갱신하고 궤적이 길다면 구현 2를 선택하세요.** 라운드마다 상태를 교체하면 긴 궤적의 캐시 항목을 반복해서 무효화하므로 오래된 상태 메시지를 유지하는 것보다 비용이 커질 수 있습니다. **궤적이 짧거나 상태 메시지 하나가 큰 경우**, 예를 들어 전체 TODO 목록과 환경 스냅샷을 함께 넣는다면 **구현 1을 선택하세요.** 최근 몇 라운드의 캐시만 무효화하므로 비용이 작고 컨텍스트도 깔끔하고 모호하지 않게 유지됩니다.

> **실험 2-8 ★★: 유용한 에이전트 상태 표시줄 기법 몇 가지**
>
...
### 판독값에서 전략으로: 에이전트의 물리적 시간 인식

실험 2-8의 다섯 기법 가운데 타임스탬프 추적과 도구 호출 카운터는 서로 무관한 메타 정보처럼 보일 수 있습니다. 하지만 둘을 결합하면 더 근본적인 능력을 가리킵니다. 에이전트가 물리적 시간을 조건으로 행동하고 그에 맞춰 속도를 조절하게 하는 것입니다. 사람에게 “3분 안에 문단을 쓰라”고 할 때와 “30분 안에 문단을 쓰라”고 할 때 결과는 다릅니다. 하지만 오늘날의 최첨단 에이전트는 결과가 거의 같은 경우가 많습니다. 에이전트는 작업이 끝났는지, 장애물이 영구적인지 일시적인지, 3분째 실행 중인 도구 호출이 계속 진전 중인지 멈췄는지 판단하기 어렵습니다. 저자와 공동 연구자들은 이 결여된 능력을 **시간 감각(time sense)**이라고 부르고 세 가지 측정 가능한 축으로 나눴습니다[^ch2-8].

- **긴급성(Urgency)**—예산의 축: 시계에 맞춰 노력의 크기를 조절합니다. 시간이 촉박하면 불확실성 속에서도 단호하게 결과를 내고, 시간이 충분하면 더 깊이 조사하고 더 많이 검증하고 더 다듬습니다. 양방향이라는 점이 중요합니다. 긴급성이 낮다는 말은 “덜 하라”가 아니라 “아직 멈추지 말고 계속하라”는 뜻입니다.
- **지속성(Persistence)**—종료점의 축: 진짜 장애물과 일시적 장애물을 구분하고 작업이 끝났는지 판단합니다. 양극단에서 모두 실패할 수 있습니다. 복구 불가능한 오류를 반복해서 시도하거나, 예를 들어 410 Gone을 반환하는 엔드포인트를 다섯 번 다시 호출할 수 있습니다. 반대로 검색 두 번만 하고 “정보를 찾지 못했다”고 단정하여 복구 가능한 실패를 너무 일찍 포기할 수도 있습니다.
- **경계성(Vigilance)**—모니터링의 축: 도구 응답의 예상 밖 시간 특성을 조사할 가치가 있는 증거로 취급합니다. 500ms 안에 반환해야 하는 호출이 5초 걸리거나, 1ms 만에 “성공”했지만 빈 본문을 반환하는 경우는 에이전트가 이러한 판독값을 모니터링할 때 모두 유의미한 신호가 됩니다.

이 세 축은 상태 표시줄과 직접 연결됩니다. 타임스탬프는 긴급성과 경계성의 신호를, 도구 호출 카운터는 지속성의 신호를 제공합니다. 하지만 **이 판독값을 모델에 보여 주기만 해서는 행동이 바뀌지 않습니다.** 한 벤치마크는 시간 정보 없음, 원시 타임스탬프만 제공, 타임스탬프와 해석 지침 제공, 에이전트가 생성한 속도 평가라는 네 조건을 비교했습니다. 원시 타임스탬프의 결과는 시간 정보가 없는 경우와 거의 같았으며 차이는 2~3%포인트에 불과했습니다. 합격률을 10%대 초반에서 40~50%로, 19~49%포인트 높인 것은 행동 지침이었습니다. 다시 말해 모델은 `elapsed_ms=5000 expected_ms=500`을 볼 수 있지만 자동으로 속도를 조정하지 않습니다. 부족한 것은 판독값이 아니라 **판독값에 따라 행동하는 전략**입니다.

이는 앞서 남겨 둔 빈틈을 메웁니다. 도구 호출 카운터는 “세 번째 호출입니다(3/3)”라는 판독값 하나로 행동을 바로잡을 수 있습니다. 한도에 도달하면 멈춘다는 의사결정 규칙이 분명하기 때문입니다. 하지만 “얼마나 많은 노력을 들일지”, “이 장애물을 우회할지” 같은 속도 판단에서는 규칙이 덜 분명하고, 모델이 원시 판독값만으로 올바른 행동을 안정적으로 추론할 수 없습니다. 따라서 효과적인 “속도 상태 표시줄”에는 **판독값**, 즉 작업에 걸린 시간, 도구가 느린지, 장애물을 몇 번 만났는지와 짧은 **행동 전략**, 즉 시간이 촉박하면 결과를 내고, 느린 호출은 진단하며, 넘기 어려운 장애물은 우회하라는 지침이 모두 필요합니다. 둘 중 하나만으로는 부족합니다. 명시적인 판독값은 원재료이고, 이를 행동으로 바꾸는 지침도 모델에 필요합니다.

이 빈틈은 특정 모델만의 문제가 아닙니다. Claude, Gemini, GPT, Qwen 등 네 공급자 계열의 여섯 모델 모두 행동 지침이 없을 때 합격률이 10%를 조금 넘는 수준에 머물렀습니다. 특정 모델의 지능 부족이라기보다 현재의 사후 학습이 시간에 민감한 제어 행동을 충분히 가르치지 못하는 경우가 많다는 뜻입니다. 앞서 설명한 “상태 표시줄 + 행동 지침” 방식으로 추론 시점에 이 빈틈을 보완할 수 있습니다. 작은 모델이 프롬프트에 의존하지 않고 이러한 속도 감각을 가져야 한다면 가중치에 증류할 수도 있습니다. 사후 학습을 다루는 7장에서는 이 학습 경로와 중요한 대조 결과를 설명합니다. 희소한 결과 보상으로는 행동을 유도하지 못했지만 밀도 높은 토큰 수준 신호는 성공했습니다.
...
이 기법들은 모두 메타 정보를 사람이 읽을 수 있는 형태로 컨텍스트에 넣으므로 실용적인 장점이 있습니다. 개발자는 에이전트가 어떤 정보를 받았고 어떤 결정을 내렸는지 확인할 수 있습니다. 더 중요한 점은 모델을 수정할 필요가 없다는 것입니다. 미세 조정 없이 어떤 언어 모델에도 적용할 수 있고, 기법을 개별적으로 시험하거나 필요에 따라 조합할 수 있습니다.
...
컨텍스트 압축에는 서로 다른 두 가지 목적이 있습니다. 효과적인 압축 전략을 설계하려면 둘 다 이해해야 합니다.
...
근본 원인은 어텐션 메커니즘의 성질에 있습니다. **컨텍스트 내 학습의 내부 메커니즘은 사고보다 검색에 가깝습니다.** 1장에서 이 개념을 간단히 소개했고, 에이전트 상태 표시줄 절에서는 메커니즘, 실증 근거, 엔지니어링 실천으로 확장했습니다. 이제 이것이 압축에 어떤 의미인지 살펴보겠습니다.

### 컨텍스트 내 학습의 내부 메커니즘: 사고가 아니라 검색

간단히 말해 **사고가 아니라 검색**이라는 주장은 어텐션이 기존 콘텐츠를 찾아보는 데에는 능숙하지만 한 번의 순전파에서 집계 요약을 능동적으로 계산하는 데에는 약하다는 뜻입니다. 모델이 사고 사슬을 생성하여 단계적으로 사고할 수 있음을 부정하지 않습니다. 한 번의 순전파에서 기존 컨텍스트를 소비하는 방식이 검색에 더 가깝다는 뜻입니다. 압축에 주는 시사점은 분명합니다. 상태 표시줄은 계산한 결론을 컨텍스트에 **추가**하고, 압축은 부풀어 오른 원시 기록을 계산한 결론으로 **교체**합니다. 둘 다 원시 어텐션에 없는 증류 계층을 제공합니다. 차이는 상태 표시줄은 보통 **코드**가 단계별로 결정론적으로 유지하지만, 압축은 큰 원문 덩어리를 증류하기 위해 LLM 호출을 더 자주 사용한다는 점입니다.
...
더 깊은 문제는 긴 컨텍스트가 검색 정확도를 낮춘다는 점입니다. 컨텍스트 창이 가득 차려면 아직 멀었는데도 에이전트가 갑자기 핵심 정보를 찾지 못하거나 이미 해결한 문제에 계속 집중할 수 있습니다. 이 현상을 **컨텍스트 부패(Context Rot)**라고 합니다. 컨텍스트 부패는 컨텍스트 오버플로, 즉 창의 여유가 없어지는 현상과 다릅니다. 오버플로는 “더 넣을 수 없음”이고 부패는 “들어 있지만 찾을 수 없음”입니다. 에이전트는 정상적으로 일하는 것처럼 보이지만 의사결정 품질이 조용히 나빠지므로 후자가 더 교묘합니다. 컨텍스트가 길어질수록 어텐션 가중치가 더 많은 토큰에 퍼져 토큰 하나가 받는 비중이 작아집니다. 더 중요한 점은 무관한 콘텐츠가 컨텍스트의 대부분을 차지하면 에이전트의 의사결정 품질이 떨어진다는 것입니다. 실무에서 가장 흔한 실패는 컨텍스트 창이 작아서가 아니라 정보 밀도가 낮아서 생깁니다. 가끔만 필요한 지식을 매번 불러오고, 안정적인 규칙과 동적 상태를 뒤섞으며, 모델이 보는 콘텐츠는 늘지만 유용한 부분은 더 찾기 어려워집니다. 큰 도서관에서 책 한 권을 찾는 상황에 비유할 수 있습니다. 서가에 무관한 책이 많을수록 원하는 책을 찾기 어렵습니다. 실험 2-2의 어텐션 시각화는 이 현상을 명확하게 보여 줍니다. 긴 컨텍스트에서 모델의 어텐션은 강한 위치 편향을 보입니다. 매우 긴 텍스트 중간에 핵심 정보 하나를 숨기고 모델이 찾는지 시험하는 유명한 “건초더미 속 바늘(Needle in a Haystack)” 실험이 드러낸 문제이기도 합니다.

Andrej Karpathy는 모델의 “나쁜 기억력”이 어느 정도는 버그가 아니라 기능이라는 깊이 있는 통찰을 제시했습니다. 제한된 컨텍스트 창이 많은 세부 사항에서 일반적인 패턴을 추상화하도록 모델을 강제합니다. 사람이 모든 대화의 문장을 그대로 기억하지 않고 전체적인 인상과 행동 패턴을 추출하는 것과 같습니다.
...
이 관점에서 컨텍스트 내 학습은 진정한 학습보다 빠른 적응 메커니즘에 가깝습니다. 추론 중 특정 작업에 맞게 행동을 빠르게 조정하지만, 일시적이고 얕은 조정이므로 세션이 끝나면 사라집니다. 최근의 이론 연구[^ch2-6]도 이 판단을 뒷받침합니다. 컨텍스트에서 예시를 본 모델은 파라미터를 바꾸지 않았는데도 작은 전문 학습 세션과 비슷하게 “일시적으로 맞춤화”된 것처럼 행동합니다. 프롬프트 엔지니어링 절의 퓨샷 예시가 출력 품질을 크게 높일 수 있는 이유, 그리고 이 향상이 세션 사이에 축적되지 않는 이유를 설명합니다. 진정한 파라미터 학습과는 근본적으로 다릅니다.
...
>
> **전략 4: 컨텍스트 인식 압축**—현재 질의의 의도와 축적된 정보를 압축 결정 과정에 포함하는 것이 핵심 혁신입니다. 압축 프롬프트에 “Given the search query: {query}”와 “Current context: {context}”를 명시하여 모델이 목표에 맞는 요약을 생성하게 합니다. 결과는 7회의 반복과 40,157토큰만 필요했고 전체 압축률은 약 3.0%였습니다. 한 사례에서는 147,877자를 1,963자, 즉 약 1.3%로 압축하면서도 창업자 이름과 직책 변화 같은 핵심 정보를 유지했습니다. 이후 검색에서는 무관한 과거 배경과 중복 콘텐츠를 걸러내고 직책 변화와 새 회사 같은 핵심 정보를 지능적으로 추출할 수 있었습니다. 성공의 바탕에는 중요한 통찰이 있습니다. 다단계 작업에서는 단계마다 필요한 정보의 밀도와 유형이 달라집니다. 초기에는 폭넓게 정보를 모으고, 중간에는 사실을 정확히 검증하며, 후기에는 정보를 종합해야 합니다. 컨텍스트 인식 압축은 압축의 초점을 동적으로 조절하여 정보 가치를 극대화합니다.
>
> **전략 5: 인용을 포함한 컨텍스트 인식 압축**—지능형 압축에 정보의 출처를 더하여 각 사실에 소스 URL 인용 표시를 붙입니다. 토큰 사용량은 222,992로 늘고 압축률은 4.1%이지만 인용 덕분에 검증할 수 있습니다. 손실 압축과 무손실 색인을 결합한 방식입니다. 콘텐츠를 압축하더라도 남겨 둔 출처 링크를 따라 원래 자료로 돌아갈 수 있습니다.
>
...
>
> - **임계값 트리거**: 컨텍스트 사용량을 계속 모니터링합니다. 프롬프트 토큰 수가 창의 80%, 즉 128K 창에서 102,400토큰을 넘을 때만 압축을 활성화합니다.
> - **일괄 압축**: 트리거되면 아직 표시하지 않은 모든 도구 결과를 한꺼번에 압축합니다. 예를 들어 네 번째 반복 무렵 컨텍스트가 102,400토큰을 넘었다고 감지하면, 실제로는 약 135,600토큰에서 트리거되어 압축되지 않은 도구 메시지 10개를 즉시 모두 압축합니다.
...
다섯 계층의 순서가 중요합니다. 처음 세 계층은 구현 비용이 가장 낮고 캐시에 미치는 영향을 가장 통제하기 쉬우므로 먼저 사용해야 합니다. 마지막 두 계층은 비용이 더 높지만 압축 효과가 강하므로 폴백 수단으로 사용합니다.

### 압축 전략의 설계 원칙

지금까지 압축의 두 목적, 즉 길이 통제와 사고 품질 향상, 그리고 “컨텍스트 내 학습은 본질적으로 검색”이라는 내부 메커니즘을 분석했습니다. 이를 바탕으로 구체적인 압축 전략을 안내할 네 가지 원칙을 정리할 수 있습니다. 여기서 설명하는 압축은 현재 작업을 위한 것입니다. 여러 작업의 궤적을 오프라인에서 지속적인 경험으로 통합한다면 8장에서 다룰 지속적 진화의 문제가 됩니다.
...
컨텍스트 압축 전략 연구는 에이전트 시스템 설계의 근본적인 문제를 보여 줍니다. **압축은 이해입니다.** 압축 담당 모듈에는 주 모델과 비슷한 수준의 언어 이해 능력이 필요하므로 재귀적인 모델 호출 아키텍처가 만들어집니다. **압축 전략은 작업 유형과 결합됩니다.** 정보 검색 작업은 폭을, 분석 작업은 깊이를, 창작 작업은 영감의 단서를 보존해야 합니다. 미래의 에이전트는 작업 유형에 맞춰 압축 전략을 적응적으로 선택할 수 있어야 합니다.
...
압축이 가장 쉽게 잃는 것은 세부 정보 자체가 아니라 **초기의 아키텍처 결정, 제약의 근거, 실패한 경로**입니다. LLM은 나중에 다시 얻을 수 있어 보이는 정보를 우선 삭제하는 경향이 있습니다. 프로덕션급 에이전트 시스템에서는 압축할 때 보존 우선순위를 명시적으로 정의하는 것이 좋습니다.

1. **아키텍처 결정과 핵심 제약**: 요약해서는 안 됩니다.
2. **수정한 파일 목록과 핵심 변경 기록**: 완전하게 보존합니다.
3. **검증 상태**(통과/실패): 반드시 유지합니다.
4. **해결되지 않은 TODO와 롤백 메모**: 반드시 유지합니다.
5. **도구 출력**: 통과/실패 결론만 남기고 삭제할 수 있습니다.

또한 UUID(Universally Unique Identifier), 해시, IP 주소, 포트 번호, URL, 파일 이름 같은 식별자는 **있는 그대로 정확하게 보존해야 합니다.** PR 번호나 커밋 해시에서 숫자 하나만 바뀌어도 이후 도구 호출이 곧바로 실패합니다.

### 압축보다 격리: 하위 에이전트의 컨텍스트 격리

압축은 정보가 이미 컨텍스트에 들어온 **뒤에** 제거합니다. 더 직접적인 방법은 부피가 큰 중간 정보를 처음부터 주 컨텍스트에 넣지 않는 것입니다. 이것이 **하위 에이전트 컨텍스트 격리(Sub-Agent Context Isolation)**입니다. 주 에이전트는 “많은 파일 읽기”, “코드베이스에서 폭넓게 검색하기”처럼 중간 콘텐츠를 대량으로 만드는 작업을 독립된 하위 에이전트에 위임합니다. 하위 에이전트는 자체 컨텍스트 안에서 탐색을 끝내고 수백 토큰의 간결한 요약만 주 에이전트에 반환합니다.
...
많은 기술적 세부 사항을 다뤘지만 이 장의 중심 주장은 하나입니다. 모델 자체의 능력보다 모델에 무엇을 보여 주고 어떻게 구성하는지가 최종 결과에 더 큰 영향을 미칩니다. API의 메시지 구조는 컨텍스트의 기본 구조를 정의합니다. KV Cache는 무엇을 바꿀 수 있고 없는지 제약합니다. 프롬프트 엔지니어링과 에이전트 스킬은 정적 지시와 동적 지식을 모델에 효율적으로 제공하는 방법을 결정합니다. 에이전트 상태 표시줄은 암묵적 상태를 바로 쓸 수 있는 명시적 정보로 바꿉니다. 압축 전략은 계속 늘어나는 컨텍스트 문제를 길이 통제에만 머무르지 않고 원시 데이터를 정보 밀도가 높은 구조화 지식으로 능동적으로 요약하여 해결합니다.

이 기법들의 공통점은 명시적이고 공학적으로 설계한 정보 관리입니다. 모델이 방대한 컨텍스트에서 단서를 수동적으로 찾게 두지 않고, 정제되고 구조화된 상태를 능동적으로 제공합니다. Rich Sutton의 「Bitter Lesson」으로 돌아가면, 더 많은 계산량을 효과적으로 사용하는 범용 방법이 결국 우세해집니다. 이 장에서 제시한 KV Cache 친화적인 컨텍스트 배치부터 컨텍스트 인식 압축까지 모든 기법은 현재 모델 능력의 경계에서 정보 효율을 극대화하기 위한 구체적인 엔지니어링 실천입니다. 한 가지 구분은 명확히 해야 합니다. 이 장은 **하나의 작업 안에서** 일어나는 상태 갱신과 컨텍스트 저하를 다룹니다. 8장 “에이전트의 지속적 진화”는 다른 시간 척도에서 작동합니다. 여러 작업의 궤적을 평가하고 공통 패턴을 미래 시스템 버전을 바꾸는 지속적 업데이트로 변환하는 방법을 살펴봅니다.

1장의 하네스 프레임워크로 돌아가면 이 장의 모든 기법은 “컨텍스트와 도구” 계층 안에서 작동합니다. 함께 작용하여 각 의사결정 시점에 에이전트가 충분하고 정제되고 구조화된 정보를 받는지 결정합니다. 스킬은 파일 읽기를 통해 도구 결과로 궤적에 들어오고, 압축은 기존 궤적 메시지를 더 간결한 표현으로 바꿉니다. 에이전트 상태 표시줄이 특이한 것은 API 수준에서뿐입니다. 전용 메타 정보 역할이 없으므로 환경 상태와 작업 진행률을 전달하는 데 `user` 메시지를 사용합니다. 의미상으로는 여섯 번째 요소를 만드는 것이 아니라 기존의 다섯 컨텍스트 구성 요소를 보완합니다. 다섯 부분의 구조는 그대로이고, 이 장이 엔지니어링 세부 사항을 더했습니다.

다음 장에서는 하나의 컨텍스트 창 안에서 이루어지는 정보 관리를 넘어 세션 사이에 지속되는 지식 시스템, 즉 사용자 메모리와 지식 베이스를 다룹니다. 이러한 시스템을 통해 에이전트는 시간이 흐르면서 경험을 축적하고 점차 도메인 전문가가 될 수 있습니다.
