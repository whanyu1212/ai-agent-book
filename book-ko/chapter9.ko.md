...
>
> **실험 9-2 ★: WebRTC로 ‘사용자에게 전화하는’ 음성 에이전트 구축**
>
> ‘전화 에이전트’라고 해서 반드시 PSTN에 연결하거나 독자가 실제 전화번호를 준비해야 하는 것은 아닙니다. 알림, 정보 수집, 확인, 후속 조치 업무에서는 전화를 받는 사람이 사용자 본인인 경우가 많습니다. 이때는 브라우저 WebRTC가 더 쉽게 재현됩니다. 사용자가 로컬 페이지를 열고 마이크 사용을 명시적으로 허용하면 에이전트가 브라우저와 실시간 미디어 세션을 맺어 사용자에게 직접 ‘전화’합니다. 브라우저는 마이크 RTP를 로컬 peer로 보내고 에이전트의 하향 음성 RTP를 받습니다. data channel은 오디오 제출 제어와 접근성 자막 미러링만 전달하며 canonical 사용자 의미는 전달하지 않습니다. 이 모든 과정에 E.164 번호나 전화 통신 사업자 계정은 필요하지 않습니다. PSTN/IVR은 외부 기관에 연락해야 하는 프로덕션 업무에는 여전히 적합하지만, ‘에이전트 도구로서의 음성 통화’를 이해하기 위한 전제 조건은 아닙니다.
>
> **실험 목표**: 브라우저 음성 세션을 능동적으로 시작하고, 사용자에게서 누락된 정보를 수집한 뒤 다시 읽어 확인받고, 구조화된 결과를 반환하는 에이전트를 구축합니다. 비교를 위해 ‘직접 호출’ 그룹과 ‘ReAct 계획’ 그룹을 모두 유지합니다.
>
> **기술적 접근법**: 로컬 FastAPI 서비스가 브라우저의 SDP offer를 받고, `aiortc`가 answer를 반환해 미디어를 종단한 뒤 실제로 수신한 마이크 RTP를 PCM으로 디코딩합니다. 로컬 Whisper는 이 PCM에 ASR을 수행하며 대화 모델에는 ASR transcript만 들어갑니다. 에이전트의 확인 질문과 최종 확인은 모두 실제 TTS로 PCM을 합성해 서버의 WebRTC 하향 오디오 트랙에 큐잉합니다. 연결음, 브라우저 `speechSynthesis`, data-channel 텍스트로 음성을 흉내 내서는 안 됩니다. 직접 그룹에서는 호출자가 이름, 목표, 컨텍스트, 지시 사항을 미리 입력해야 합니다. ReAct 그룹은 자연어 업무 하나만 받습니다. 실제 외부 LLM이 민감 정보를 제거한 raw request/response, response ID, 정확한 모델, usage, finish status, latency, hash를 남기고, 감사 가능한 observation / reason / action 요약을 통해 (1) 통화 목표, (2) 알려진 컨텍스트, (3) 누락된 정보, (4) 통화에서 질문하고 확인할 내용을 결정합니다. 두 그룹은 모두 같은 `complete_task` 도구로 사용자가 명시적으로 확인한 필드를 저장합니다. 모델, ASR, TTS 중 하나라도 실패하면 바로 불합격이며 로컬 planner/parser fallback은 사용하지 않습니다.
>
> 에이전트의 워크플로: 업무는 “내게 전화해서 내일 치과 검진을 확인해 줘”입니다 → ReAct 계획이 정확한 시간과 확인 번호가 빠졌음을 발견합니다 → 사용자가 브라우저에서 전화를 받습니다 → 에이전트가 음성으로 질문합니다 → 사용자가 답합니다 → 에이전트가 내용을 다시 읽고 최종 확인을 요청합니다 → `complete_task`를 호출합니다 → 인터페이스가 transcript, 핵심 필드, 전송 통계를 저장합니다 → 에이전트가 사용자에게 결과를 읽어 줍니다. 이 로컬 실험은 사용자가 확인한 내용만 기록하며, 병원에서 실제 예약을 완료한 것처럼 꾸미지 않습니다.
>
> **통과 기준**: 두 그룹이 각각 실제 WebRTC 세션을 하나씩 맺고, SDP offer/answer, ICE 연결 완료, 열린 data channel, 브라우저 마이크 트랙과 서버 하향 트랙의 존재, 양방향 audio RTP packet/byte가 0보다 크다는 증거를 모두 남겨야 합니다. 서버는 마이크 RTP에서 얻은 ASR 입력과 그 hash, Whisper checkpoint 출처, 실제 모델 receipt, 완전히 전송된 TTS asset 두 개 이상도 저장해야 합니다. canonical 사용자 transcript source는 ASR만, 에이전트 transcript source는 TTS만 가능하며 data channel은 제어/자막 용도로만 사용할 수 있습니다. 통화 후에는 누락 필드 확인, 명시적 확인, 구조화된 `complete_task` 필드를 입증해야 합니다. mock, fallback, 연결음, 텍스트 입력, preflight만 실행하는 방식으로 대신할 수 없습니다. 또한 직접 그룹에는 완전한 매개변수 네 개가 필요하지만, ReAct 그룹은 불완전한 업무 설명 하나만 입력받아 실제 LLM이 누락 필드를 식별하고 통화 중에 채운다는 차이를 보여야 합니다.
>
> 이 실험은 음성 에이전트의 중요한 활용 방향을 보여 줍니다. **에이전트는 사용자가 채팅 창을 열기만 기다리는 존재가 아니라, 시작·확인·완료 상태가 명확한 실시간 세션을 능동적으로 맺을 수 있습니다.** 브라우저 WebRTC는 가장 재현하기 쉬운 ‘사용자에게 전화하기’ 경로를 다룹니다. 사용자를 대신해 외부 세계에 연락하거나 상담원을 기다리거나 IVR을 탐색해야 할 때는 같은 통화 도구 계약을 규정을 준수하는 PSTN/SIP 사업자에 연결하면 됩니다.
...
**MiniCPM-o 4.5**는 이 경로를 소비자용 또는 워크스테이션 GPU 한 장에서 로컬 실행할 수 있는 규모로 압축한다. SigLip2, Whisper-medium, CosyVoice2, Qwen3-8B를 기반으로 한 약 9B 파라미터 모델로, 텍스트·이미지·비디오·오디오를 네이티브로 입력받고 텍스트와 음성을 직접 생성한다. 여기서 유용한 실험은 또 다른 순위표를 복사하는 일이 아니라 앞서의 end-to-end 대 self-cascade 주장을 검증하는 것이다. 같은 모델이 오디오 잠재 상태에서 바로 답할 때와 오디오를 먼저 일반 텍스트로 평탄화할 때 서로 다른 방식으로 실패하는가?

> **실험 9-4 ★★: MiniCPM-o 4.5 로컬 실행 — End-to-End 대 Self-Cascade**
>
> 공개 checkpoint `openbmb/MiniCPM-o-4_5`를 revision `1f761131…`에 고정하고 96GB RTX PRO 6000 Blackwell 한 장에서 BF16으로 로컬 실행했다. 최대 할당 VRAM은 20.27GiB, 모델 로드는 6.15초였으며 외부 API 호출은 없었다. thinking mode는 의도적으로 껐다. 이 실험은 Omni 모델의 정보 보존을 측정하며, 뒤 절의 ‘말하면서 생각하기’를 측정하지 않는다.
>
> 작은 합성 WAV 네 개는 말의 내용만으로 답이 정해지는 음성 산수 두 문제와, 문장은 같지만 말하기 속도가 빠르거나 느린 두 샘플로 구성된다. **End-to-end arm**은 WAV를 MiniCPM-o에 직접 준다. **Self-cascade arm**은 같은 모델이 어조와 속도를 의도적으로 제외한 단어 전사만 만든 뒤 그 텍스트만으로 답하게 한다. 두 arm 모두 sampling을 끈다.
>
> 표 9-1 MiniCPM-o 4.5 로컬 결과(메커니즘 확인 4건이며 benchmark가 아님)
>
> | 작업 유형 | End-to-end | Self-cascade | 관찰 |
> | --- | ---: | ---: | --- |
> | 의미 산수(2) | 1/2 | 2/2 | 직접 경로는 “twelve boxes”를 8로 들었지만 명시적 전사는 올바른 12를 보존했다 |
> | 준언어적 속도(2) | 2/2 | 1/2 | 두 전사가 같은 문장이 되어 self-cascade는 빠른 샘플에도 “slow”라고 답했다 |
> | 합계 | 3/4 | 3/4 | 총점은 같지만 실패 위치는 반대 |
>
> 이 작은 실행은 질적 예측을 재현했다. 텍스트가 관련 정보를 모두 담을 때 명시적 전사가 지각 오류를 고칠 수 있지만, 답이 말하기 속도에 의존하면 일반 텍스트 병목은 증거를 되돌릴 수 없게 지운다. 두 arm 모두 75%이므로 end-to-end가 자동으로 더 정확하다는 뜻은 아니다. 로드 후 평균 전체 호출은 0.69초와 0.55초였지만, 고정된 실행 순서와 다른 출력 길이, 네 샘플뿐이라는 조건 때문에 엄밀한 지연 순위로 해석할 수 없다.
>
> 네이티브 audio-to-audio 호출은 실제 11.56초, 24kHz mono WAV도 남겼지만 12→8 지각 오류를 그대로 물려받았다. 원시 응답, 모델 전사, 단계별 시간, 입출력 hash와 승인 검사는 [`chapter9/end-to-end-speech`](../chapter9/end-to-end-speech/)에 있다.
...
> **실험 9-6 ★: 컴퓨터 사용 실행(Anthropic 참조 경로 또는 오픈 모델 경로)**
>
