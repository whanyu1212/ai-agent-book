...
이 추출 과정에는 몇 가지 핵심 특성이 있습니다. **선택성**—에이전트는 “검색에서 선택지 세 개를 반환했다”처럼 일시적인 정보는 기억하지 않고 미래에 유용한 사실만 기억합니다. **추상화**—“창가 좌석을 선호한다”는 말을 이번 항공편에만 묶지 않고 일반적인 선호로 정제합니다. **구조화**—나중에 쉽게 검색하도록 각 메모리에 선호, 제한, 계정 번호 같은 유형을 표시합니다. 다음에 사용자가 항공편을 예약할 때에는 좌석 선호나 식사 요구를 다시 물을 필요가 없습니다. 이미 메모리에 있기 때문입니다.

### 메모리 능력 평가: 3단계 프레임워크

메모리 시스템을 설계하기 전에 먼저 질문 하나에 답해야 합니다. 어떤 메모리 시스템이 “좋은” 시스템일까요? 평가 기준을 미리 세우면 뒤의 모든 설계를 비교할 공통 척도를 얻을 수 있습니다. 공개 벤치마크가 여러 개 있으며 대표적인 예가 **LoCoMo**(Long-term Conversational Memory; Maharana et al., 2024, arXiv:2402.17753)입니다. 최대 35개 세션에 걸쳐 평균 약 300라운드의 초장기 대화를 구성하고, 세 가지 작업군으로 모델의 장기 대화 메모리와 이해 능력을 시험합니다. 단일 홉·다중 홉·시간적 사고·개방형·적대적 질문으로 세분된 질의응답, 사건 요약, 멀티모달 대화 생성입니다.
...
이를 바탕으로 에이전트 시나리오에 더 적합한 3단계 평가 프레임워크를 설계하여 메모리 능력을 점진적인 수준으로 나눴습니다. 이 프레임워크는 이 장에서 반복해서 사용합니다. 뒤의 실험 3-10과 3-12에서는 검색 기법이 메모리 능력을 얼마나 높이는지 이 틀로 측정합니다.
...
>
> 이 평가 세트와 평가 스크립트는 동반 저장소의 `user-memory` 프로젝트에 포함되어 있습니다. 이 장 뒤의 실험 3-2도 같은 프로젝트를 사용합니다. 각 단계의 테스트 사례 전체 정의를 그곳에서 확인할 수 있습니다.
...
일부 에이전트는 개발자가 정의한 높은 수준의 상태 추상화인 **비즈니스 상태(Business State)**도 지원합니다. “명확화 필요”, “요청 처리 중”, “결제 대기 중”, “요청 완료”처럼 작업의 논리적 단계를 나타냅니다. 이러한 상태 추상화는 이벤트 기반 에이전트 아키텍처에서 특히 중요합니다. 4장에서 이벤트 기반 아키텍처 설계를 다룹니다.
...
**단순 노트(Simple Notes)**는 최소주의 설계를 구현합니다. 각 메모리는 “사용자 이메일: john@example.com”처럼 더 나눌 수 없는 최소한의 사실입니다. 장점은 오버헤드가 매우 작다는 것입니다. 연산은 O(1), 즉 데이터 양과 무관한 상수 시간입니다. 대신 사실 사이의 연관을 완전히 잃습니다. “TechCorp에서 선임 엔지니어로 일하며 추천 시스템 개발을 담당한다”는 내용을 “TechCorp에서 근무”, “직책은 선임 엔지니어”, “추천 시스템 담당”이라는 독립적인 사실 세 개로 나누면서 하나의 직무 안에 있던 연결이 끊어집니다. 여러 정보를 종합해야 하는 질의에서는 키워드가 겹치는 사실끼리 관련됐을 것이라고 추측하는 식의 휴리스틱 규칙으로 조각을 다시 맞춰야 합니다.

**향상된 노트(Enhanced Notes)**는 전체적인 관점을 취하여 완전한 컨텍스트를 담은 문단 하나로 각 메모리를 저장합니다. 같은 직무 정보를 “사용자는 3년 동안 TechCorp에서 선임 소프트웨어 엔지니어로 근무했으며 머신러닝을 전문으로 하고, 현재 5명으로 구성된 팀을 이끌어 추천 시스템 프로젝트를 진행하고 있다”라고 저장합니다. 서사 구조를 보존하면 의미가 완전하고 풍부하게 유지됩니다. “내 배경에 맞는 새 프로젝트를 추천해 줘”처럼 기술 수준, 리더십 경험, 기술 선호를 종합적으로 사고해야 하는 시나리오에 적합합니다.

비용은 세 가지입니다. 같은 정보가 여러 문단에 반복되는 저장 중복, 속성 하나가 바뀌어도 여러 문단을 다시 써야 하는 갱신의 복잡성, 나중의 검색을 방해할 만큼 긴 문단입니다. 마지막 비용의 이유는 간단합니다. 텍스트를 컴퓨터가 검색할 수 있는 형식으로 바꿀 때 문단이 길수록 벡터 임베딩이 핵심 의미를 포착하기 어렵습니다. 책 소개가 길어질수록 요점을 파악하기 어려운 것과 같습니다. 임베딩과 검색의 기술적인 세부 사항은 이 장의 RAG 절에서 설명합니다.
...
네 가지 방식을 비교하면 메모리 시스템 설계의 근본적인 긴장, 즉 단순성과 표현력의 상충 관계가 드러납니다. 단순 노트는 의미의 완전성을 희생해 극단적인 단순성을 얻고, 향상된 노트는 구조와 갱신 가능성을 희생해 서사의 완전성을 얻습니다. JSON 카드는 유연성을 희생해 구조를 얻고, 고급 JSON 카드는 단순성을 희생해 포괄성을 얻습니다. 절대적인 승자는 없으며 구체적인 사용 사례에 따라 선택해야 합니다. 성숙한 AI 에이전트 시스템은 여러 방식을 섞어 쓸 수 있습니다. 일시적인 정보는 단순 노트로 빠르게 기록하고, 정확한 중의성 해소와 장기 유지보수가 필요한 핵심 정보는 고급 JSON 카드로 처리합니다.
...
첫째, **통계 집계**입니다. “2025년에 해외에 몇 번 갔는가?”라는 질문을 텍스트 메모리로 처리하려면 모든 여행을 회상해 하나씩 세어야 하며 기록이 늘수록 정확도가 떨어집니다. 논문에 따르면 검색 기반 메모리는 이러한 집계 문제에서 정확도가 6~43%에 불과합니다. User as Code에서는 표현식 하나로 처리하여 거의 99%의 정확도를 달성합니다[^uac].
...
같은 여권 만료일을 저장하면서 여행 출발일과 만료일 사이에 며칠이 남았는지 계산하는 데에도 사용할 수 있습니다. LLM이 아니라 결정론적인 인터프리터가 계산하므로 사용자가 묻기 전에도 에이전트가 “여권이 곧 만료됩니다”라고 경고할 수 있습니다. 집계, 충돌 탐지, 강한 제약은 텍스트 메모리가 가장 약하고 코드가 가장 강한 영역입니다. 대신 코드를 생성하고 실행하기 위한 엔지니어링 기반이 필요합니다. 느슨하게 구조화된 기타 정보에는 코드가 이점이 없으므로 `notes` 필드에 여전히 텍스트를 저장할 자리를 둡니다.

User as Code는 메모리를 텍스트에서 실행 가능한 코드로 발전시키지만, 앞의 텍스트 형식과 마찬가지로 여전히 모델 밖의 **외부** 저장소입니다. 모델은 먼저 이를 검색한 뒤 컨텍스트 안에서 사고해야 합니다. 이 표현 스펙트럼을 더 안쪽으로 밀어 넣으면 사용자 메모리를 **모델 자체의 파라미터**에 직접 기록할 수 있으며, 여기서 두 가지 최첨단 형태가 나옵니다.

**로컬 파라미터에 쓰기: User as Engram.** 사용자 사실을 모델 가중치에 직접 쓰는 방법, 예를 들어 사용자마다 전용 LoRA를 학습하는 아이디어가 자연스럽게 떠오릅니다. 하지만 이 경로에는 이해하기 어려운 장애물이 있습니다. 사실-LoRA는 직접 질문하면 사실을 거의 완벽하게 재현하지만, 해당 사실을 **간접적으로 사고**해야 할 때에는 실패합니다. 고정된 백본 모델이 일시적으로 붙은 어댑터를 “참조하는” 방법을 배운 적이 없기 때문입니다. 달리 말해 **사실을 저장하는 일과 모델이 언제 그 사실을 가져올지 알게 하는 일은 별개**입니다. User as Engram[^engram]은 바로 이 문제를 해결합니다. LoRA를 학습하지 않고 Engram 모델의 비어 있는 **해시 N-gram 슬롯**에 사용자 사실을 정밀하게 기록합니다. 이러한 모델은 사전 학습에서 컨텍스트 인식 게이팅 메커니즘의 통제를 받아 해시 테이블 조회로 메모리를 검색하도록 학습합니다. 따라서 새로 쓴 사실도 필요할 때 자연스럽게 회상하여 “저장했지만 사용하지 못하는” 문제를 피합니다. 서로 다른 사용자의 사실은 겹치지 않는 슬롯에 들어가 서로 위에 쌓을 수 있습니다. 여러 Stable Diffusion LoRA를 연결해 조합하는 것과 비슷합니다. 사용자 사이의 간섭도 없고 백본 모델 자체를 건드리지도 않습니다.

**멀티모달: 말로 표현할 수 없는 지각 저장하기.** 지금까지 저장한 것은 모두 불연속 기호로 쓸 수 있는 사실이었습니다. 하지만 사용자 메모리에는 **지각**의 영역도 있습니다. 얼굴의 생김새, 지난주보다 오늘 더 지쳐 보이는 목소리, 시기마다 달라지는 화가의 붓질은 텍스트로 옮길 때 완전하게 보존되지 않습니다. “갈색 머리의 남성”이라고 쓰는 순간 갈색 머리의 두 사람을 구별하는 미묘한 신호를 잃습니다. Parametric Multimodal User Memory[^mmm]의 아이디어는 지각을 **지각 형태 그대로** 보존하는 것입니다. 고정된 모델에 작은 메모리 뱅크를 붙이고, 기억할 정체성마다 한 행을 사용합니다. Key는 얼굴의 ArcFace, 미술 스타일의 CLIP처럼 기성 인코더가 계산한 지각 벡터이고, Value는 `<id_11>` 같은 모델 자체 토큰의 임베딩입니다. 생성할 때 현재 지각이 Query 역할을 하여 메모리 뱅크 전체에서 어텐션을 계산하고, 텍스트를 전혀 사용하지 않은 채 출력을 일치하는 토큰 쪽으로 부드럽게 유도합니다. 새 정체성을 등록하려면 학습 없이 뱅크에 행 하나만 추가하면 됩니다. 가장 흥미로운 점은 이렇게 저장한 지각이 직접 벡터 검색의 효과와 맞먹을 뿐 아니라 **이를 능가한다**는 것입니다. 언어 모델 자체의 표현 공간에서 일치를 판단하므로 인코더 고유의 유사도보다 더 세밀하게 구별할 수 있고, 인코더에서 가장 약하고 오류가 나기 쉬운 단계를 정확히 보완합니다.

일반 텍스트에서 실행 가능한 코드, 로컬 파라미터, 연속적인 지각에 이르기까지 사용자 메모리 표현은 모델 “밖”에서 “안”으로 이어지는 스펙트럼을 이룹니다. 바깥쪽 계층은 갱신·감사·이전하기 쉽고, 안쪽 계층은 더 간결하며 그 순간의 사고가 더 빠르고 말로 담을 수 없는 지각을 표현할 수 있습니다. 안쪽으로 향하는 두 경로는 각각 7장의 파라미터 미세 조정과 9장의 멀티모달에 닿습니다. 여기서는 미리보기만 제공합니다.
...
[^engram]: 사전 학습한 Engram 모델의 해시 N-gram 슬롯에 경사 갱신 없이 사용자 사실을 정밀하게 삽입하는 설계와 평가는 Li, Bojie. *User as Engram: Internalizing Per-User Memory as Local Parametric Edits.* arXiv:2606.19172, 2026을 참조하세요.
[^mmm]: 고정된 모델에 연속 어텐션 메모리를 붙여 “말로 표현할 수 없는 지각”을 담는 방법은 Li, Bojie. *Parametric Multimodal User Memory: Storing What Captions Cannot Carry.* 2026(출간 예정)을 참조하세요.
...
실무에서는 다계층 압축 전략이 효과적입니다. 첫 번째 계층은 중요도 점수로 메모리를 거릅니다. 일반적인 중요도 점수는 네 요소를 고려합니다. 접근 빈도(자주 검색한 메모리일수록 중요), 시간 감쇠(오래된 메모리일수록 잊힐 가능성이 큼), 감정 강도(강한 감정 표시가 있는 메모리는 더 오래 유지), 정보의 고유성(중복 정보의 중요도는 낮아짐)입니다. 임계값보다 낮은 메모리는 압축하거나 삭제할 수 있다고 표시합니다. 예를 들어 3일 전에 생성되고 5번 접근했으며 강한 감정 표시가 있고 중복도 없는 메모리는 높은 중요도 점수를 받습니다. 반면 90일 전에 생성되어 한 번만 접근했고 감정 표시가 없으며 비슷한 항목이 세 개 있는 메모리는 압축 임계값 아래로 내려갈 수 있습니다.

두 번째 계층은 클러스터링합니다. 비슷한 메모리를 묶고 각 그룹의 대표 요약을 생성합니다. 예를 들어 날씨에 관한 여러 대화를 “사용자는 날씨를 자주 물으며 특히 비가 오는지에 관심이 많다”라고 압축합니다. 원래의 상세 메모리는 보조 저장소로 옮길 수 있습니다.

세 번째 계층은 추상화하고 일반화합니다. 구체적인 일화 메모리에서 일반 규칙을 추출하여 의미 또는 절차 메모리로 바꿉니다. 예를 들어 여러 쇼핑 대화에서 “가격 대비 성능이 좋은 제품을 선호하고 사용자 리뷰를 중시한다”는 사실을 배울 수 있습니다.

충돌 탐지에는 버전 관리 방식을 사용합니다. 과거 버전은 유지하고 최신 버전을 표시합니다. 현재 주소 같은 정보는 최신 버전만 남기고, 경력처럼 이력이 중요한 정보는 전체 기록을 보존합니다.

마지막으로 다른 장의 내용과 혼동하지 않도록 경계를 분명히 해야 합니다. 이 절은 메모리 **저장 계층**의 정리 알고리즘, 즉 어떤 메모리를 선택·클러스터링·추상화하고 어떤 형태로 바꿀지를 다룹니다. 2장의 컨텍스트 압축은 단일 세션 안에서 창 크기 문제를 해결하므로 두 메커니즘은 서로 다른 계층에서 작동합니다. 이 장은 지식의 저장, 색인, 검색도 담당합니다. 8장은 “온라인으로 증거를 추가하고 오프라인으로 통합한다”라는 2단계 패턴을 에이전트 행동의 진화로 일반화하여, 어떤 운영 증거가 지속적 갱신을 촉발하기에 충분한지 살펴봅니다.
...
일반적인 RAG 시스템은 두 부분으로 이루어집니다. 검색기는 지식 베이스에서 관련 조각을 찾고, 생성기(보통 LLM)는 이 조각을 컨텍스트로 삼아 답변을 생성합니다. 먼저 두 가지 예로 RAG의 작동 방식을 직관적으로 살펴본 뒤 검색기의 기술적 세부 사항을 알아보겠습니다.

**예시 1: Wikipedia 지식 베이스.** 사용자가 “양자 얽힘이란 무엇인가요?”라고 묻습니다. 기반 모델의 학습 데이터에는 최신 실험 결과가 들어 있지 않을 수 있습니다. RAG는 다음과 같이 처리합니다.
...
**예시 2: 기업 지식 베이스.** 사용자가 “구매한 상품을 환불하고 싶은데 절차가 어떻게 되나요?”라고 묻습니다.
...
일반적인 혼합 검색 파이프라인은 세 단계로 이루어지고, 단계마다 맡은 역할이 다릅니다. 첫 번째는 **병렬 검색**입니다. 시스템이 밀집 검색 엔진과 희소 검색 엔진에 질의를 동시에 보내고, 각 엔진이 후보 문서 집합을 불러옵니다. 두 번째는 두 결과 집합을 하나의 후보 풀로 합치는 **결과 융합**입니다. 두 경로의 점수를 직접 비교할 수 없다는 점이 어렵습니다. 밀집 검색의 유사도 점수(예: 이론상 −1에서 1 사이인 코사인 유사도. 실제로 정규화한 텍스트 임베딩은 보통 0에서 1 사이)와 희소 검색의 BM25 점수(0부터 수십까지 어떤 값도 가능)는 척도와 분포가 완전히 다릅니다. 융합 방식은 보통 두 가지입니다. 첫째, 각 경로의 점수를 따로 정규화한 뒤 가중합을 계산합니다. 둘째, Reciprocal Rank Fusion(RRF)은 원래 점수를 완전히 버리고 순위만 봅니다. 각 문서의 결합 점수는 결과 집합별 순위의 완화된 역수를 더한 값, 즉 score = Σ 1/(k + rank)입니다. 여기서 k는 상위 순위 간의 점수 차이를 줄이는 완화 상수로 흔히 60을 사용합니다. RRF는 단순하고 견고하지만 순위 정보만 사용하므로 원래 점수에 담긴 풍부한 관련성 신호를 버립니다. 가중 정규화 융합은 점수를 보존하지만 척도를 맞춰야 하며, 이를 조정하기가 실제로 어렵습니다. 세 번째 단계인 **신경망 재순위화**는 단지 RRF가 버린 정보를 보완하는 데 그치지 않습니다. 앞에서 어떤 융합 방식을 사용하든, 재순위화는 더 강력한 일치 패러다임으로 전환한다는 점에서 가치가 있습니다. 교차 인코더는 질의와 문서 사이를 깊고 상호작용적으로 일치시킵니다. 둘을 독립적으로 인코딩한 뒤 벡터 연산으로 비교하는 검색 단계의 이중 인코더보다 훨씬 정확합니다. 구체적으로는 융합 후보 풀의 상위 N개(예: 50개)를 하나씩 평가하여 최종 순위를 만듭니다. 재순위화는 융합을 **대체하지 않습니다**. 융합은 두 결과 집합을 하나의 후보 풀로 만들고, 재순위화는 그 풀 안의 순서를 정교하게 다듬습니다. 전자가 없으면 후자는 어떤 문서에 점수를 매겨야 할지조차 알 수 없습니다.
...
지금까지 검색한 것은 모두 일반 텍스트였습니다. 현실의 지식은 이보다 훨씬 다양한 형태로 존재합니다.

### 멀티모달 정보 추출: 텍스트의 경계를 넘어서

지식 베이스 파이프라인에서 멀티모달 정보 추출은 맨 앞단인 **수집 및 색인** 단계에 있습니다. 비텍스트 콘텐츠가 어떤 형태로 지식 베이스에 들어가는지를 결정하며, 따라서 이후의 청킹과 임베딩, 검색이 활용할 수 있는 정보량도 좌우합니다. 지식은 텍스트에만 있지 않습니다. 차트, PDF 레이아웃, 음성도 모두 처리해야 합니다. 아키텍처 관점에서는 세 가지 경로가 있으며, 핵심 절충점은 충실도와 비용입니다.

#### 네이티브 멀티모달 처리: 통합 의미 공간

**네이티브 멀티모달 처리**의 핵심 기술적 혁신은 특화된 인코더를 통해 서로 다른 데이터 유형을 하나의 고차원 의미 공간에 대응시키는 것입니다. 공개된 아키텍처를 사용하는 멀티모달 모델(Qwen-VL, LLaVA 등)은 이미지 처리를 위해 대개 **Vision Transformer**(ViT) 기반 시각 인코더를 통합합니다. 간단히 말하면 “이미지를 작은 패치로 잘라 ‘시각 단어’처럼 취급한 뒤 Transformer로 처리”합니다. GPT-4o와 Gemini 같은 폐쇄형 모델의 구체적인 아키텍처는 공개되지 않았지만 일반적으로 비슷한 방식을 따르는 것으로 여겨집니다. 구체적으로 ViT는 이미지를 고정 크기 패치로 나누고 문장의 단어를 처리하듯 각 패치를 벡터로 직렬화합니다. 그러면 패치와 텍스트 단어 벡터가 하나의 공유 멀티모달 임베딩 공간에 함께 놓입니다. Transformer의 자기 어텐션 메커니즘은 텍스트 토큰과 이미지 토큰을 동등하게 다루며 임의의 교차 모달 상관관계를 계산할 수 있습니다. 이러한 엔드투엔드 공동 처리는 비교하기 어려울 만큼 높은 컨텍스트 충실도를 제공합니다. 모델이 PDF의 페이지 레이아웃과 차트, 텍스트를 직접 “보면” 텍스트와 이미지 사이의 공간적·의미적 관계를 이해할 수 있으므로, 레이아웃이 복잡하고 정보 밀도가 높은 문서에 특히 적합합니다.

#### 텍스트 추출: 저비용 접근법

**텍스트 추출**은 2단계 과정입니다. 먼저 OCR 서비스나 음성 전사 서비스 같은 전문 도구로 비텍스트 콘텐츠를 일반 텍스트로 변환한 뒤 언어 모델에 입력합니다. 모듈성과 비용 효율성을 중시하는 설계 철학을 반영합니다. 어떤 멀티모달 작업도 일반 텍스트 작업으로 바꾸므로 모든 언어 모델과 호환되고, 추출한 텍스트를 캐시하여 재사용할 수 있습니다. 대가는 컨텍스트 손실입니다. 추출 과정에서 레이아웃, 차트, 이미지 정보가 모두 사라집니다.

#### 도구 기반 분석: 필요할 때 심층 분석

**멀티모달 분석을 도구로 취급하는 방식**은 혼합 접근법입니다. 먼저 텍스트를 추출해 에이전트에 기본 텍스트 요약을 제공하면서, 원본 파일을 심층 분석하는 도구(예: `analyze_image`, `analyze_pdf`)도 에이전트에 부여합니다. 이 “필요할 때 심층 분석” 전략은 초기 처리 비용을 낮게 유지하면서도 심층 분석 시에는 높은 충실도를 확보합니다.

> **실험 3-7 ★★: 멀티모달 정보 추출: 세 가지 기술 패러다임 비교**
>
> `multimodal-agent` 프로젝트는 하나의 프레임워크 안에서 세 전략을 체계적으로 비교하고 평가합니다. `demo.py`를 사용해 동일한 멀티모달 파일(예: 차트가 포함된 PDF 보고서)과 동일한 질문을 세 모드에 입력하고 성능 차이를 관찰합니다.
>
> 실험 결과는 세 전략의 절충 관계를 분명히 보여 줍니다. **네이티브 멀티모달 모드**는 시각적·공간적 정보를 깊이 이해하므로 차트 분석과 문서 레이아웃 이해 같은 작업에서 가장 뛰어납니다. **텍스트 추출 모드**는 일반 텍스트 중심 문서에 가장 비용 효율적이지만 시각 정보가 필요한 질의에는 전혀 대응하지 못합니다. **도구 기반 모드**는 상호작용 시나리오에서 유연합니다. 초기 질의 대부분을 낮은 비용으로 처리하고 필요하면 도구 호출을 통해 비용이 큰 심층 분석을 수행합니다. 다만 단 한 번의 엔드투엔드 처리로 깊이 이해해야 하는 시나리오에서는 네이티브 모드보다 성능이 떨어집니다.
>
> 각 전략이 유리한 곳이 다르므로 보편적인 정답은 없습니다. `multimodal-agent`의 가치는 절충 관계를 추측의 영역에 두지 않고 직접 측정할 수 있게 한다는 데 있습니다.
...
이어서 여섯 가지 주제를 다룹니다. 엄격한 단계 구조는 아니며, 각 주제는 서로 다른 관점에서 지식 구성과 검색을 다룹니다. 지식을 어떻게 구성할지 다루는 두 가지 **구조화 색인** 기술(RAPTOR와 GraphRAG), 가벼운 지식 관리 접근법인 OpenViking의 **파일 시스템 패러다임**, 만료되어 갱신과 정리가 필요한 지식을 위한 **지식 베이스의 적시성과 거버넌스**, 에이전트가 검색 전략을 직접 선택하는 **Agentic RAG**, Agentic RAG보다 상위 계층이 아니라 가장 기본적인 연결 고리인 청킹으로 되돌아가 각 조각 자체의 검색 가능성을 높이는 **Contextual Retrieval**, 마지막으로 **구조화 데이터 세트**에서 심층 지식을 추출하는 방법입니다.
...
**다중 홉 관계 사고**는 지식 그래프에서 가장 대체하기 어려운 능력입니다. 사용자가 “내 의사가 근무하는 병원의 주소는 어디인가요?”라고 물으면 시스템은 “사용자 → 의사 → 병원 → 주소”라는 관계 사슬을 차례로 풀어야 합니다. 평면적인 메모리 저장소에서 이런 다중 홉 질의는 독립 검색을 여러 번 수행한 뒤 LLM으로 이어 붙여야 하거나(비효율적이고 사슬이 끊기기 쉬움) 아예 표현할 수 없습니다. 지식 그래프의 그래프 구조는 관계 간선을 따라가는 일을 자연스럽게 지원하므로 이런 질의를 효율적이고 안정적으로 처리합니다.

**엔터티 명확화**도 지식 그래프의 장점입니다. 이는 앞의 밀집 임베딩 절에서 다룬 “다의어”와는 다릅니다. 문장에서 `bank`가 강둑인지 금융 기관인지를 판단하는 것은 단어 의미 명확화 문제로, 컨텍스트 인식 임베딩으로 해결할 수 있습니다. 반면 현실에 동명이인인 “Dr. Zhang” 두 명을 구분하는 것이 엔터티 명확화이며, 엔터티 자체에 관한 지식을 유지해야 합니다. “네 가지 저장 형식” 절의 “고급 JSON 카드”에서 `person`과 `relationship` 같은 수동 설계 필드로 한 사용자의 여러 “Dr. Zhang” 연락처를 구분했던 것을 기억하나요? 지식 그래프에서는 그래프 구조 자체가 명확화 능력을 갖습니다. (Dr. Zhang-A, Department, Dentistry)와 (Dr. Zhang-B, Department, Cardiology)는 서로 다른 노드이며, 각 관계 간선을 통해 서로 다른 사람과 기관에 연결됩니다. 명확화 과정에 추가적인 사고가 필요하지 않습니다.
...
> **실험 3-8 ★★★: 구조화 색인: RAPTOR와 GraphRAG의 지식 구성 철학**
>
...
**구조화 색인은 언제 필요할까요?** 모든 시나리오에 RAPTOR나 GraphRAG가 필요한 것은 아닙니다. 앞서 소개한 혼합 검색 방식(밀집 + 희소 + 재순위화)만으로도 대부분의 요구를 충족할 수 있습니다. 판단 기준은 간단합니다. 질의가 주로 “이 정보를 포함한 문서 조각 찾기”(예: “환불 정책은 무엇인가요?”)라면 혼합 검색으로 충분합니다. **문서 간 종합**(예: “CPU의 SSE와 AVX 명령어 집합은 아키텍처상 어떻게 다른가요?”)이나 **다단계 탐색**(예: “전체 아키텍처에서 구체적인 명령어까지 파고들기”)이 자주 필요하다면 구조화 색인에 투자할 가치가 있습니다. 대신 색인을 구축할 때 LLM 호출 횟수, 시간, 비용이 크게 늘어나므로 더 단순한 선택지로 부족할 때만 도입해야 합니다.
...
전문 데이터베이스가 아니라 Markdown 일반 텍스트를 지식의 기반 표현으로 선택한 것은 언뜻 직관에 어긋나 보이지만 신중히 내린 엔지니어링 결정입니다. 5장에서는 오픈 소스 에이전트 프레임워크 OpenClaw의 비슷한 선택을 자세히 다룹니다. 일반 텍스트를 사용하면 사용자가 에이전트의 지식을 직접 읽고 편집하며 바로잡을 수 있고, Git으로 버전을 관리하고 되돌릴 수 있습니다. 더 중요한 점은 에이전트가 `write_file` 기능을 얻으면 지식을 스스로 기록하고 정리할 수 있다는 것입니다. 세션이 끝날 때 시스템은 사용자 선호의 갱신 내용을 `user/memories/`에, 작업 기록을 `agent/memories/`에 쓸 수 있습니다. 전자는 이 장에서 다루는 사용자 지식 관리에 속합니다. 후자는 결과 평가와 여러 궤적에 걸친 일반화, 이후의 검증을 거쳐야만 8장에서 말하는 경험 학습이 됩니다. 임의의 단일 작업을 곧바로 믿을 만한 경험으로 취급해서는 안 됩니다.

하지만 이러한 일반 텍스트와 파일 시스템식 구성에는 간과하기 쉽지만 검색의 성패를 직접 좌우하는 전제가 있습니다. **파일 사이에 링크와 색인을 구축해야 합니다.** 앞서 소개한 `.abstract`와 `.overview`는 세로 방향의 계층형 요약을 해결합니다. 여기서 강조하는 것은 가로 방향의 연관성입니다. 지식을 서로 독립적인 텍스트 파일로만 나누어 디렉터리에 평면적으로 늘어놓고 상호 참조를 만들지 않으면, 에이전트는 모든 파일을 차례로 훑거나 벡터 검색을 사용하는 것 외에는 관련 항목 사이를 탐색할 방법이 거의 없습니다. 지식이 늘어날수록 흩어진 파일 더미를 검색하기는 오히려 어려워집니다. 올바른 방법은 Wikipedia처럼 지식 베이스를 구성하는 것입니다. 한 항목에서 다른 항목을 언급할 때마다 링크를 걸고, 진입 페이지와 색인 페이지를 덧붙여 에이전트가 한 개념에서 이웃 개념으로 링크를 따라가게 합니다. 가벼운 파일 링크만으로 GraphRAG의 엔터티 관계 그래프가 제공하는 탐색 능력 일부를 구현하는 셈입니다. 여기에는 실무상 중요한 차이도 있습니다. **모델마다 이런 링크를 능동적으로 만들고 관리하는 신뢰성이 다릅니다.** 더 강력한 모델은 새 지식을 쓸 때 기존 항목을 자발적으로 다시 참조하고 색인을 관리하지만, 많은 모델은 그러지 않고 고립된 파일만 추가합니다. 따라서 지식 작성 프롬프트에 요구 사항을 명시해야 합니다. 새 항목을 추가할 때마다 먼저 관련 기존 항목을 검색하여 링크하고, 소속 디렉터리의 색인 페이지를 갱신하여 양방향으로 도달할 수 있는 참조 네트워크를 만들어야 합니다. 지식이 서로 연결되지 않은 섬으로 퇴화하게 두어서는 안 됩니다.

### 지식 베이스의 적시성과 거버넌스

앞 절에서는 “지식을 잘 구성하고 정확히 검색하는 방법”을 다뤘습니다. 하지만 지식 베이스를 실제로 운영하면 간과하기 쉬우면서도 신뢰성에 직접 영향을 주는 또 다른 문제가 생깁니다. 지식은 만료되고 콘텐츠는 효력을 잃으며, 여러 사용자가 공유해야 할 때도 많습니다. 이는 지식 베이스의 **거버넌스**에 해당하므로 따로 짚어 볼 필요가 있습니다.

**지식 만료와 증분 갱신.** 지식 베이스는 한 번 구축하고 방치하는 정적 자산이 아닙니다. 회사 정책은 개정되고 법규는 갱신되며 문서는 교체됩니다. 이상적으로는 문서 하나를 추가하거나 수정할 때 전체 저장소를 다시 구축하지 않고 색인만 증분 갱신해야 합니다. 이때 색인 구조의 선택이 실질적인 결과를 낳습니다. 실험 3-4에서 ANNOY와 HNSW를 비교한 내용을 떠올려 보겠습니다. 트리 기반인 ANNOY는 증분 삽입을 지원하지 않아 문서를 추가할 때 색인 전체를 다시 구축해야 하므로 내용이 거의 바뀌지 않는 정적 저장소에 적합합니다. 그래프 기반인 HNSW는 새 벡터의 증분 삽입을 기본으로 지원하여 새로운 지식을 계속 받아들여야 하는 동적 시나리오에 더 알맞습니다. 자주 갱신하는 지식 베이스에 잘못된 색인 구조를 선택하면 재구축 부담이 운영 비용을 압도합니다.

**효력을 잃은 콘텐츠의 탐지와 폐기.** 만료된 콘텐츠를 삭제하기만 하면 끝나는 것은 아닙니다. 새 버전으로 대체된 과거 정책이 저장소에 남으면 검색할 때 새 버전과 함께 불려 와 모델이 서로 모순되거나 낡은 답변을 내놓을 수 있습니다. 프로덕션 시스템은 일반적으로 각 조각에 버전 번호와 발효일·만료일 같은 메타데이터를 붙이고 검색 단계에서 만료된 콘텐츠를 제외하거나, 요약에 “이 항목은 [날짜]에 폐기됨”이라고 명시합니다. 앞서 사용자 메모리에서 소개한 버전 기반 충돌 탐지를 공유 지식 베이스 규모로 확대한 것과 같은 발상입니다.

**다중 사용자 공유: 권한과 테넌트 격리.** 지식 베이스를 모든 사용자가 공유한다고 해서 “모든 콘텐츠를 누구나 볼 수 있다”는 뜻은 아닙니다. 부서나 테넌트, 권한 수준이 다른 사용자는 서로 다른 문서 집합에 접근할 때가 많습니다. 핵심 원칙은 **호출자의 권한에 따라 검색 단계에서 필터링**하여 권한 없는 문서가 사용자의 컨텍스트에 절대 들어가지 않게 하는 것입니다. 문서를 불러와 컨텍스트에 주입한 뒤 검토 단계를 추가하는 대신 권한 필터링을 검색 계층으로 내리는 일이 특히 중요합니다. 민감한 콘텐츠가 LLM의 컨텍스트에 한 번 들어가면 어떤 형태로든 최종 답변에 유출되지 않는다고 보장하기 어렵습니다. 다중 테넌트 시스템은 테넌트별 벡터 색인과 메타데이터도 격리하여 한 테넌트의 질의가 다른 테넌트의 비공개 지식을 “교차 오염”하여 검색하지 않도록 해야 합니다.
...
이제 기본 검색에서 구조화 색인, Agentic RAG에 이르는 완전한 기술 스택을 살펴봤습니다. 이 장 앞부분에서 남겨 둔 문제를 떠올려 보겠습니다. 사용자 메모리가 수천, 수만 개로 늘어났을 때 관련 있는 몇 개를 어떻게 정확히 찾고, 서로 모순되는 기록을 어떻게 구별할까요? 이제 이러한 지식 베이스 기술을 **다시 사용자 메모리로 돌려** 이 장의 처음에서 다룬 문제에 적용합니다. 이어지는 실험 3-10과 실험 3-12에서는 이 장의 첫머리에서 세운 3단계 평가 프레임워크와 실험 3-1의 평가 세트를 그대로 사용하여, 이 기술이 사용자 메모리 검색의 정확성과 충돌 문제를 단계별로 해결할 수 있는지 검증합니다.
...
이러한 한계의 근본 원인은 전통적인 청킹 방식에 내재한 결함입니다. 다음 절에서는 문제를 근본적으로 해결하는 컨텍스트 인식 검색을 소개하고, 실험 3-12에서 사용자 메모리 시나리오에 적용합니다.
...
>
> 성능 향상의 대가는 색인 단계에서 추가로 호출하는 LLM입니다. 하지만 프롬프트 캐싱을 사용하면 충분히 제어할 수 있습니다. 2장에서 소개한 요청 간 캐싱 메커니즘은 같은 프롬프트 접두사를 반복 호출할 때 원래 비용의 약 1/10만 사용하여, 문서 토큰 100만 개당 약 1달러까지 비용을 낮춥니다. Anthropic 연구에 따르면 이 기술을 BM25와 결합하면 검색 실패율(앞의 “검색 품질은 어떻게 측정할까요?”에서 말한 상위 20개 미적중률, 1 − recall@20)을 49% 낮추고, 재순위 모델까지 결합하면 67% 낮춥니다. 이 실험은 프로덕션급 RAG를 구축할 때 더 지능적인 컨텍스트 인식 지식 전처리에 투자하는 일이 매우 큰 수익을 내는 엔지니어링 결정임을 분명히 보여 줍니다.

위에서는 문서 지식 베이스에 컨텍스트 인식 검색을 적용한 효과를 검증했습니다. 같은 기술을 사용자 메모리 시나리오에 적용하면 다음 실험으로 이어집니다.

> **실험 3-12 ★★★: 컨텍스트 인식 검색으로 사용자 메모리 강화**
>
> 사용자 메모리에 컨텍스트 인식 검색을 적용하면 대화 기록 청킹의 문제를 직접 해결할 수 있습니다. “좋아요, 이걸로 예약해 주세요”라는 고립된 문장에는 아무런 정보가 없습니다. 앞의 컨텍스트가 “Shanghai에서 Seattle까지 500달러짜리 편도 항공권”이었다는 사실을 알아야 의미가 생깁니다. 이 실험은 실험 3-10의 프레임워크를 바탕으로 대화 기록을 색인하기 전에 중요한 “컨텍스트 생성” 단계를 추가합니다. 각 대화 조각에 대해 LLM을 호출하여 핵심 배경 정보를 담은 접두 요약을 만듭니다.
>
...
RAG는 “기존 문서를 어떻게 검색할 것인가”라는 문제를 해결합니다. 하지만 현실에서는 가치 있는 지식이 문서 형태로 존재하지 않고 구조화 데이터의 통계적 패턴에 숨어 있는 경우가 많습니다. 이 절에서는 RAG를 보완하기 위해 데이터 세트에서 이러한 암묵지를 발굴하는 방법을 소개합니다.
...
> **실험 3-13 ★★★: 구조화 데이터에서 암묵지 추출: 사법 판례 분석 사례**
>
...
**지식 획득**의 핵심 기술 스택은 다음과 같습니다. 문서 청킹으로 검색 단위를 정하고, 밀집 임베딩으로 의미를 포착하며, 희소 임베딩으로 키워드를 일치시킵니다. 결과 융합으로 후보를 하나의 풀에 모으고, 신경망 재순위화로 최종 순서를 다듬으며, recall@k 같은 지표로 검색 품질을 측정합니다. 멀티모달 추출은 시스템의 범위를 일반 텍스트에서 차트와 문서 레이아웃까지 넓힙니다.
...
3. ★★★ Agentic RAG에서는 에이전트가 언제 검색하고 무엇을 검색하며 계속 검색할지를 능동적으로 결정합니다. 그러나 모델이 자신이 무엇을 모르는지 모르면 검색을 올바르게 촉발할 수 없습니다. 이 “메타인지” 문제를 어떻게 해결할 수 있을까요?
4. ★★ 멀티모달 정보 추출은 차트를 텍스트 설명으로 변환한 뒤 검색합니다. 이 “번역” 과정에서 시각 정보의 공간 관계가 사라질 수 있습니다. 일반 텍스트 설명으로 완전히 전달할 수 없는 차트 정보의 구체적인 사례를 하나 들고, 이를 보존할 방안을 설계해 보세요.
5. ★★★ Rich Sutton의 “Bitter Lesson”은 범용적인 방법(탐색과 학습)이 궁극적으로 손으로 설계한 특성을 능가한다고 주장합니다. 이 장에서 구축한 전체 지식 시스템(청킹 전략, 색인 구조, 검색 파이프라인)도 일종의 “수작업 설계”일까요? 모델이 충분히 강력해지면 이러한 설계를 단순히 “전부 입력하기”로 대체할 수 있을까요?
6. ★★★ 모델의 능력이 향상되어도 도메인별 지식 베이스가 여전히 중요할까요? 미래의 강력한 기반 모델이 특정 도메인의 지식 베이스에 있는 정보를 모두 포함하여 지식 베이스가 필요 없어질 가능성이 있을까요?
7. ★ RAPTOR는 상향식 계층 요약으로 트리 색인을 구축하고, GraphRAG는 엔터티 관계로 그래프 구조 색인을 구축합니다. 두 가지 구조화 색인은 각각 어떤 유형의 질의에 강할까요?
8. ★★ 파일 시스템 패러다임은 지식을 파일 시스템과 비슷한 계층 구조로 구성합니다. 전통적인 벡터 데이터베이스 RAG와 비교했을 때 어떤 시나리오에서 이 접근법이 유리할까요?
9. ★★★ 구조화 데이터(예: 사법 판결 데이터베이스)에서 “판단 요인”과 “요인 중요도 계층”을 자동으로 발견하는 것은 본질적으로 에이전트가 데이터에서 규칙을 귀납하는 일입니다. 이러한 데이터 기반 지식 추출이 인간 전문가가 직접 작성한 규칙과 같은 품질에 도달할 수 있을까요?
