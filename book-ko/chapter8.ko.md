...
![그림 8-1 에이전트의 지속적 진화 전체 루프](images/fig8-1.svg)
...
>
> **대조군과 지표:** 기준 조건은 종합 점수만 출력하고 실험 조건은 차원마다 `pass`, `fail`, `uncertain`과 증거 및 신뢰도를 함께 출력합니다. 보정 과정에서는 각 차원의 실패 탐지 정밀도와 재현율을 측정하고 전문가 라벨과 정확히 일치한 비율을 보고합니다. 또한 거짓 약속 같은 실패가 근거 없는 결론이 되지 않도록 비어 있지 않은 증거를 포함하는지 확인합니다.
>
> **통과 기준:** 검증기는 중대한 위반, 거짓 약속, 과도한 거절을 안정적으로 탐지해야 합니다. 종합 점수가 높더라도 개인정보나 정책 실패를 숨겨서는 안 됩니다. 신뢰도가 낮거나 위험도가 높은 사례는 자동으로 학습 신호가 되는 대신 두 번째 검증기나 사람의 검토로 보내야 합니다.
>
> 부록 구현은 [`trajectory-verifier`](../chapter8/trajectory-verifier/)에서 확인할 수 있습니다. 기본값은 오프라인에서 재현할 수 있는 품질 판정자를 사용하며, 구현된 실제 LLM 검증기를 실행하려면 `--judge llm`을 사용하십시오.
...
![그림 8-3 지속적 진화를 위한 네 가지 업데이트 방법](images/fig8-3.svg)
...
![그림 8-4 평가된 궤적에서 경험 지식 문서로](images/fig8-4.svg)
...
>
> 부록 구현은 [`gaia-experience`](../chapter8/gaia-experience/)에서 확인할 수 있습니다. `demo_documents.py`는 기본적으로 오프라인에서 실행되며, `--extractor llm`을 사용하면 실제 LLM이 여러 궤적에 기반한 경험 후보를 제안할 수 있습니다.
...
>
> 부록 구현은 [`prompt-auto-optimization`](../chapter8/prompt-auto-optimization/)에서 확인할 수 있습니다. 오프라인 테스트가 진단과 릴리스 게이트를 다루며, `--quick`은 업무 에이전트, LLM 판정자, 코딩 에이전트를 실제로 호출합니다.
...
>
> 부록 구현은 [`browser-use-rpa`](../chapter8/browser-use-rpa/)에서 확인할 수 있으며 결정론적 상태 머신 데모와 실제 브라우저 에이전트를 호출하는 실행 경로를 모두 제공합니다.
...
>
> 부록 구현은 [`self-modifying-agent`](../chapter8/self-modifying-agent/)에서 확인할 수 있습니다. 결정론적 후보 생성기와 실제 LLM 코딩 에이전트 중 하나를 사용할 수 있으며 두 경로는 같은 릴리스 게이트를 공유합니다.
...
> **실험 8-6 ★★★: Hermes에게 이 책을 주면 스스로 업그레이드할 수 있을까?**
>
...
![그림 8-5 온라인 실행과 오프라인 진화를 위한 이중 루프](images/fig8-5.svg)
...
더 구체적으로 Voyager는 맞물려 작동하는 세 메커니즘으로 구성됩니다. **자동 커리큘럼 생성기**는 현재 인벤토리, 환경, 습득한 스킬을 바탕으로 적절히 도전적인 다음 목표를 제안하여 탐색이 무작위 방황이 되지 않게 합니다. **스킬 라이브러리**는 성공한 프로그램을 검색하고 조합할 수 있는 코드로 저장합니다. 예를 들어 고급 채집 스킬은 기본 이동·제작 스킬을 호출할 수 있습니다. **반복 프롬프팅 메커니즘**은 업무가 실제로 통과할 때까지 환경 관찰, 실행 오류, 자기 검증 결과를 다음 코드 생성 라운드에 되돌려 줍니다. 논문의 기준 모델과 비교하여 Voyager는 고유 아이템을 3.3배 더 많이 얻고 2.3배 더 멀리 이동했으며 핵심 기술 트리 이정표를 최대 15.3배 빠르게 해제했고, 스킬 라이브러리를 새로운 Minecraft 세계에 전이했습니다. 이 지표는 고정된 에이전트가 한 번의 시험에서 얼마나 잘하는지가 아니라 경험에 따라 능력이 어떻게 성장하는지를 측정합니다.
...
Hermes는 백그라운드 진화의 더 완전한 사례를 제공합니다. 장기 정보를 용량이 제한된 `MEMORY.md`와 `USER.md` 파일, 이전 세션에 대한 SQLite/FTS5 검색, 주문형 스킬, Honcho 같은 선택적 외부 메모리 제공자로 분리합니다. 세션 검색은 먼저 LLM으로 요약하는 대신 원래 메시지를 반환하므로 검색과 생성을 분리하고 감사 가능성을 유지합니다. 업무에 도구 호출이 많거나 오류·막다른 길에서 복구했거나 사용자 수정을 받았거나 자명하지 않은 워크플로를 발견하면 백그라운드 검토가 스킬을 만들거나 국소 수정할 수 있습니다. 메모리와 스킬 쓰기에 승인 게이트를 적용할 수도 있습니다. 별도의 Curator가 스킬 사용량, 오래됨, 보관 상태를 추적하고 유휴 시간에 결정론적으로 정리하며, 선택적으로 LLM을 호출해 내용을 병합할 수 있습니다. 먼저 변경 스냅샷을 만들어 잘못된 통합을 롤백할 수 있게 합니다[^hermes-memory]. 이로써 ‘기록-통합-검증-정리’가 비유를 넘어 실제 운영 가능한 능력 생명 주기가 됩니다.
...
>
> 부록 구현은 [`self-evolution-eval`](../chapter8/self-evolution-eval/)에서 확인할 수 있습니다. 기본값은 업데이트 가능, 추가 전용, 정적이라는 세 참조 에이전트를 비교합니다. 실제 LLM이 같은 장기 업무 스트림을 수행하게 하려면 `--profile llm`을 사용하십시오.
