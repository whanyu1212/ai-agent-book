...
>
> Werewolf는 플레이어의 사고, 기만, 사회적 전략을 시험하는 고전적인 사회적 추론 게임입니다. 이 실험에서는 AI 에이전트와 사람이 실시간 음성 상호 작용을 통해 함께 Werewolf를 플레이하는 멀티 에이전트 시스템을 만듭니다. 에이전트의 사고, 역할 수행, 실시간 상호 작용 역량을 시험합니다.
>
> **아키텍처 설계**:
>
> **1. 게임 상태 관리**: 심판(LLM이 아니라 코드로 구동)이 중앙 상태를 유지합니다. 플레이어 목록(사람 + AI 혼합), 정체, 진영, 생존 상태, 게임 단계(Night/Day/Vote/Resolution), 과거 사건 기록이 포함됩니다.
>
...
>
> **3. 실시간 음성 상호 작용**: 9장의 실시간 음성 에이전트를 사람 플레이어와 AI 에이전트 사이의 통신 기반으로 사용합니다. 낮 토론에서는 심판이 발화 순서를 제어합니다. 플레이어는 자리 순서대로 발언하거나 발언권을 요청할 수 있습니다. 투표할 때 심판은 사람이 말했든 AI 에이전트가 생성했든 각 플레이어의 표를 모아 집계하고 누가 탈락하는지 발표합니다.
>
> **4. 에이전트 사고와 전략**:
>
...
>
> **인수 조건**:
> - 6~8명(사람 플레이어 1명 + AI 에이전트 5~7개)의 게임을 구성합니다.
> - 역할 구성: 늑대인간 2명, 예언자 1명, 마녀 1명, 나머지는 주민이며 사람 플레이어에게 무작위로 역할을 배정합니다.
