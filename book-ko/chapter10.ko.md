...
- **도구 호출 매개변수:** 상위 에이전트가 하위 에이전트의 도구 매개변수로 구조화된 데이터를 전달합니다. 타입과 구조가 명확한 데이터가 필요한 상황에 적합합니다.
...
두 IPC 패러다임에 대응시키면 공유 파일 시스템은 ‘공유 메모리’에, 도구 호출 매개변수와 메시지 버스는 ‘메시지 전달’에 해당합니다. 도구 매개변수는 호출과 함께 동기적으로 전달되고 버스 메시지는 중개자를 통해 비동기로 전달됩니다. 각 패러다임에는 절충이 있습니다. Go에는 “메모리를 공유하며 통신하지 말고, 통신하여 메모리를 공유하라”는 유명한 격언이 있습니다. 공유 메모리는 빠르지만 개발자가 동시성 위험을 관리해야 합니다. 메시지 전달에는 더 많은 오케스트레이션 코드가 필요하지만 데이터 소유권이 명확하고 추적 가능합니다. 이 절충은 뒤의 상태 질의와 동시성 충돌 논의에서 계속 등장합니다.
...
운영체제에 익숙한 독자에게는 공유 컨텍스트 에이전트가 스레드, 비공유 컨텍스트 에이전트가 프로세스와 비슷하다는 비유가 유용할 수 있습니다. 스레드는 주소 공간을 공유하므로 전환과 통신 비용이 작지만 격리가 약하며 한 스레드의 메모리 손상으로 전체 프로세스가 중단될 수 있습니다. 프로세스는 각자 주소 공간을 가져 격리와 안전한 병렬성이 더 강하지만 명시적인 IPC로 통신해야 합니다. 표 10-1의 기준은 이러한 절충에서 나옵니다.

표 10-1은 하위 업무 수, 컨텍스트 창, 병렬성, 정보 격리, 비용 예산이라는 다섯 관점에서 두 아키텍처의 선택 기준을 요약합니다. 초기 아키텍처를 선택할 때 체크리스트로 사용할 수 있습니다.

표 10-1 공유 컨텍스트와 비공유 컨텍스트 선택 기준

| 선택 기준 | 공유 컨텍스트 | 비공유 컨텍스트 |
|---------------|-----------------------------------|--------------------------------------------|
| 하위 업무 수 | 적음(역할 2~3개) | 많음(병렬 처리 필요) |
| 컨텍스트 창 | 모든 역할의 정보를 담을 수 있음 | 단일 창으로 부족함 |
| 병렬성 | 주로 직렬(같은 궤적에서 역할이 교대) | 대규모 병렬 확장 가능(컨텍스트가 독립적이고 서로 막지 않음) |
| 정보 격리 | 필요 없음(모든 역할이 정보 공유) | 필요함(예: 보안 검토는 다른 에이전트의 내부 컨텍스트를 받지 않아야 함) |
| 비용 예산 | 하나의 궤적을 단계마다 넘기며 토큰이 단계별로 누적 | 여러 에이전트가 독립적으로 작업하여 전체 토큰은 보통 수 배에서 열 배 많음 |
...
표 10-2는 어떤 협업 방식이 새 정보를 도입하는지 보여 주며 멀티 에이전트 협업이 단일 에이전트보다 실질적인 가치를 주는지 판단하도록 돕습니다.
...
공유 컨텍스트를 이용한 멀티 에이전트 협업에서 각 단계는 자체 시스템 프롬프트와 도구 집합을 가진 독립 에이전트이지만 앞 에이전트의 전체 궤적을 물려받습니다. 교대 근무를 이어받는 동료가 전임자가 남긴 모든 업무 기록을 펼쳐 볼 수 있는 것과 같습니다. 이러한 상속 기반 협업의 핵심 장점은 정보 손실이 전혀 없다는 것입니다. 모든 에이전트가 어느 이전 단계의 세부 사항도 검토할 수 있습니다. 문제는 물려받은 방대한 기록에 주의가 흐트러지지 않고 현재 에이전트가 자신의 책임에 집중하게 하는 것입니다.

### 다단계 역할 전환

먼저 정의에 관한 논쟁을 꺼내 놓겠습니다. 1장의 용어로 다단계 역할 전환은 **워크플로식 오케스트레이션**입니다. 실행 경로(예: 요구 사항 명확화 → 구현 → 검토)가 미리 정의되어 있습니다. 프로세스 관점에서는 하나의 프로세스가 같은 메모리를 계속 유지하며 여러 단계를 순서대로 실행합니다. 따라서 ‘진짜 멀티 에이전트가 아니다’라는 주장에도 일리가 있습니다. 그래도 이 장에서는 멀티 에이전트 패턴으로 다룹니다. 각 단계가 자체 시스템 프롬프트, 도구, 초점을 가질 수 있고 단계 경계를 품질 게이트로 사용할 수 있다는 실용적 이점이 있기 때문입니다.

복잡한 업무에서는 단계마다 에이전트의 역할과 책임이 크게 달라질 수 있습니다. 하나의 정적인 시스템 프롬프트를 처음부터 끝까지 사용하면 단계별 안내를 제공하기에 너무 일반적이거나 모든 단계의 지시를 포함해 지나치게 길어집니다. 다단계 역할 전환은 현재 단계에 따라 시스템 프롬프트와 도구 집합을 바꾸어 에이전트가 가장 적절한 역할로 일하게 합니다. 전환에 새 인스턴스나 프로세스가 필요하지는 않습니다. 같은 실행 세션 안에서 시스템 프롬프트와 도구 집합만 바꿉니다. 역할이 바뀌어도 대화 기록과 업무 상태는 공유되므로 새 역할의 에이전트도 이전 단계에서 쌓인 모든 정보에 접근할 수 있습니다.

![그림 10-2: 단계 기반 역할 전환](images/fig10-2.svg)

> **실험 10-1 ★★: 실행 단계에 따라 시스템 프롬프트 결정**
>
> 이 실험은 단계별 시스템 프롬프트가 완전한 코딩 에이전트 워크플로 전반의 성능을 높이는 방법을 보여 줍니다.
>
> **업무 상황**: 사용자가 소프트웨어 개발을 요청하면 에이전트가 요구 사항 명확화, 코드 구현, 품질 검토라는 세 단계를 거칩니다.
>
> **1단계: 요구 사항 명확화**(역할: 요구 사항 분석가)
>
> 시스템 프롬프트는 다음을 강조합니다.
> - “여러분의 책임은 사용자의 필요를 완전히 이해하는 것입니다. 모호한 부분을 명확히 하도록 질문하여 기대하는 기능, 사용 상황, 성능 요구 사항을 모두 이해했는지 확인하십시오.”
> - “서둘러 구현하지 마십시오. 이 단계에서 할 일은 코드를 작성하는 것이 아니라 질문하고 확인하는 것입니다.”
> - “모든 핵심 요구 사항이 명확하다고 확인하면 `complete_requirements_analysis()` 도구를 호출하여 이 단계를 끝내십시오.”
>
> 도구 집합은 제한되어 있습니다. 사용자에게 확인 질문을 하는 `ask_clarifying_question(question)`, 확인된 요구 사항을 기록하는 `save_requirement(key, value)`, 단계 완료를 표시하는 `complete_requirements_analysis()`입니다.
>
> 에이전트는 스크립트가 어떤 파일 유형을 처리해야 하는지, 하위 폴더를 재귀적으로 처리할지, 파일을 옮긴 뒤 원래 파일명을 보존할지 사용자에게 묻습니다. 이러한 대화로 구조화된 요구 사항을 만들고 기록합니다. 요구 사항이 충분히 명확해지면 `complete_requirements_analysis()`를 호출합니다. 이 완료 신호에 따라 시스템이 다음 단계 구성을 로드합니다.
>
> **2단계: 코드 구현**(역할: 소프트웨어 엔지니어)
>
> 새 시스템 프롬프트는 다음을 강조합니다.
> - “여러분의 책임은 확인된 요구 사항을 바탕으로 고품질 Python 코드를 작성하는 것입니다.”
> - “모범 사례를 따르십시오. 코드를 모듈화하고 오류를 적절히 처리하며 도움이 되는 곳에 주석을 포함하십시오.”
> - “코드를 완성하고 기본 테스트를 통과하면 `submit_for_review()`를 호출하여 검토 단계로 들어가십시오.”
>
> 도구도 바뀝니다. 요구 사항 명확화 도구가 `write_file(path, content)`, `read_file(path)`, `execute_code(code)` 같은 개발 도구로 교체됩니다. 에이전트는 첫 단계에서 기록한 요구 사항을 사용해 핵심 로직을 작성하고 오류 처리를 추가하고 테스트를 만듭니다. 요구 사항 세부 정보를 확인하기 위해 이전 대화를 볼 수 있지만 더 질문하지 않고 구현에만 집중합니다. 끝나면 `submit_for_review()`를 호출합니다.
>
> **3단계: 코드 검토**(역할: 코드 검토자)
>
> 새 시스템 프롬프트는 다음을 강조합니다.
> - “기능적 정확성, 코딩 표준 준수, 오류 처리, 성능, 보안 관점에서 코드를 검토하십시오.”
> - “비판적으로 접근하여 잠재적인 문제와 코드를 개선할 기회를 식별하십시오.”
> - “심각한 문제가 발견되면 `request_revision(issues)`를 호출하여 구현 단계로 돌아가 수정하게 하십시오. 품질이 적절하다면 `approve_code()`를 호출하여 업무를 완료하십시오.”
>
> 도구 집합이 다시 바뀌어 `run_linter(file)`, `run_tests(file)`, `analyze_complexity(file)` 같은 코드 품질 분석 도구로 교체됩니다. 에이전트는 검토자 관점에서 코드를 다시 살펴보고 정적 분석을 실행하고 잠재적인 버그, 성능 문제, 보안 위험을 확인합니다.
>
> 이 3단계 설계를 사용하면 에이전트가 각 단계의 핵심 업무에 집중할 수 있습니다. 더 중요한 점은 명확한 단계 전환이 모든 단계의 완료를 보장한다는 것입니다. 요구 사항 분석을 건너뛰고 바로 코딩을 시작하거나 검토 없이 결과를 전달할 수 없습니다.
>
> **실험 요구 사항**:
> 1. 각기 명확한 역할 정의와 행동 지침이 있는 3단계 시스템 프롬프트를 구현합니다.
> 2. 단계마다 맞는 도구 집합을 구성합니다.
> 3. 특정 도구 호출을 통한 단계 전환 트리거 메커니즘을 구현합니다.
> 4. 단계 간 컨텍스트 연속성을 보장합니다.
> 5. 롤백 상황, 즉 코드 검토에서 문제가 발견되면 구현 단계로 돌아가는 흐름을 처리합니다.
> 6. 서로 다른 프롬프트가 서로 다른 행동을 만드는 방식을 보여 주도록 각 단계의 활동을 기록합니다.
>

### 도메인 간 역할 전환

다단계 역할 전환은 하나의 업무 유형(소프트웨어 개발) 안에서 단계별 실행을 보여 주었습니다. 도메인 간 역할 전환은 더 나아가 업무가 여러 도메인을 거치면서 에이전트가 동적으로 역할을 바꿉니다. 미리 정의된 선형 프로세스를 따르는 대신 사용자의 변화하는 필요에 따라 채택할 전문 역할을 선택합니다.

> **실험 10-2 ★★: 다중 역할 전환**
>
> **선수 지식**: 먼저 2장의 에이전트 스킬 메커니즘을 다시 살펴보기를 권합니다.
>
> **시스템 아키텍처**: 다섯 역할을 정의합니다.
>
> - **triage(접수 담당, 기본 진입점)**: 사용자의 전반적인 필요를 파악하고 업무를 순차적인 하위 업무로 나누며 각 하위 업무를 적합한 전문가에게 전달한 뒤 모든 하위 업무가 끝나면 최종 확인을 수행합니다. 유일한 도구는 `transfer_to_agent`입니다.
> - **research(정보 검색 전문가)**: `web_search`를 사용하여 데이터, 사실, 자료를 찾습니다.
> - **coding(프로그래밍 전문가)**: `execute_python`을 사용하여 프로그래밍·스크립팅 업무의 코드를 작성하고 실행합니다.
> - **data_analysis(데이터 분석 전문가)**: `calculate` / `descriptive_stats`를 사용하여 정량 계산과 통계를 수행합니다(예: 전년 대비 성장률, 연평균 성장률(CAGR), 평균).
> - **writing(글쓰기 전문가)**: 검색한 데이터와 분석 결과를 독자에게 맞춘 명확한 초안으로 만듭니다(대략적인 길이를 확인하려면 `count_characters`를 사용할 수 있습니다).
>
> **핵심 메커니즘: transfer_to_agent 도구**
>
> 모든 역할에는 `transfer_to_agent(target_role, reason)` 도구가 있습니다. 역할이 이를 호출하면 시스템이 현재 대화 기록을 저장하고 대상 역할의 프롬프트와 도구 집합을 로드하고 기록을 해당 역할에 넘긴 뒤 실행을 재개합니다.
>
> **실험 상황**: 시스템은 기본적으로 `triage` 역할로 시작합니다. 사용자가 여러 도메인에 걸친 업무를 제출합니다. “투자자용 자료를 준비하고 있어. 2021년, 2022년, 2023년 중국의 신에너지 자동차 판매량을 찾아서 3년간 연평균 성장률을 계산하고, 투자자를 위해 120자 이내 중국어 요약을 작성해 줘.” `triage`는 ‘데이터 검색 → 지표 계산 → 초안 작성’으로 나누고 먼저 `research`에 넘깁니다.
>
> ```python
> transfer_to_agent(target_role="research", reason="2021~2023년 신에너지 자동차의 연간 판매량 찾기")
> ```
>
> `research`는 `web_search`로 판매량을 찾아 핵심 데이터를 대화에 추가한 뒤 `data_analysis`에 업무를 넘깁니다.
>
> ```python
> transfer_to_agent(target_role="data_analysis", reason="데이터 준비 완료. 2021~2023년 CAGR 계산")
> ```
>
> `data_analysis`는 `calculate`로 성장률을 계산합니다. 이어서 `writing`에 넘겨 요약 초안을 작성하고 최종 확인을 위해 `triage`에 돌려보냅니다. 전체 체인은 `triage` → `research` → `data_analysis` → `writing` → `triage`입니다. 모든 역할이 전체 대화 기록을 볼 수 있으므로 다음 역할은 이미 완료된 일을 자연스럽게 압니다.
>
> 역할 전환 결정은 시스템 프롬프트의 지침에 따릅니다. `triage` 프롬프트는 데이터나 출처 자료 찾기 → `research`, 코드 작성·실행 → `coding`, 정량 계산·통계 → `data_analysis`, 자료를 초안으로 다듬기 → `writing`이라는 라우팅 규칙을 명시합니다. 깊은 도메인 전문성이나 특수 도구가 필요한 업무는 핸드오프해야 합니다. 각 전문가의 프롬프트에도 다음으로 적합한 역할을 적거나 업무를 `triage`에 돌려보내도록 지시합니다.
>
> **실험 요구 사항**:
> 1. 적어도 세 전문 역할의 시스템 프롬프트와 전문 도구 집합을 구현합니다.
> 2. 동적 전환을 지원하는 `transfer_to_agent` 도구를 구현합니다.
> 3. 역할 전환 뒤 컨텍스트 연속성을 보장합니다.
> 4. 역할 사이를 반복해서 전환하는 순환 핸드오프를 방지합니다.
> 5. 여러 도메인에 걸친 복잡한 업무 흐름을 설계하여 역할 전환의 가치를 보여 줍니다.
>
...
앞에서는 통신 메커니즘을 프로세스 간 통신의 형태에, 공유·격리 컨텍스트를 스레드·프로세스에 비유했습니다. 이 비유를 더 확장할 수 있습니다(표 10-3).

표 10-3 멀티 에이전트 시스템과 운영체제의 대응
...
프로그램은 정적인 코드이고 프로세스는 프로그램이 실행 중인 하나의 인스턴스입니다. 마찬가지로 정적 접두부는 에이전트가 누구인지 결정하고 궤적은 어디까지 진행했는지 기록합니다. LLM은 CPU 역할을 합니다. 자체 상태를 보유하지 않고 서로 다른 컨텍스트를 로드하여 여러 에이전트가 시분할로 공유합니다. ‘컨텍스트 전환’이라는 용어 자체도 운영체제에서 빌려 왔습니다. 같은 이유로 더 빠른 CPU로 바꿔도 프로그램은 이전처럼 실행되고, 더 강한 모델로 바꿔도 에이전트는 같은 에이전트로 남습니다. 정체성과 메모리는 모델 가중치가 아니라 접두부와 궤적에 있기 때문입니다.

이 추상화는 새로운 것이 아닙니다. 비공개 상태, 비동기 메시지, 새 구성원을 만드는 능력은 정확히 1970년대 액터 모델(Actor model)의 기본 구성입니다[^actor-model]. 따라서 멀티 에이전트 시스템은 LLM 기반 액터 모델로 볼 수 있으며 운영체제와 분산 시스템에서 축적한 많은 지식을 직접 적용할 수 있습니다. 다만 한 가지 중요한 지점에서 비유가 깨집니다. 프로세스는 바이트를 비트 단위로 충실히 전달하지만 에이전트는 의미를 전달하며, 다시 말할 때마다 의미가 왜곡될 수 있습니다. 이것이 이 장의 ‘실패 모드’ 절에서 다루는 새로운 문제입니다.
...
그림 10-3은 네 영역 유형을 하나의 디렉터리 트리 아래에 통합 마운트하는 방법을 보여 줍니다. 에이전트는 통합 인터페이스로 전체 트리에 접근하고, 사용자는 공유 공간에서 파일을 업로드·다운로드하며, 외부 데이터 원본은 어댑터로 마운트되고, 내장 시스템 리소스는 읽기 전용으로 제공됩니다.

![그림 10-3: 에이전트 가상 파일 시스템의 네 영역 유형 마운트 구조](images/fig10-3.svg)

표 10-4는 네 영역 유형을 가시성, 생명 주기, 읽기/쓰기 권한, 동시성 제어라는 네 차원에서 비교하며 파일 시스템 레이아웃 설계의 체크리스트로 사용할 수 있습니다.

표 10-4 에이전트 가상 파일 시스템의 네 영역 유형
...
진행 파일은 **정지 탐지**도 가능하게 합니다. `progress.md`나 궤적 파일의 마지막 수정 시각이 N분 넘게 바뀌지 않으면 시스템은 하위 에이전트가 비활성이라고 보고 제한 시간 안전망을 작동할 수 있습니다(4장의 Heartbeat와 `monitor_shell` 메커니즘에 대응). 정지한 하위 에이전트가 전체 시스템의 발목을 잡는 일을 막습니다.
...
**III. 실행 종료.** 병렬 협업에는 ‘하나가 성공하면 나머지는 필요 없다’는 상황이 흔합니다. 여러 에이전트가 따로 검색하다 하나가 대상을 찾으면 나머지는 즉시 멈춰야 합니다(이 장 실험 10-6의 연쇄 종료). 종료에는 두 단계가 있으며 Unix 사용자라면 SIGTERM과 SIGKILL의 차이로 이해할 수 있습니다. **정상 종료**가 우선입니다. 주 에이전트가 `terminate` 신호를 보내면 하위 에이전트가 현재 단계의 안전한 지점에서 응답하고 자원을 정리하고(브라우저 세션 닫기, 대기 중인 파일 쓰기, 잠금 해제), 확인(ack)을 보낸 뒤 종료합니다. **강제 종료**는 폴백입니다. 하위 에이전트가 정상 신호에 응답하지 않을 때만 프로세스를 직접 종료하며, 해제되지 않은 자원과 불완전한 쓰기가 남을 수 있습니다. 두 가지 엔지니어링 사항에 주의해야 합니다. 첫째, 정상 종료를 위해 하위 에이전트는 루프에서 종료 신호를 주기적으로 확인해야 합니다(4장의 인터럽트 메커니즘과 비슷함). 그렇지 않으면 신호를 받을 수 없습니다. 둘째, 연쇄 종료에는 경쟁 조건이 있습니다. 여러 하위 에이전트가 거의 동시에 성공을 보고할 수 있습니다. 주 에이전트는 잠금이나 멱등 설계로 하나의 성공만 받아들이고 종료 신호를 한 번만 브로드캐스트해야 합니다. 실험 10-6의 경쟁 조건 논의를 참고하십시오.
...
동료 협업에는 보통 동등한 지위의 에이전트 2~3개가 여러 라운드에 걸쳐 서로 피드백을 제공합니다. 핵심 가치는 인지적 다양성입니다. 서로 다른 에이전트가 같은 문제를 다른 각도에서 살펴 혁신성과 견고성의 균형을 맞추고 어느 단일 에이전트보다 나은 결과를 만듭니다.
...
**토론:** 여러 에이전트가 서로 다른 입장을 가지고 대립적 대화로 문제 공간을 탐색합니다. 예를 들어 기술 해법을 평가할 때 에이전트 A는 ‘지지자’ 역할로 장점과 기회를 나열하고, 에이전트 B는 ‘반대자’ 역할로 위험과 한계를 지적합니다. 각 토론 라운드에서 상대의 주장에 반박하거나 확장합니다. 단일 에이전트가 문제를 분석하면 한 관점을 선호하고 반대 증거를 놓치는 경우가 많습니다. 구조화된 토론은 양쪽 입장을 충분히 발전시켜 의사 결정자가 더 균형 잡힌 판단을 내리게 돕습니다.
...
**브레인스토밍:** 여러 에이전트가 독립적으로 아이디어를 생성한 뒤 서로 공유하며 영감을 줍니다. 예를 들어 제품 혁신 업무에서 에이전트 1이 ‘소셜 공유 기능 추가’를 제안하면 에이전트 2가 영감을 받아 ‘소셜 네트워크 공유뿐 아니라 개인화된 공유 포스터도 생성’을 제안하고, 에이전트 3이 둘을 종합하여 ‘사용자가 포스터 템플릿을 직접 만들 수 있는 템플릿 마켓플레이스’를 제안합니다. 서로 다른 프롬프트나 모델을 사용해 에이전트마다 ‘사고 선호’가 다르므로 서로 자극하여 더 넓은 해법 공간을 탐색하고 단일 에이전트가 생각하기 어려운 창의적 조합을 찾습니다.

**패널 토론:** 여러 에이전트가 각기 특정 전문 분야의 관점을 대표하여 학제 간 문제를 함께 논의합니다. 예를 들어 신제품의 실현 가능성을 평가할 때 엔지니어 에이전트는 기술 관점에서 구현 난도를 분석하고, 제품 에이전트는 사용자 경험 관점에서 시장 매력을 평가하며, 운영 에이전트는 비용·자원 관점에서 사업 가능성을 분석합니다. 이 에이전트들은 대립하지 않고 상호 보완하며 문제의 전체 그림을 함께 맞추고 도메인 간 제약과 기회를 찾습니다.
...
‘에이전트가 서로에게 도구가 되는’ 추상화는 4장의 ‘협업 도구’ 절에서 확립했습니다. `spawn_subagent / send_message_to_subagent / cancel_subagent / list_agents` 인터페이스 설계는 여기서 관리자가 하위 에이전트를 호출할 때 그대로 적용됩니다. ‘관리자 → 하위 에이전트’ 방향으로 전달하는 내용은 뒤의 핸드오프 패키지 설계(업무 설명, 확인된 사실과 제약, 구조화된 산출물 참조)를 참고하십시오. 대응하는 질문은 하위 에이전트가 ‘하위 에이전트 → 관리자’ 방향으로 무엇을 반환하는가입니다. 답은 **전체 궤적이 아닌 구조화된 요약**입니다. 하위 에이전트는 업무 결론, 핵심 발견, 산출물의 파일 경로, 마주친 문제를 반환하고 전체 실행 궤적은 자체 로그에 남겨야 합니다. 그래야 관리자의 컨텍스트가 폭발하지 않고 하위 업무 수에 따라 천천히 선형으로 증가합니다. 아래 실험 10-3의 관리자가 번역 내용을 저장하지 않고 파일 색인만 유지하는 이유도 같습니다.
...
이는 4장의 주장과 충돌하지 않습니다. 4장은 제안 모델과 검토 모델의 능력이 비슷해야 한다고 했지만 이는 **검토 상황**에 관한 말입니다. 검토자는 검토 대상의 사고를 따라가야 결함을 찾을 수 있습니다. 검토자가 대상보다 훨씬 약하면 사고를 충분히 따라가지 못해 결함을 식별하지 못할 수 있습니다. 관리자 패턴은 다른 문제인 **계획과 실행의 분업**을 다룹니다. 계획자가 업무를 잘못 분해하면 아무리 강한 실행자도 복구할 수 없습니다. 따라서 가장 강한 모델과 가장 신중한 프롬프트를 계획자에게 먼저 줍니다. 실행자들의 능력이 균형을 이뤄야 하는지는 하위 업무가 얼마나 긴밀하게 결합되어 있는지에 달려 있습니다. 출력을 마지막에 하나로 조립해야 한다면 가장 약한 고리가 전체 품질을 끌어내리는 경우가 많습니다.
...
![그림 10-5: 관리자의 순차 조율](images/fig10-5.svg)
...
>
> 단일 에이전트를 사용하면 심각한 컨텍스트 관리 문제가 생깁니다. 에이전트가 책을 장별로 처리하면서 전체 용어집, 번역한 장, 현재 문단, 번역 작업 추적, 도구 결과가 컨텍스트에 쌓입니다. 수백 쪽 분량의 기술 서적에 이러한 중간 자료까지 더하면 컨텍스트 창을 쉽게 넘습니다. 더 중요한 점은 지나치게 긴 컨텍스트로 작업하는 에이전트가 ‘길을 잃기’ 쉽다는 것입니다. 앞의 용어 관례를 잊고 2장과 8장에서 서로 다른 번역을 사용하거나, 교정 중 중복 검사에 자원을 낭비하거나, 주의가 너무 분산되어 존재하지 않는 용어 규칙을 ‘기억’할 수도 있습니다.
>
...
![그림 10-7: 관리자의 병렬 조율](images/fig10-7.svg)
...
> 4. 예외와 오류 사례를 처리합니다.
>
> **실험 10-5 ★★★: 자율적으로 오케스트레이션되는 전화·컴퓨터 에이전트**
>
> 실험 10-4에서는 이중 에이전트 협업을 미리 설계했습니다. 이 실험은 더 나아가 사람이 계획한 흐름을 따르는 대신 에이전트 스스로 협업자를 시작할 시점을 정하는 **자율 에이전트 오케스트레이션**을 탐구합니다.
>
...
>
> **선수 지식**: 먼저 4장의 이벤트 기반·인터럽트 메커니즘을 다시 살펴보기를 권합니다.
>
> 이 실험은 정보 수집 상황에서 멀티 에이전트 병렬 실행을 적용하는 방법을 탐구합니다. 이질적인 에이전트 두 개의 협업에 초점을 둔 실험 10-4, 10-5와 달리 **동질적인 에이전트 여러 개의 병렬 검색**과 중앙 조율을 통한 효율적인 업무 완료·자원 최적화에 초점을 둡니다.
>
...
관리자 패턴은 명확한 제어 구조와 전역 가시성을 제공하지만 탈중앙화 패턴이 단순히 그 단점을 보완하는 것은 아닙니다. 중앙 제어자를 없애는 주된 동기는 인간 사회의 조직 방식을 모방하는 데 있습니다. 여러 동료 역할이 분업하고 서로 검토하며 각자 전문 관점에서 문제를 살피고 누구와 대화할지 스스로 결정하게 합니다. 모든 판단을 하나의 관리자에게 모으지 않습니다. 마이크로서비스 분야에서는 이 두 선택을 **오케스트레이션**과 **코레오그래피**라고 부릅니다. 전자는 지휘자가 모든 것을 중앙에서 조율하고 후자는 각 무용수가 스스로 들어갈 때를 감지합니다.

탈중앙화 패턴은 다른 아키텍처 접근법을 사용합니다. **단일 중앙 제어자가 없으며 에이전트가 동료로 협업합니다.** 각 에이전트가 전문적 판단에 따라 다른 에이전트에게 연락할 시점을 스스로 결정합니다. 업무를 핸드오프하거나(“제 몫은 끝났으니 이어서 해 주세요”), 피드백을 요청하거나(“이 계획은 기술적으로 가능한가요?”), 문제를 보고합니다(“전달받은 요구 사항이 모순됩니다. 다시 논의해야 합니다”).

다음 사례는 부분적인 탈중앙화에서 완전한 탈중앙화 순으로 나아갑니다. MetaGPT는 고정 파이프라인을 사용하며 통신만 탈중앙화합니다. AutoGen은 공유 대화 기록과 중앙 스케줄링을 결합합니다. OpenAI Swarm은 제어 흐름 결정을 동료 에이전트에 직접 분산합니다.

**공유 컨텍스트 없는 핸드오프에서 무엇을 전달할까요?** 그림 10-10은 두 핸드오프 유형을 대조합니다. 실험 10-2의 `transfer_to_agent`는 공유 컨텍스트를 사용하므로 새 역할이 전체 기록을 자동으로 물려받습니다. 핸드오프 체인 패턴은 컨텍스트를 공유하지 않으므로 보내는 에이전트가 받는 에이전트에 필요한 정보를 명시적으로 조립해야 합니다.

실제로 효과적인 ‘핸드오프 패키지’에는 보통 세 부분이 들어갑니다. **업무 설명**(수신자가 해야 할 일과 통과 기준), **확인된 사실과 제약**(사용자 선호, 비즈니스 규칙, 이전 단계에서 내린 결정), **구조화된 산출물 참조**(파일 내용이 아닌 경로이며 수신자가 필요할 때 읽음)입니다. 보내는 에이전트의 시행착오 과정, 중간 작업, 실패한 시도인 전체 궤적은 수신자에게 대부분 잡음이므로 의도적으로 제외합니다.

이것이 두 핸드오프 유형의 본질적인 차이입니다. 공유 컨텍스트 핸드오프는 전체 기록을 유지하여 모든 정보를 보존하지만 컨텍스트가 계속 커집니다. 공유 컨텍스트 없는 핸드오프는 정제한 패키지를 전달하여 일부 정보 손실을 받아들이는 대신 각 에이전트가 깨끗하고 집중된 컨텍스트에서 일하게 합니다. 어떤 에이전트도 다른 에이전트의 작업 과정을 이해할 필요가 없습니다. 핸드오프 패키지와 출력 산출물의 형식·의미만 알면 됩니다. 이 인터페이스 기반 협업은 계약에 의한 설계라는 소프트웨어 엔지니어링 원칙을 따릅니다.

**MetaGPT: SOP 주도 소프트웨어 회사 시뮬레이션(파이프라인에서 분리된 통신으로 가는 전환 사례).**


![그림 10-11: MetaGPT 멀티 에이전트 협업 네트워크](images/fig10-11.svg)


MetaGPT의 핵심 통찰은 소프트웨어 회사가 개발하고 다듬어 온 **표준 운영 절차**(Standard Operating Procedures, SOP)를 멀티 에이전트 시스템의 협업 프로토콜로 활용할 수 있다는 것입니다. 이 SOP를 인코딩하면 각 역할은 조립 라인의 전문 작업자처럼 표준화된 산출물을 만들고, 그 산출물이 자연스럽게 역할 간 통신 인터페이스가 됩니다.

MetaGPT에서 역할은 제품 관리자 → 아키텍트 → 프로젝트 관리자 → 엔지니어 → QA의 고정된 순서로 작업하며, 각 역할은 구조화된 산출물을 출력합니다.

- **제품 관리자 에이전트**: 요구 사항 설명을 받아 구조화된 PRD(Product Requirements Document, 기능 목록·사용자 스토리·인수 조건·우선순위 포함)를 생성합니다.
- **아키텍트 에이전트**: PRD를 읽고 아키텍처를 결정한 뒤(기술 스택 선정, 모듈 분할, 인터페이스 정의, 데이터 모델 설계) 설계 문서를 출력합니다.
- **프로젝트 관리자 에이전트**: 아키텍처 설계를 읽고 시스템을 구체적인 업무 목록과 파일 단위 할당으로 분해하며 모듈의 의존 순서를 명확히 한 뒤 엔지니어에게 업무를 배정합니다.
- **엔지니어 에이전트**: 설계 문서를 읽고 배정된 모듈을 구현하여 코드를 만듭니다. 여러 인스턴스가 병렬로 작업할 수 있습니다.
- **QA 엔지니어 에이전트**: 코드와 PRD를 읽고 테스트 사례를 생성하며 테스트를 실행하고 버그를 기록하여 테스트 보고서를 출력합니다.

MetaGPT가 탈중앙화 통신에 기여한 진정한 부분은 **공유 메시지 풀 + 역할별 구독**이라는 정보 전달 메커니즘입니다. 각 역할은 모든 역할이 볼 수 있는 풀에 구조화된 메시지를 게시합니다. 다른 역할은 일대일로 통신하는 대신 구독 설정에 따라 자신의 책임과 관련된 메시지만 소비합니다. 게시자는 자신의 출력을 누가 소비할지 알 필요가 없습니다. 역할을 추가하려면 그 역할이 구독할 메시지 유형만 선언하면 되며 기존 역할은 바꿀 필요가 없습니다. 이로써 진정한 분리가 이루어집니다. 예를 들어 제품 관리자를 더 강력한 모델로 교체해도 PRD가 계속 규격을 준수하는 한 다른 에이전트를 수정할 필요가 없습니다.

MetaGPT의 반복 개선은 주로 엔지니어링 단계에서 **실행 가능한 피드백**을 통해 이루어집니다. 엔지니어가 코드와 테스트를 실행하고 오류와 실패를 디버깅 루프의 지침으로 삼아 테스트가 통과할 때까지 계속합니다. 수정은 다른 에이전트의 의견이 아니라 결정론적인 실행 결과에 따라 이루어집니다.

분명히 말하면 MetaGPT의 **제어 흐름**은 탈중앙화되어 있지 않습니다. 역할 순서가 SOP로 미리 정해져 있으므로 전체 시스템은 조립 라인에 더 가깝습니다(1장의 용어로는 워크플로). 이 절에서 MetaGPT를 다루는 이유는 메시지 풀과 구독 통신 메커니즘이 탈중앙화 시스템의 가장 중요한 설계 요소인 분리를 보여 주기 때문입니다. “QA가 제품 관리자에게 직접 연락해 요구 사항을 명확히 하기”나 “엔지니어가 아키텍트와 대안 해법을 논의하기” 같은 다방향 동적 피드백은 이 아키텍처에서 자연스럽게 생각할 수 있는 확장이지만 원래 MetaGPT에는 구현되지 않았습니다.

**AutoGen 그룹 채팅: 공유 대화 기록 + 중앙 스케줄링.** AutoGen의 그룹 채팅에서는 여러 에이전트가 같은 대화에 참여할 수 있습니다. 매 라운드마다 ‘발화자 선택기’가 다음에 말할 에이전트를 결정합니다. 선택기는 단순한 라운드 로빈 규칙을 따르거나 지금까지의 대화를 바탕으로 어떤 에이전트가 응답하기에 가장 적합한지 LLM으로 판단할 수 있습니다. 모든 에이전트의 발언은 참여자 모두에게 공개됩니다.

제어 흐름 관점에서는 완전히 탈중앙화된 방식이 아닙니다. `GroupChatManager`가 발화자를 중앙에서 선택하며 누구의 차례인지 정하는 일 자체가 제어 흐름 결정이기 때문입니다. 따라서 더 정확한 분류는 **공유 대화 기록 + 중앙 스케줄링**입니다. 모든 에이전트가 동일한 공개 기록을 보지만 각자 독립적인 시스템 프롬프트와 도구 집합을 유지하며, 선택기가 스케줄링 권한을 가집니다.

이 모델은 계획 검토나 교차 영역 분석처럼 여러 관점의 논의가 필요하고 발화 순서를 미리 정할 수 없는 업무에 적합합니다. 다만 모든 에이전트가 계속 발언하는데도 집단이 앞으로 나아가지 않는 라이브락 형태로 대화가 표류할 수 있습니다. 따라서 명확한 종료 조건이 필수입니다. 이 장의 설계 차원에서 AutoGen은 하이브리드입니다. 스케줄링은 중앙화되어 있고 컨텍스트는 일부 공유됩니다. 이는 토폴로지와 컨텍스트 공유가 서로 독립적인 설계 차원임을 보여 줍니다.

**OpenAI Swarm과 Agents SDK: 핸드오프 네트워크.** 반대로 OpenAI의 Swarm과 후속 제품인 Agents SDK는 제어 흐름을 P2P 방식으로 탈중앙화합니다. 각 에이전트는 여러 핸드오프 선택지를 가지며 언제든 네트워크의 다른 에이전트로 제어권을 넘길 수 있습니다. 고객 서비스 분류 에이전트가 문제에 환불이 필요하다고 판단하면 환불 에이전트로 업무를 넘깁니다. 그 에이전트가 기술 장애를 발견하면 기술 지원 에이전트로 다시 넘길 수 있습니다. 중앙 스케줄러는 없습니다. 동료 에이전트 사이에서 바통처럼 제어권이 전달되고 각 에이전트가 스스로 라우팅 결정을 내립니다. 이는 그림 10-10 핸드오프 체인 패턴의 엔지니어링 구현입니다. 위험은 순환입니다. A가 B에게 넘기고 B가 다시 A에게 넘기면 업무가 무한히 맴돕니다. 이를 끊으려면 최대 핸드오프 횟수 같은 보호 장치가 필요합니다.

> **용어: Agent Swarm.** 2025년 이후 ‘에이전트 스웜(Agent Swarm)’은 각 벤더의 인기 키워드가 되었지만 단일 아키텍처를 가리키지는 않습니다. 업계의 사용법은 크게 두 가지입니다. 첫째, OpenAI Swarm식 핸드오프 네트워크(LangGraph의 swarm 라이브러리와 Microsoft Agent Framework의 핸드오프 오케스트레이션도 같은 부류)로 이 절의 탈중앙화 패턴에 해당합니다. 둘째, 일부 주류 상용 제품의 Agent Swarm은 규모화된 관리자 패턴입니다. Kimi K2.5가 처음 선보인 Agent Swarm은 주 에이전트가 100개가 넘는 하위 에이전트를 동적으로 생성해 병렬로 실행하며, ‘언제 나눌지, 몇 개로 나눌지’라는 오케스트레이션 결정을 병렬 에이전트 강화 학습으로 모델에 직접 학습시킵니다. Kimi K3는 이를 독립 모델 등급으로 이어 갔고 병렬 에이전트 훈련 샌드박스 AgentEnv[^ch10-kimi-swarm]를 함께 오픈 소스로 공개했습니다. Anthropic의 멀티 에이전트 연구 시스템과 Manus의 Wide Research도 모두 orchestrator-worker 스타 토폴로지입니다. 이 책을 다 읽은 독자가 개념 뒤의 본질을 꿰뚫어 보고 제1원리의 관점에서 멀티 에이전트 시스템을 분석할 수 있기를 바랍니다.

[^ch10-kimi-swarm]: Moonshot AI, *Kimi Agent Swarm: 100 Sub-Agents at Scale*, 2026, https://www.kimi.com/blog/agent-swarm. GTC 2026에서 병렬 하위 에이전트 수 상한이 300개로 확장되었다고 공개했습니다. AgentEnv는 Moonshot AI와 KVCache.ai가 협력해 오픈 소스로 공개한 에이전트 훈련 샌드박스로, 2026년 7월 Kimi K3와 함께 출시되었습니다.
...
동시성 충돌은 이미 확립된 운영체제와 데이터베이스 기법으로 다룰 수 있는 파일 수준의 문제입니다. 연쇄 오류는 프로세스 비유가 더는 통하지 않는 곳에서 발생하므로 다릅니다. 프로세스는 바이트를 정확히 전달하지만 에이전트는 의미를 전달하며, 다시 전할 때마다 왜곡될 수 있습니다. 여러 에이전트가 자주 상호 작용하면 한 에이전트의 오류가 뒤따르는 에이전트에 의해 점차 강화될 수 있습니다. 정보가 갈수록 왜곡되는 ‘전화 놀이’와 같습니다.

구체적인 상황을 생각해 보겠습니다. 번역 시스템이 관리자 패턴(실험 10-3의 아키텍처)을 사용하여 관리자가 기술 서적의 장들을 여러 번역 에이전트에게 배정한다고 가정합니다.

```text
용어 에이전트: "reasoning"을 "推理"로 번역하지만, 중국어에서 "推理"는 보통 inference에 더 많이 쓰여 모호해짐
        ↓ glossary.json에 기록
번역 에이전트 A: 2장을 번역하며 용어집에서 "reasoning tokens"를 읽어 "推理 token"으로 번역
번역 에이전트 B: 7장을 번역하며 "inference latency"도 "推理 latency"로 번역
        ↓ 각 장의 번역문에 기록
교정 에이전트: 책 전체가 일관되게 "推理"를 사용하므로 용어도 일관되고 번역도 옳다고 판단 ✗
```

오류는 어디에 있을까요? ‘reasoning’(모델의 사고 과정)과 ‘inference’(배포 시 모델의 순전파)는 서로 다른 개념입니다. 하지만 용어 에이전트가 먼저 ‘reasoning’을 ‘推理’로 옮겼으므로 뒤따르는 에이전트도 ‘inference’를 만났을 때 자연스럽게 같은 단어를 선택했습니다. 서로 다른 두 개념이 하나의 번역어로 합쳐져 독자는 둘을 구분할 수 없게 됩니다. 올바른 선택은 ‘reasoning’에 ‘思考’(사고), ‘inference’에 ‘推理’(추론)를 쓰는 것입니다. 그런데 교정 에이전트는 책 전체에서 ‘推理’가 ‘일관되게’ 사용된 것을 보고 번역 품질이 높다고 결론 내립니다.

하나의 용어 오류는 세 에이전트를 거치며 일관되게 적용되었기 때문에 오히려 더 믿을 만해 보입니다. 이 책이 서문에서 설명했듯 reasoning을 思考로, inference를 推理로 구분하는 이유입니다. 최초의 실수는 환각이 아닐 수도 있고 그저 나쁜 용어 선택일 수도 있습니다. 어느 쪽이든 뒤의 에이전트가 그 실수를 강화할 수 있습니다. 근본 원인이 진짜 환각이라면, 예를 들어 번역 에이전트가 주의력 표류 때문에 존재하지 않는 용어 규칙을 ‘기억해 냈다면’ 같은 증폭 메커니즘이 적용되며 결과는 더 심각할 수 있습니다. 관리자 패턴은 부정확한 하위 에이전트 요약이 이후 모든 작업의 전제가 될 수 있으므로 특히 취약합니다.

이 연쇄를 끊는 핵심은 **교차 검증**입니다. 같은 사고 경로에 에이전트를 더 투입하는 것이 아니라 한 에이전트가 **독립적인 관점**에서 결론을 다시 살피게 해야 합니다. 앞선 에이전트의 사고 궤적은 무시하고 원래 증거와 최종 결론이 일치하는지만 검사합니다. 이는 5장의 제안자-검토자 메커니즘을 멀티 에이전트 환경으로 확장한 것입니다. 검토자의 가치는 코드나 서식 오류를 찾는 데만 있지 않습니다. 독립적인 심판으로서 전체 사슬이 놓친 모순을 식별하는 데도 있습니다. 위험도가 높은 결정에는 단위 테스트, 컴파일러, 데이터베이스 쿼리 같은 결정론적 검사를 사용할 수도 있습니다. 이런 도구는 서로의 오류를 강화하는 모델의 연쇄를 끊을 독립적인 증거를 제공합니다.

조기 종료에는 대칭적인 반대편인 **폭주 루프**도 있습니다. 동료 협업 절에서는 작업이 반쯤 된 상태에서 멈추는 루프를 다뤘지만 여기서는 끝없이 계속되며 결과를 악화하는 루프를 막아야 합니다. 자율 에이전트 루프를 운용한 경험에서 흔한 실패 모드 세 가지가 드러났습니다. 첫째는 **토큰 비용 폭주**입니다. 방치한 루프가 몇 시간 동안 실행되어 예산을 태우고 아무도 요청하지 않은 코드를 산더미처럼 만듭니다. 둘째는 **이해 부채**입니다. 루프가 코드를 빠르게 내놓을수록 엔지니어의 구현 이해가 더 뒤처집니다. 사람이 개입해야 할 때가 되면 아무도 시스템을 이해하지 못합니다. 셋째는 **인지적 항복**입니다. 설계자가 루프에 작업을 맡기는 데 익숙해져 점차 독립적으로 생각하고 검토하기를 멈추며 품질이 악화되도록 내버려 둡니다. 해법은 오류 증폭의 해법과 같습니다. 명시적인 예산과 중단 조건, 실제 관찰에 근거한 검증기, 그리고 단지 ‘시작 버튼을 누르는 사람’이 아니라 계속 ‘루프의 엔지니어’로 남는 사람이 필요합니다.

지금까지 이 장은 한 에이전트 집단이 업무에 어떻게 협업할 수 있는지 엔지니어링 관점에서 살펴봤습니다. 이제 초점을 다른 질문으로 옮깁니다. 하나의 목표에 이끌리지 않는 다수의 에이전트가 오랫동안 공존하면 무엇이 출현할까요? 다음 절은 최전선 연구를 탐구하므로 엔지니어링 독자는 필요한 부분을 골라 읽어도 좋습니다.
...
이 성찰은 8장에서 논의한 지속적인 진화와 다르다는 점에 유의하십시오. 생성형 에이전트가 일상 활동을 하는 중에 발생하며 즉각적인 내부 상태와 목표를 갱신하는 데 목적이 있습니다. 8장에서 업무 후 성찰은 기껏해야 후보 교훈입니다. 결과 평가, 궤적 간 종합, 후속 검증을 거쳐야 장기적인 역량 갱신이 됩니다.
...
경쟁 차원은 단일 에이전트 벤치마크에서는 결코 드러나지 않는 게임 이론적 행동을 끌어냅니다. 실제 실행에서 에이전트들은 서로 가격을 더 낮추며 가격 전쟁을 벌였습니다. 반대로 모든 경쟁자에게 이메일을 보내 단일 가격을 제안하고 가격 담합 동맹을 맺은 실행도 있었습니다. 일부는 내부 사고에서 담합이 ‘비윤리적이고 불법’임을 인정하면서도 ‘시장 안정화’를 명분으로 계속했습니다. 이 환경에서 에이전트는 정적인 환경이 아니라 자기 전략을 끊임없이 조정하는 상대와 마주합니다. 따라서 계획만 시험하는 벤치마크보다 실제 비즈니스에 가까우며 ‘경제적 출현’이 비유를 넘어 관찰 가능한 현상이 됩니다.
...
> **실험 10-8 ★★★: 음성 Werewolf 에이전트 시스템**
>
> Werewolf는 추론, 기만, 사회적 전략을 시험하는 고전적인 사회 추론 게임입니다. 이 실험에서는 AI 에이전트가 사람 또는 독립적인 LLM 사용자 시뮬레이터와 음성으로 플레이합니다. 사람이 없다는 이유만으로 자동 인수를 중단해서는 안 됩니다. 시뮬레이터는 실제 모델을 사용하고, 자기 좌석에 허용된 컨텍스트만으로 추론하며, 게임이 제공한 도구로 행동합니다.
>
> **아키텍처 설계**:
>
...
>
> **3. 실시간 음성과 자동 사용자 시뮬레이션**: 사람 경로는 9장의 음성 에이전트를 기반으로 합니다. 자동 경로에서는 독립 LLM이 해당 턴의 유일한 합법 도구를 호출하고, 선택한 발화를 실제 오디오로 합성한 뒤 실제 ASR API로 보냅니다. 게임은 오디오 이전 텍스트가 아니라 ASR 전사만 소비하며, 도구가 선택한 대상과 ASR이 해석한 대상이 다르면 fail-closed 합니다. VAD와 끼어들기는 사람 경로 전용 범위입니다.
>
> **4. 에이전트 사고와 전략**:
>
...
>
> **측정 결과(2026-08-01)**: [`voice-werewolf` 검증 기록](../chapter10/voice-werewolf/validation/runs/)은 실제 OpenRouter 호출과 네이티브 오디오 입력으로 자동 경로를 실행했습니다. 엄격한 독립 재검증은 해석 불가능한 “P1 is not”을 기권으로 잘못 처리한 초기 실행 두 개를 거부했습니다. 수정된 경계는 이제 ASR이 `abstain`, `skip`, `none` 중 하나를 명시하도록 요구합니다. 영향을 받지 않은 v2는 사용자 좌석, 역할 구성, LLM 도구, 합성 오디오, 실제 ASR, 두 번의 행동 일치, 세 번의 완전한 주기, 정보 격리, 규칙 기반 승자 게이트를 통과했습니다. 그러나 주민이 예언자를 잘못 추방해 전략 평가는 실패했습니다. 따라서 시스템의 end-to-end는 검증되었지만 전체 전략 품질은 아직 통과하지 못했습니다.
>
>
> ![그림 10-13: 음성 Werewolf 에이전트 시스템](images/fig10-13.svg)
>
>

## 장 요약

멀티 에이전트 시스템에는 컨텍스트 공유와 협업 토폴로지라는 독립적인 설계 차원 두 가지가 있습니다. 컨텍스트를 공유하면 각 에이전트가 이전 에이전트의 전체 컨텍스트를 물려받아 정보를 보존하지만 컨텍스트가 빠르게 늘어납니다. 컨텍스트를 공유하지 않으면 에이전트가 독립적으로 작업하며 정제된 핸드오프 패키지, 파일, 메시지를 교환합니다. 동료 협업은 소수 에이전트 사이에서 반복해서 개선할 때 적합하고, 관리자 패턴은 동적인 스케줄링이 필요한 업무에 적합하며, 탈중앙화 패턴은 책임이 대등하고 제어가 분산된 작업에 적합합니다.

이 패턴들은 토폴로지와 독립적이고 운영체제에서 영감을 얻은 두 구성 요소에 의존합니다. 에이전트와 런타임의 관계는 프로세스와 커널의 관계와 같습니다. 정적 접두부는 프로그램이고 궤적은 메모리이며 LLM은 시분할 CPU입니다. 데이터 평면은 공유 파일 시스템이며 네 종류의 마운트 영역을 가진 가상 디렉터리 트리로 나타냅니다. 에이전트별 작업 공간, 멀티 에이전트 공유 작업 공간, 외부 리소스, 내장 시스템 리소스입니다. 에이전트는 파일 경로를 전달하여 산출물을 교환합니다.

제어 평면은 메시징, 상태 조회, 종료, 리소스 스케줄링을 담당합니다. 에이전트는 메시지로 상태를 비동기 보고하거나 부모 에이전트가 하위 에이전트가 실시간 갱신하는 파일을 관찰할 수 있습니다. 파일은 전체 궤적일 수도 있고 합의한 진행 상황 파일일 수도 있습니다. 궤적은 모델이 볼 수 있는 대화 상태를 포착하며, 도구·세션 상태와 외부 부작용에 대한 별도 체크포인트·복구 계약을 함께 갖추면 마지막으로 확인된 지점에서 세션을 재개할 수 있습니다. 실시간·비동기·다자간 조율을 위한 제어 평면은 흔히 메시지 버스로 구현합니다. 조직 간 협업에는 A2A 같은 표준 상호 운용 프로토콜도 필요합니다.

최근 연구는 여러 에이전트가 단일 에이전트보다 나은지 판별하는 핵심 기준을 제시합니다. **협업이 생성 시점에는 없던 새로운 정보를 도입합니까?** 토론 모드처럼 여러 에이전트가 같은 텍스트를 다시 살피기만 한다면 같은 연산량을 쓴 단일 에이전트와 성능이 같습니다. 하지만 검토자가 코드 실행 결과, 렌더링된 스크린샷, 도구 검증 출력 같은 외부 피드백을 얻을 수 있다면 멀티 에이전트의 이점이 큽니다. **루프의 병목은 검증기다**라는 Loop Engineering의 주장도 이 점을 가리킵니다. 게으른 가짜 완료, 성급한 포기, 거짓 성공이라는 세 형태의 조기 종료를 막으려면 모델 자신의 주장이 아니라 실제 관찰에 근거한 검증기가 필요합니다.

단계 예산도 단순히 늘린다고 결과가 좋아지는 것은 아닙니다. 명시적인 예산 인식 메커니즘이 에이전트가 연산량을 합리적으로 배분하도록 안내해야 합니다. 관리자 패턴에서는 계획자의 역량이 전체 시스템의 병목이므로 가장 강력한 모델과 가장 세심하게 작성한 프롬프트를 계획하는 에이전트에 배정해야 합니다.

에이전트 수가 충분히 많아지면 누구도 설계하지 않은 집단 행동이 나타납니다. Stanford AI Town의 에이전트 25개는 스스로 소식을 퍼뜨리고 파티를 조율했습니다. Agentopia는 시뮬레이션을 10년으로 확장하고 Life Reward로 모델 학습에 사용할 가상 궤적을 선택하여 에이전트 사회에 축적된 ‘사회적 지혜’가 하위 업무로 전이되게 했습니다. Moltbook의 에이전트 150만 개에서는 디지털 종교와 기계 고유의 협업 프로토콜이 생겨났습니다. 경제적 차원에서 Vending-Bench Arena의 경쟁 에이전트들은 가격 전쟁을 벌이고 지시받지 않았는데도 가격을 담합했습니다. Pinchwork는 시장을 통해 에이전트가 서로를 고용하게 하고 RentAHuman은 에이전트가 암호화폐를 지급하여 물리적 업무를 맡을 사람을 고용하게 합니다. 이 사례들은 시장 메커니즘을 통한 탈중앙화 자원 배분이라는 새로운 조율 형태를 함께 시사합니다.[^agoric] 이 시장 기반 모델을 이 장의 세 협업 아키텍처와 어떻게 비교할지는 여전히 열린 질문입니다.

[^agoric]: 계산 리소스를 시장 메커니즘으로 배분한다는 발상은 새롭지 않습니다. Miller, M. S., Drexler, K. E. *Markets and Computation: Agoric Open Systems.* In Huberman, B. A. (ed.), *The Ecology of Computation*, North-Holland, 1988.
